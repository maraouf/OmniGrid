...
- Drawer port-link now learns the LAN suffix from curated hosts before falling back to the browser's location. Pre-fix the SPA naively used everything-after-the-first-dot of `window.location.hostname` — for an SPA loaded at `omnigrid.www.home.lan` that promoted bare `debian13docker` to `debian13docker.www.home.lan`, wrong because `www` is the SPA's own subdomain and the LAN sits on `home.lan` directly. New `_resolveLanSuffixFromCuratedHosts(promotingHost)` scans every curated host's `address` / `host_hostname` / `label` for FQDN-shaped strings (multi-label, non-IP, hostname-charset, not URL-shaped), then computes the longest-common-suffix label-wise across every FQDN found — that's the operator's authoritative LAN domain. So `debian13docker.home.lan` + `web01.home.lan` + `mail.corp.home.lan` → `home.lan`. Requires ≥ 2 labels to count. Fallback now takes the LAST TWO labels of `window.location.hostname` (instead of everything-after-first-dot) so both `omnigrid.home.lan` and `omnigrid.www.home.lan` correctly resolve to `home.lan`. Last resort is the bare hostname kept as-is when neither yields a multi-label suffix.
- Drawer port-link now auto-appends the browser's domain suffix to bare hostnames. Swarm reports node names as short hostnames (`debian13docker`, `web01`) which don't resolve in the browser unless the operator's DNS handles short names natively. When the resolved host has no dots AND the browser is currently at a FQDN (e.g. `omnigrid.home.lan`), `itemPortLink` extracts the SPA's domain suffix and appends it (`debian13docker` → `debian13docker.home.lan`). Skips when the host is an IPv4 literal (DNS suffix on an IP makes no sense) or when the SPA's own location has no domain suffix (single-label hostname like `localhost`). Operators with curated host `address` already set to a FQDN are unaffected — that value wins before the fallback chain runs.
- Drawer port-link host-resolution now walks `item.placements` for Swarm services. Pre-fix `itemPortLink` only consulted `item.node`, which is stamped on standalone containers only — services kept their node info in `placements[*].node` and the lookup fell through to the Portainer public-URL hostname. User-flagged: a service on `debian13docker` linked to `http://portainer.example.com:9618/` instead of `http://debian13docker:9618/`. The candidate-nodes list is now built in priority order — `item.node` first, then every running placement, then every other placement (stopped placements still carry a useful last-known node). Each candidate is matched against the curated host list (id / host_hostname / label); when matched, the curated row's `address` field is preferred. When no curated host matches, the raw Swarm node hostname is used directly (typically resolves in the operator's local DNS / hosts file). Portainer-URL fallback only kicks in when no placement-derived candidate resolved.
