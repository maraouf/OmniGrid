...
- Cleared a static-analysis weak-hashing alert on the RustDesk integration. The pseudo device-id sent in the RustDesk login is now derived from the server URL alone; it previously also folded in the console username, which a taint analyzer cannot distinguish from the password it arrives alongside, producing a false "password hashing" finding. No credential has ever been hashed here, and RustDesk auth is unaffected (the id only needs to stay consistent within a login). The non-security SSH resolve-trace cache key (MD5) and the host-provider cache-bust key (SHA-256) are also marked `usedforsecurity=False` to reflect their intent. No behaviour change.
