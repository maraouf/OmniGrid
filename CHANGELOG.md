...
- Profile → Notifications panel UX overhaul. The flat per-event matrix was getting unwieldy as the event catalogue grows; redesigned around three collapsible category cards (Operations / Health / Security) with a search input + Expand all / Collapse all controls. Each category header carries an active-count badge plus one chip per delivery medium showing column state (all / partial / none) — clicking a chip bulk-toggles every event in the category for that medium. Operations is expanded by default for first-time discovery; Health and Security collapse to keep the panel short. Search filters event rows live against the translated label or event key. Underlying data model (`profileForm.notify_events` keyed by bare event name with per-medium dict) is unchanged so existing helpers + dirty tracking continue to work. New CSS for `.notify-category` / `.notify-category-header` / `.notify-cat-chip` with all/partial/none colour tiers; new i18n family under `profile.notifications.cat_*`.
