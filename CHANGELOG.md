...
- **The AI's host-status view now matches the dashboard.** The assistant could report hosts as "Down" (e.g. routers, switches, printers) that the web dashboard showed as up — because the AI read the fast snapshot status, which lags the dashboard's live per-host re-probe. The AI now **re-probes any host the snapshot calls down/unknown using the same per-host check the dashboard runs** (bounded + time-budgeted so it never stalls a reply), so its status matches what you see on the Hosts page, and it groups any remaining problem hosts by their real status (separate Down / Unknown / Paused) instead of lumping them all under "Down".
