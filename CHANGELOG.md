...
- Release-calendar widget — upcoming Lidarr album releases now show their cover art in the day popover, matching the Lidarr app card. Album art often isn't published to a public image CDN, so the calendar previously fell back to a blank placeholder; it now uses the same most-reliable-first artwork resolver the app card uses (album art → artist art → MusicBrainz Cover Art Archive → the app's own media cover).
