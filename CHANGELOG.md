...
- **The AI's host-status view now matches the dashboard.** The assistant could report hosts as "Down" (e.g. routers, switches, printers) that the web dashboard showed as up — because the AI read the fast snapshot status, which lags the dashboard's live per-host re-probe. The AI now reconciles a stale "down"/"unknown" host to "up" when its most recent provider probe actually succeeded (a genuinely-down host, whose latest probe failed, correctly stays down), and it groups any remaining problem hosts by their real status (separate Down / Unknown / Paused) instead of lumping them all under "Down".
