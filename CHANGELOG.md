...
- **Host figures were missing from the Nodes view for hosts identified by address.** The Nodes and Hosts views looked up a host's monitoring target differently, and only the Hosts view understood the host's Address field. A machine configured that way showed full CPU / memory / disk on the Hosts page and nothing on its Node card. Both views now resolve it the same way.
