...
- Temperature chart polylines were invisible — Y-axis labels and X-axis labels rendered, but the chart body was empty even on hosts with multiple sensors. SVG `stroke` as an attribute does NOT evaluate CSS variables; only inline `style` does, and `:stroke="line.color"` was binding the literal string `"var(--primary)"` which the browser treats as an invalid stroke and falls back to the default. Switched the temperature chart's `<path>` to `:style="'stroke: ' + line.color"` so the resolved palette colour is applied correctly (#443).
