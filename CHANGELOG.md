...
- **OPNsense** — new app integration. Pin your OPNsense firewall / router (API key + secret) and the card shows version + **update-available**, **WAN gateway health** (online / total + per-gateway latency & packet loss), **running services**, **memory + load**, **firewall state-table count**, **DHCP lease count**, **uptime** and **temperature**. AI / Telegram actions: **status**, **list gateways**, **list services**, and **restart a service** by name.
