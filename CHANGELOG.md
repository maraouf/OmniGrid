...
- UI sprite (`/img/ui-sprite.svg`) now served via a discrete route with `Cache-Control: no-cache, must-revalidate` instead of through the catch-all StaticFiles mount. Pre-fix the browser's heuristic caching could hold a stale sprite copy across deploys, so additive symbol edits (new brand icon, new tab icon) wouldn't land until a hard refresh. With must-revalidate the browser sends If-Modified-Since on every navigation — unchanged sprite returns 304 (cheap), changed sprite returns 200 with the new symbols. Same headers as the SPA shell.
