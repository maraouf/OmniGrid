...
- Dependencies — bumped the in-browser Tailwind runtime (`@tailwindcss/browser` 4.3.0 → 4.3.1) and the dev-only JS linter (`eslint` 10.4.1 → 10.5.0). `package.json` keeps its `"*"` ranges (the lockfile pins the resolved versions); the committed `node_modules/` subtrees were updated to match.
