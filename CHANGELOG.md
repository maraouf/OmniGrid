...
- SNMP probe wall-clock budget against slow embedded devices is now an operator-tunable separate from the per-OID timeout. Pre-fix the budget was hardcoded as `max(5.0, (timeout + 2.0) * 2)` — with the default 5s per-OID timeout that gave only 14s, far too short for the ~60 OID operations the probe fans out (sysName / hrStorage / hrProcessorLoad / ifTable / ENTITY-MIB + vendor-private MIBs). Slow embedded snmpd (WD MyCloud-class NAS, network printers, low-power gear with ~500 ms RTT) blew past 14s on every cycle, hit the 5-failures auto-pause threshold, and only succeeded when manually resumed (cool-down clear gave the probe a fresh start). New `tuning_snmp_wall_clock_budget_seconds` (default 60s, range 5..600) sets the total wall-clock window for ONE probe; `tuning_snmp_probe_timeout_seconds` keeps its semantic as the per-OID UDP timeout (fast-fail on truly dead hosts). Both labels reworked in Admin → Config so the distinction between "per-OID timeout" and "wall-clock budget" is obvious.
...
- Worker-node service stats — multi-round fix. Per-node `/containers/json?all=1&size=1` sweep with `X-PortainerAgent-Target=<host>` merges every daemon's containers (handles agent-mode topologies). Tasks endpoint backfill via `/tasks` adds task-derived `(cid, NodeID)` tuples that aren't visible in any per-node listing (handles topologies where Portainer doesn't aggregate worker containers at all — tasks API is global Swarm state). For task-only cids, the size dicts stay unpopulated so disk renders as `—` (unknown) rather than the misleading `0 B`. `_one_container_stats` now logs the actual response status code on failure (`[stats] <cid> no stats — agent_target=<node> status=<X>, untargeted status=<Y>`) so operators can distinguish "404 — agent doesn't have the cid" from "connection error — Portainer agent isn't deployed on that node at all and there's no API path through Portainer" (the latter requires deploying the Portainer agent as a global Swarm service — operator fix, not a code fix).
