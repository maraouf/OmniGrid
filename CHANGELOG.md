...
- Admin / Settings page-title bar — each view's content area now renders a heading at the top with the ACTIVE sub-tab's icon + label, matching the avatar-dropdown entry icon for the parent view (Admin = shield, Settings = settings/profile). Visual continuity holds end-to-end: avatar dropdown → sidebar entry → page-title bar all share the same icon symbol. New helpers `activeAdminSection()` / `activeAdminSectionIcon()` / `activeAdminSectionLabel()` (and matching trio for `settingsSections`) in `static/js/app.js`; new `.page-title-bar` / `.page-title-icon` / `.page-title-text` CSS rules using existing tokens (#387).
