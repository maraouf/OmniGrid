...
- **Apps view — "Extras only" filter.** A new toggle next to the status filters shows only the apps that have a rich expanded card / AI skills (AdGuard Home Sync, Radarr, Sonarr, Seerr, Bazarr, APC, Speedtest, AdGuard, Pi-hole), so you can quickly narrow a large app list to the ones with extras.
