...
- **Apps — Radarr integration (movie library at a glance + AI actions).** Radarr instances pinned via Admin → Apps gain a per-instance **API key** field (Radarr → Settings → General → API Key) with a Test-connection button, following the same drawer / loading / error UX as Bazarr and Seerr. Once the key is set, the Apps card expands to show the four metrics that matter for a movie manager — total **Movies**, **Missing** (monitored, no file — amber when any), **Downloading** (the active queue), and **Disk free** on the library volume — plus a Radarr version + health-issue footnote. The data comes from a small set of v3 endpoints (library list + queue / disk / health), cached server-side with a refresh interval configurable per instance. The AI assistant and Telegram bot get three skills: **Radarr status** (read-only library summary), **Search for missing movies**, and **Refresh movie library** — both actions queue a non-destructive background task on Radarr. The API key is stored encrypted-style (never returned in the clear — only an `api_key_set` flag), keep-current-if-blank like every other secret.
