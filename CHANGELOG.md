...
- `<!-- INCLUDE: ... -->` server-side partial expansion in `_render_shell()`. `static/index.html` was at 14k lines and growing; the 18 admin sub-tabs (~5180 lines, ~36% of the file) are now extracted into `static/_partials/admin/<tab>.html` and inlined at request time. Path-traversal guard refuses anything outside `static/_partials/`; cache key tracks every partial's mtime so a partial edit invalidates the assembled cache on the next request without a restart. Same SPA semantics — Alpine processes the assembled DOM as one tree. `index.html` is now 9k lines; future large views (hosts, settings) can use the same pattern.
