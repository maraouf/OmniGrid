...
- **Telegram "Bot is typing…" no longer disappears mid-task on slow commands.** Long-running Telegram skill commands (e.g. `/seerr_suggest_movie`, which queries your library and TMDB across several pages) showed the typing indicator only briefly — Telegram clears it after ~5 seconds, so the chat looked idle for the rest of the work. The bot now keeps the indicator alive until the result is ready.
