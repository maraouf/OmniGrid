...
- Worker-node service stats regression — CPU / memory show for services whose tasks live on a Swarm worker, even on Portainer endpoint types that don't expose worker-node containers via `/containers/json` at all. First the per-node sweep (with `X-PortainerAgent-Target=<host>`) merges every daemon's containers into one list with definitive cid → host mapping. Follow-up patch: when even per-node calls don't surface worker containers, `gather_stats` now ALSO pulls `/tasks` (Swarm state, visible from the manager regardless of agent topology), extracts `Status.ContainerStatus.ContainerID` + `NodeID` for every running task, and registers task-derived cids into the running-stats pipeline so `_one_container_stats(cid, agent_target=node)` gets invoked on them. CPU + memory populate for worker-node services on every Portainer topology that exposes the tasks endpoint (which is universal — tasks are core Swarm state). `size_root` / `size_rw` stay at 0 for tasks whose cid wasn't in any per-node container list; that's the acceptable trade-off (disk size is secondary, and most operators read CPU + memory first). New `[stats] gather_stats: tasks added N cid(s) not present in container list` diagnostic surfaces the fallback path in Admin → Logs.
...
- OIDC callback's post-login redirect target now validated through a regex allow-list before it reaches `RedirectResponse(url=next_path)`. The path must match `^/[A-Za-z0-9/_.~%?#&=:@!$'()*+,;\[\]\-]*\Z` (RFC 3986 path / query / fragment chars only, must start with `/`). Pre-fix the `_safe_next` helper used a series of prefix checks and returned the user value verbatim — CodeQL's `py/url-redirection` taint tracker doesn't recognise prefix-only checks as a sanitiser, so the alert kept firing. Defensive prefix gates for `//`, `/\`, and any backslash also retained (some user-agents normalise backslashes to forward-slashes mid-flight, which can re-introduce a `//attacker.example` past a pure prefix check). Both call sites in `logic/oidc.py` (callback + login-start) benefit from the central tightening; defence-in-depth pattern matches `login.js:nextPath()` on the JS side.
- Cool-down regex `re.search(r"(\d+)s remaining", low)` in `main.py:_humanize_provider_test_error` bounded to `\d{1,10}` to close CodeQL `py/polynomial-redos`. The probe response text it parses is partially user-influenced (flows from `/api/snmp/test` body through provider-specific error formatters), and although Python's regex engine handles `\d+` linearly, CodeQL flags unbounded quantifiers on user data conservatively. 10 digits caps at ~317 years in seconds — well past any plausible cool-down value.
