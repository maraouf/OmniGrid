...
- Jellyfin / Emby — more streaming insight, for both brands at once: while something's playing the card now also shows the **distinct viewers and devices** ("2 viewers · 3 devices"); a **"N added this week"** chip surfaces how much new media landed in the last 7 days; the drawer draws a **daily peak-streams sparkline** plus a **"peak N streams today / peak over the window"** stat (OmniGrid samples the concurrent-stream count into a new history table — neither server keeps that time series itself); and a new **"Scan one library"** AI / Telegram action re-indexes a single library by name (e.g. "scan the Movies library") instead of the whole server. Sampler cadence and retention are tunable in Admin → Config.
