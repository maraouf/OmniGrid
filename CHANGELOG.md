...
- Container recreate-success path inspected the wrong container ID on every successful Portainer `/recreate` call — the response body (Docker inspect JSON, typically multi-KB) was truncated to 500 chars BEFORE `json.loads`, so the parser raised `ValueError` and the silently-caught exception left `new_container_id` pointing at the original (now-reaped) container. The follow-up inspect then 404'd on the stale ID, logged the "Post-recreate inspect 404 on new container `<old-12>` — can't verify image-id" warning, and skipped the silent-no-op detection that would have triggered the manual fallback path. Visible to operators as "Recreate ran, success toast lands, but the container's local digest never moves and the row stays flagged needs-update" (reported on `netbootxyz`). Fix in `logic/ops.py:do_update_container`: capture the FULL `r.text` into `recreate_response_full` BEFORE the 500-char log truncation, parse `Id` from the full body, then truncate `recreate_response_body` only for the operator-facing log lines. A new warning log line fires if the JSON parse itself fails (was a silent `pass` before) so the next failure mode is visible without a code dive.
