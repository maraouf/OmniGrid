...
- **Fing** — new app integration. Pin your Fing Agent / Fing Desktop (Local API key) and the card shows **online / total devices**, **offline count**, **new-device count** (an unknown device that recently joined the network), a **device-type breakdown**, an **online-device occupancy trend** sparkline (sampled locally, since Fing's Local API keeps no history), and a **device list** (online first, with type / vendor / IP + a 🆕 badge for newcomers). AI / Telegram actions: **network status**, **list devices**, and **is &lt;device&gt; online** by name / IP / MAC.
