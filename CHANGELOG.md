...
- Public pre-built images on GHCR — OmniGrid is now published to `ghcr.io/maraouf/omnigrid` so anyone can pull a known release without rebuilding from source. Tag layout: `:latest` floats to the newest minor; `:<MAJOR>.<MINOR>` (e.g. `:1.4`) floats to the newest patch on that minor; `:<MAJOR>.<MINOR>.<PATCH>` is the immutable per-build pin (use for rollbacks). Publish trigger is the `.github/workflows/publish-ghcr.yml` workflow firing on tags matching `v<MAJOR>.<MINOR>.[01]` — i.e. the operator-cut MINOR tag AND the first auto-PATCH that lands on top — so daily auto-PATCH churn stays in the maintainer-private registry. Package is public; `docker pull` works anonymously, no `docker login` needed for read access. Compose / Swarm wiring: `export OMNIGRID_IMAGE=ghcr.io/maraouf/omnigrid:latest` before `docker stack deploy --resolve-image=always`. README's Deploy section gains a new "Pull a pre-built image (no build step)" subsection; full publish-trigger contract + tag matrix lives in `docs/guidelines/deploy.md` under "Pre-built images on GHCR".
