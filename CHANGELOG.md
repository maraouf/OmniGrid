...
- **OPNsense** — new app integration. Pin your OPNsense firewall / router (API key + secret) and the card shows version + **update-available**, **WAN gateway health** (online / total + per-gateway latency & packet loss), **running services**, **CPU + memory + load**, **per-interface throughput** (↓ download / ↑ upload, rate-diffed from the live byte counters), **firewall state-table count**, **DHCP lease count** (Kea or ISC), **uptime** and **temperature**. AI / Telegram actions: **status**, **list gateways**, **list services**, and **restart a service** by name. **Follow-up patch:** corrected the diagnostics API calls that returned 0 — OPNsense's system endpoints use camelCase action names (`systemResources` / `systemTime`, not snake_case, which silently returned the login page), the service search is a GET (not POST), DHCP is now read Kea-first (the modern default) with an ISC fallback, and CPU% + per-interface throughput were added. On boxes where those system endpoints still stay
  unreadable, **memory % and load averages now fall back to parsing the firewall's own `top`-style activity output** (the same source CPU% reads from), so they no longer read 0; the firewall state-table count also tolerates the summary / paginated-list / bare-list shape variants.
