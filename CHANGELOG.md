...
- Annotated two non-security SHA-256 uses (the RustDesk device-id derivation and the host-provider cache-bust key) as `usedforsecurity=False` to reflect their intent — neither hashes a password / credential for storage — clearing a static-analysis weak-hashing alert. No behaviour change.
