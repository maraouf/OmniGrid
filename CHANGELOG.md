...
- UniFi per-app integration — admin-pinned UniFi chips get an expanded stat card (devices online / total, access points / switches / gateways, connected clients with wired / wireless split, console version) sourced from the official UniFi Network Integration API. API-key auth (UniFi site → Settings → Control Plane → Integrations → Create API Key; needs UniFi OS 4.x / Network 9.0+). AI / Telegram skills: UniFi status, List devices (grouped by type), Clients, and Restart a device (by name or MAC; requires a confirm). Follows the per-app encapsulation pattern (module + extender + editor / extras partials + catalog template + icon + Test-connection).
