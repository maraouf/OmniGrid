...
- **Sign in to a private image registry, so its services stop showing a false error (Admin → Registries).** OmniGrid asks each image's registry for the current digest to tell *update available* from *up-to-date*. Only Docker Hub could authenticate, so a **private** registry answered 401, no digest came back, and every image from it sat with a red **error** dot — on services that were running perfectly, with the reason buried in the logs. Add the registry's hostname, a username and a token, and those rows resolve normally. Each row has a **Test** button: name a repository (`user/image:latest`) and it runs the real update check, so a pass there is a pass on the next refresh; leave it blank and it only confirms the login is accepted, and says so. Passwords are write-only — saved credentials show as "Stored", blank keeps what's there, and a tick clears it. Saving re-checks your images straight away rather than waiting for the cache to lapse. Two registry quirks are handled: one that asks for a catch-all scope and then refuses to grant it (Forgejo does this — OmniGrid retries the way Docker itself asks), and one that wants plain HTTP auth with no tokens at all. Docker Hub credentials are unchanged, still `DOCKERHUB_USER` / `DOCKERHUB_TOKEN` in `.env`. A credential is only ever sent to the exact hostname it was stored for. Each row also decides whether to **verify the registry's TLS certificate** — on by default, and worth knowing that it previously wasn't a choice at all: update checks borrowed the Portainer connection's TLS setting, so running Portainer with a self-signed certificate quietly switched off certificate checking for every registry too. That decision is now per registry, and untick it only for a registry on an internal CA — the page says as much underneath the box when you do.
- **When a service won't start, the drawer can now tell you why — and put it back the way it was.** Swarm's answer to a service that died on startup is `task: non-zero exit (255)`, which is the exit status of a process whose actual complaint — a missing setting, a volume it couldn't write, an image built for the wrong architecture — went to its own log and vanished with the container. The drawer now has a **Why did it fail?** button that goes and reads that log, names the cause in plain language, and shows the log lines it based that on, so you can judge the verdict rather than take it on faith. It recognises the common startup failures: wrong architecture, a start command that isn't in the image, config it refused to parse, a required setting that wasn't supplied, permissions on a mount, a port already taken, credentials its database rejected, a migration that didn't finish, a dependency it couldn't reach, and being killed for memory. When it doesn't recognise the failure it says so and still shows you the log, because a confident wrong answer is worse than none. Alongside the verdict it offers only the fixes that cause actually warrants — never a restart into a permissions problem — including **Roll back to previous version**, which asks Swarm to restore the image, environment and mounts the service ran before its last update. That is the answer when something updated and stopped starting, and it is offered only when Swarm still has that previous version to give.
- **Ask the AI to bounce the port a device is on, without looking the port up yourself.** "Get the interface on switch52 holding mac 6c:63:f8:53:bd:1f and bounce it" now works end to end: the AI asks the switch which port has seen that address, then bounces the port it found. The address can be written any way you have it — colons, dashes, dots or bare hex. If the address turns up on more than one port it will say so and ask which, rather than guess, since one of them is usually a trunk to another switch and bouncing that would take out everything behind it. The lookup is read-only but still asks you to confirm before it touches the switch, and the confirmation now names the host and address so you can see what you are approving.
- **One-click fix for a Docker node that can't reach its socket.** When Troubleshoot finds that the socket is there but the SSH user has no access to it, it now offers to fix that for you instead of telling you which command to run. On TrueNAS the change goes through the appliance's own API so it survives an upgrade, and the result is confirmed by actually re-opening the socket rather than trusting the command's exit code. It asks first, and says why it asks: access to the Docker socket is equivalent to root on that machine.
- **Optionally save a device’s configuration before rebooting it (Admin → SSH).** Answering "yes" to a switch’s *"you haven’t saved your changes"* question does not save them — it means "carry on anyway", so they are lost. If you want unsaved changes to survive a reboot, set a save command (Cisco gear uses `write memory`) and OmniGrid runs it first. Left blank — the default — nothing changes: the device reboots onto its last saved configuration. Note that the port-bounce safety net relies on changes *not* being saved, so think twice before enabling this on a switch you bounce ports on.
- **Bounce a single switch port (`/bounce`, AI, Cmd-K).** When one device on a switch misbehaves, the fix is usually a link-flap on *its* port — not a reboot of the whole switch, which drops every other port with it. `/bounce <switch> <interface> [seconds]` shuts the port, holds it down (30s by default, tunable and overridable per run), and brings it back, then notifies you. You can also just ask — "bounce port 37 on switch52" — from Telegram or the AI sidebar, or type `/bounce switch52 gi37` into Cmd-K. Every run is confirmed first, lands in History with who ran it, and refuses an interface name that isn't a plain interface name.
- **Optional dead-man switch for port bounces (per host, Admin → Hosts → SSH).** Bouncing the port your own SSH session runs through would normally leave it down until someone walks to the rack. With the failsafe enabled, OmniGrid tells the switch to reboot in N minutes *before* it touches the port and cancels that reboot only once the port is back — so a self-inflicted lockout resolves itself by reverting to the saved config. It refuses to bounce at all if the switch doesn't confirm the reboot was scheduled, and tells you plainly if the cancel didn't land.
- **Optional: native tool-calling per AI provider (Admin → AI → each provider card).** When the assistant needs to look something up — recent logs, failure history, a host's full record, an SSH diagnostic — it currently has to *write the request out in text* and OmniGrid parses it back. That's why it sometimes forgot the call or malformed it. Ticking **Use native tool-calling** instead sends those tools as structured definitions the provider itself validates, so a malformed or incomplete call stops being possible. It's **off by default and per-provider**, because each provider's format differs and wants confirming once: turn it on for one provider, ask a diagnostic question ("why is &lt;host&gt; paused"), and check it still answers. If anything looks off, untick it — the existing text path stays in place as a fallback.
- **The assistant can now actually answer questions about a specific host.** Asking "tell me about &lt;host&gt;", "why is &lt;host&gt; paused", or "how much disk is left on &lt;host&gt;" used to get a vague answer — or "I have no data on that host" — because the assistant was only ever shown a summary of the first 30 hosts, in curated order. On a fleet larger than that, the rest were **invisible to it**. Now it always receives a complete roster of every host, problem hosts are prioritised in the detailed sample, and it can pull a host's **full record on demand** — live CPU / memory / per-mount disk, OS and hardware, uptime, network interfaces, temperatures, UPS / battery, pending package updates, detected ports, and per-provider state including *why* a provider paused and its last error, plus that host's recent failure history. It's read-only, works in both the web assistant and Telegram, and it now looks the host up instead of guessing or telling you to go check the web UI.
- **Resume auto-paused sampling straight from the assistant (`/resume`).** When a host's provider gets auto-paused after repeated failures, OmniGrid sends you a "Host sampling paused: &lt;host&gt; (&lt;provider&gt;)" alert. You can now just **reply to that alert** — "resume sampling for this" — and the Telegram or web assistant actually resumes it, instead of telling you to go click the chip in the host drawer. It reads both the host and the provider out of the alert. Also available as `/resume <host> [provider]` in Telegram and in the Cmd-K palette / AI-sidebar slash picker. Naming a provider resumes just that one; leaving it out clears the whole-host pause **and** every paused provider on that host. This one is **not** destructive — it only re-enables probing — so it runs without a confirmation step. To clear **every** paused host at once, ask "resume all paused hosts" in the Telegram or web assistant, or send `/resume all` — the reply lists which hosts were resumed, or says nothing was paused.
- **Update a host's OS packages from the AI, Cmd-K, or Telegram (`/osupdate`).** A new command runs a full OS package update on a host over SSH — `apt-get update / upgrade / dist-upgrade / autoremove` (or `yum update / upgrade`), plus Pi-hole and snap refresh when present, with an optional Raspberry Pi **firmware** step and an optional **reboot** after. Say "update dns01", "update dns01 with reboot", or "osupdate web01 firmware" to the AI; type `/osupdate <host> [reboot] [firmware]` in the Cmd-K palette or the AI-sidebar slash picker; or send `/osupdate <host> [reboot] [firmware]` to the Telegram bot (alias `/patch`). It's **admin-only and destructive** — it confirms first (inline chip in the sidebar, dialog in Cmd-K, typed-confirm on Telegram unless "Allow destructive commands" is on). Because an update can take minutes it runs as a **background job**: you get an immediate "update started" acknowledgement and a **notification when it finishes**, and every run is recorded in History. This is distinct from the existing `/update` (which pulls new Docker images) — `/osupdate` updates the host's operating-system packages. The SSH command's wall-clock cap is the new `SSH_UPDATE_TIMEOUT_SECONDS` tunable (default 30 min).
- **A second OIDC / SSO provider alongside Authentik.** OmniGrid now supports more than one OpenID Connect identity provider at once — **UnifiedSSO** ships as a built-in second provider next to Authentik. The **Admin → OIDC / SSO** tab gains a provider selector (shown when more than one is configured), and the login page shows **one "Sign in with …" button per configured provider**. Each provider is configured and tested independently, and admin rights are mapped per provider: Authentik keeps its **group-membership** model, while UnifiedSSO uses a **role claim** (a user with `role = ADMIN` becomes an admin, everyone else read-only). Existing Authentik setups are untouched — same routes, same redirect URI, same settings. **Auto-register (dynamic client registration):** for providers that support it, paste a provider-issued initial-access-token and OmniGrid registers itself and fills in the Client ID / secret automatically instead of copy-pasting them by hand.
- **Reboot a host from the AI (web sidebar, Cmd-K, and Telegram).** Asking the assistant to "reboot switch52" / "restart the opnsense box" now actually reboots the machine over SSH instead of replying that it can't — using the same device-aware reboot the Telegram `/restart` command uses (`reload` on a Cisco switch, `sudo reboot` on Linux). It's **admin-only and always confirms first** (an inline Yes/Cancel chip in the AI sidebar, a dialog in Cmd-K, and — on Telegram — only when "Allow destructive commands" is enabled, otherwise it points you at `/restart`). The target host must have **SSH enabled** (a read-only-monitored host returns a clear "enable SSH first" message). Every reboot is recorded in **History**.
- **OPNsense** — the app now shows **CPU temperatures**. The drawer lists the **current temperature per core** (coloured green/amber/red), and once a couple of days of samples accrue it adds a **max-temperature trend** sparkline ("Peak X°C · avg Y°C"). All from the data the app already pulls — no extra agent, and nothing crosses into host telemetry.
- **Reboot non-Linux devices over Telegram `/restart`.** The reboot command is no longer hardcoded to `sudo reboot` — network gear that isn't a Unix shell (an old Cisco switch reboots via `reload`, with a `Y/N` confirmation prompt) can now be rebooted too. Set a per-host **Reboot command** (and an optional **Reboot prompt answer**, e.g. `Y`, that's typed into the device's confirmation prompt) under **Admin → Hosts → SSH**, or a fleet-wide **Default reboot command** under **Admin → SSH**. Blank everywhere keeps the `sudo reboot` default for normal Linux hosts. **Follow-up patch:** a per-host reboot command now runs over an interactive shell session rather than an SSH "exec" request, because network switches commonly refuse exec (which made the reboot hang until it timed out). The device's output is also captured and shown in the failure reply — so if it stalls at a prompt, you can see exactly what it's asking and adjust the **Reboot prompt answer**. A reboot that the device acknowledges (e.g. a switch printing "Shutting down …") is now reported as success immediately, instead of waiting for the session to drop and timing out.
- **Manage a Portainer-less Docker node directly (over SSH).** A node running plain Docker with no Portainer can now be added under **Admin → Docker Nodes** — OmniGrid opens an SSH connection to its Docker socket (`/var/run/docker.sock`), reusing your existing **Admin → SSH** credentials (no daemon TLS, no certs, optional per-node user / port / password override). Its containers appear in the **Stacks** and **Nodes** views alongside the Portainer fleet, grouped by `docker compose` project, with a **"Direct"** pill and a user-chosen brand icon (e.g. TrueNAS) on the Node card, plus live CPU / memory / disk. Per-container **restart**, **remove**, and **update** (recreate with a fresh image pull — volumes / networks / env preserved) all work over SSH, with the action audit-logged the same as Portainer ops. Works additively next to Portainer, or as the sole backend if you don't run Portainer at all. Known limits (documented in `docs/guidelines/docker_nodes.md`): no stack/compose-level update on a raw daemon (update each container individually), and no Swarm-only actions (n/a for standalone Docker). A new `DOCKER_DIRECT_TIMEOUT_SECONDS` knob bounds the per-call SSH budget. TrueNAS SCALE is the reference target (enable SSH; its app containers are middleware-managed, so restart is safe but recreate may be reconciled). **Follow-up patch:** TrueNAS SCALE app stacks and containers — which the catalog namespaces with an `ix-` prefix (`ix-dockge`, `ix-dozzle`, `ix-syncthing`, `ix-beszel-agent`, …) — now show their brand icons in the Stacks view; the resolver strips the `ix-` namespace before matching, with both light and dark themes covered.
- **Update a `docker compose` stack on a direct-Docker node over SSH.** A Portainer-less node stores no compose file, so OmniGrid couldn't update its stacks — now you can record a compose project's **compose-file path** in **Admin → Docker Nodes**, and that stack gets a normal **"Update stack"** button that runs `docker compose pull && up -d` on the node over SSH (with the output captured in the op log).
- **Reach a direct-Docker node over TCP+TLS** (as an alternative to SSH). Pick **TLS** as the node's transport in **Admin → Docker Nodes** and paste the daemon's CA + a client certificate / key; OmniGrid talks to `tcp://host:2376` directly. SSH stays the default and the recommended option (it also enables the compose-stack update above, which TLS can't do).
- **Manage Swarm services on a Portainer-less Swarm manager.** If a direct-Docker node is a Swarm **manager**, OmniGrid now surfaces its **services** (replica counts, health, image-update status, placements, and the per-task failure history in the service drawer) alongside containers, and **Restart** works on them. The **Nodes** view shows the whole Swarm — a card per Swarm node (role / state / capacity), not just the manager — so a Swarm reached only over SSH/TLS is manageable without Portainer.
- **Direct-Docker stats fallback for Portainer worker nodes (opt-in).** When Portainer can't fetch a worker node's container stats — the "Portainer agent unreachable" case that shows the amber *Swarm agent unhealthy* banner and leaves CPU / memory bars empty — OmniGrid can now **SSH to that node's Docker daemon and read the stats directly**, so the bars stay populated. It's **off by default** and gated twice: a master toggle under **Admin → Docker Nodes**, plus the node's curated host must have **SSH enabled** in Admin → Hosts (matched to the Swarm node by hostname / address / label). It only fills stats Portainer couldn't — a working Portainer stats call is never replaced, and the unhealthy-agent banner still fires so you know the agent itself needs attention.
- **Portainer-unreachable graceful degradation.** When OmniGrid can't reach the Portainer API, the **Stacks / Services / Nodes** views now keep showing your **last known fleet** (instead of blanking out) with a clear **"Portainer unreachable since X — showing the last known data"** banner and a **Retry now** button; the data age keeps counting up so you know how stale it is. While Portainer is down, its actions (restart / remove / update / stack) **fail fast with a clear reason** rather than hanging, and any directly-managed Docker nodes keep working normally. A background probe checks Portainer every 30s (tunable via `PORTAINER_HEALTH_PROBE_INTERVAL_SECONDS`) and the banner clears automatically the moment it recovers.
- **FlareSolverr** — the card now shows the **oldest open session's age** ("Oldest session 7h", amber once it's been open more than 6 hours) so you can spot a **leaked / stale browser session** that a scraper never cleaned up — FlareSolverr itself doesn't timestamp sessions, so OmniGrid tracks when each first appeared. Reported by the **status** action too.
- **RustDesk** — the device list now shows **when each offline device was last seen** ("last seen 3d ago"), so you can tell a briefly-offline machine from a long-dead one at a glance.
- **Nginx Proxy Manager** — the card now flags **Let's Encrypt certs that failed to auto-renew** (expired certs that should have renewed themselves — re-run the renewal), and the drawer lists **which hosts are 404 / dead** by name. Both are reported by the **status** action too.
- **netboot.xyz** — the card now shows the **assets-directory disk usage** (the total size of the downloaded boot images), reported by the **status** action too. Shown when the webapp reports per-asset sizes.
- **Grafana** — the card's alerting line now reads **"N of M firing"** (firing rules out of the total), and a new **Test a datasource** action (AI / Telegram) health-checks a single datasource by name and reports whether it's healthy. The firing count is shown against the total by the **status** action too.
- **GitSync** — each sync pair now shows its **time since last successful sync** (e.g. "synced 3h ago", or "never synced") with a stale badge when it's overdue, the card flags how many pairs are **failing** (their last sync errored — distinct from merely overdue), and a new **Retry failing pairs** action (drawer button + AI / Telegram) re-syncs only the failing pairs in one click. The failing count is reported by the **status** action too.
- **Forgejo** — the card now shows your **activity this week** (contributions — commits, PRs, issues, reviews — from your Forgejo heatmap) with a 14-day activity sparkline in the drawer, and a new **Sync all mirrors** action (drawer button + AI / Telegram) triggers a mirror sync on every mirror repo at once, instead of one at a time. The activity stat is reported by the **status** action too.
- **Jellyfin & Emby** — the card now shows **why** the active streams are transcoding (a "codec · bandwidth · subtitle …" breakdown beside the transcode count, read straight from each session's transcode reasons), the now-playing rows label each session's transcode cause, and a new **Stop all transcoding streams** action (drawer button + AI / Telegram, confirm-gated) frees the server's CPU in one click. Both apps get all of this from the shared media-server base.
- **Tdarr** — the card now shows an **"ETA empty"** estimate (how long until the transcode queue clears, at the recent completion pace) when there's a queue, and the drawer adds a **health-check pass rate** (the share of files that passed their health check, flagged amber when it dips). Both are reported by the **status** action too. (No new settings — the ETA reuses the existing Tdarr sampler.)
- **Tracearr** — the drawer gains a **top-offenders** list (the users with the most recent account-sharing violations) and a **top violation types** breakdown — rolled up from the recent violations since Tracearr has no aggregation endpoint — plus a **per-server bandwidth** breakdown (each media server's live stream bandwidth, derived from the active streams). The **status** action reports the top offender + top violation type too. The card itself is now leaner: the plays sparkline, streaming-quality bar, and platform chips moved into the drawer.
- **Tautulli** — the card gains a **"This week"** watch-time stat (total hours streamed in the last 7 days) and a **"Peak today"** concurrent-stream count, and the drawer adds the **most-active library**, the **most-played title** (last 30 days), the **peak concurrency** over 30 days, and a **concurrent-streams trend** sparkline backed by a new local sampler — so Tautulli now has an OmniGrid-owned streams-over-time trend like Plex and Emby instead of round-tripping its history on every render. All reported by the **status** action too. Two new Admin → Config knobs tune the sampler cadence + retention.
- **Kavita** — the card now shows a **reading-activity rate** ("~45 min/day read") with a drawer trend sparkline, derived by sampling the cumulative server-wide reading time and diffing it day-over-day (a reset/reinstall is skipped, never faked). Reported by the **status** action too. Reuses the existing Kavita sampler — no new settings.
- **Speedtest Tracker** — a new **below-floor reliability** stat: set your ISP-advertised download speed as a per-instance floor and the card flags the **% of recent tests below it** ("2 of 12 tests below 500 Mbps"). The floor is editable in both the Admin → Apps instance editor **and** the card's gear-flip settings (same setting, either updates the other), with a **Recommend** button that suggests a sensible floor (~90% of your median download) from the chip's own history over a chosen number of days. Defaults to off until set.
- **Rundeck** — the card now surfaces a **recent execution failure rate** ("2 of 12 recent runs failed", danger-tinted) — the CI-health signal a scheduler dashboard wants — reported by the **status** action too, with a drawer **failure-rate trend** sparkline backed by a new local sampler ("is my automation getting flakier?" over 30 days). It also shows the **average run time**, the **scheduled-jobs** count, the **executions-per-day** throughput (runs in the last 24h), and the **next scheduled run** ("Next: Nightly Backup in 2h", on Rundeck 4.x/5.x). Two new actions land as confirm-gated per-row buttons: **⏹ Abort** on each running execution ("kill the stuck job") and **🔁 Retry** on each failed recent execution (re-run it), both also available via AI/Telegram. Two new Admin → Config knobs tune the sampler cadence + retention. The drawer now also shows a **per-project failure-rate breakdown** (which project is flakiest — one row per project with finished runs, flakiest first), and the **status** action calls out the flakiest project by name.
- **FlareSolverr** — a new **Clear all sessions** action destroys every open browser session in one click (confirm-gated) — the cleanup operators reach for when sessions leak and start slowing the host. It loops over the live session list and reports how many were cleared (and any that wouldn't die).
- **App cards — generic diagnostics panel.** Every app card's drawer can now show an **Endpoint diagnostics** panel that explains *why* a value is empty or 0: the upstream's actual per-request HTTP status, parsed-row count, and response snippet, plus a self-diagnosed hint (e.g. a 403 → exactly which API privilege / credential to fix). Apps opt in by stamping the data; OPNsense is the first to use it (its services / firewall-states / DHCP diagnostics now flow through the shared panel), with more apps to follow.
- **OPNsense** — the card now **self-diagnoses** when a stat reads 0: the drawer shows a per-endpoint diagnostics panel (the firewall's actual HTTP status + row count + response snippet for the services / firewall-states / DHCP fetches) and, when a count is 0 because of a missing API-user privilege, a banner that names exactly which privilege to grant (e.g. *"Services: HTTP 403 — grant the API user the 'Status: Services' privilege"*). No more silent zeros.
- **netboot.xyz** — the card now surfaces an **untracked-assets** count (orphaned / old downloads not referenced by the current boot menu) as a warning-tinted stat, the drawer lists exactly which files are untracked, and a new **Clear untracked boot assets** action deletes them in one click (confirm-gated; only the untracked files are removed — tracked boot assets are never touched, and the computation refuses to run on an unrecognised catalog shape so it can't mis-flag a tracked file).
- **RustDesk** — the card now surfaces **stale devices** (registered peers that haven't checked in for a configurable window, default 30 days — "N machines to clean up", with a new **Stale RustDesk devices** action that lists the oldest first) and an **OS breakdown** of the fleet (Windows / macOS / Linux / …). The drawer also gains an **online-peers trend** ("peak N · avg M concurrent devices" over 30 days) backed by a new local sampler, since RustDesk Server Pro exposes only current peer state. Three new Admin → Config knobs tune the sampler cadence, retention, and the stale-device window.
- **Tdarr** — the card now flags an **idle node** (a worker node that joined and has capacity but is processing nothing — "1 node joined but not processing") and the drawer shows **per-node throughput** (live fps + active workers for every registered node, so an underused node stands out). The bloated-files list also gains a per-row **♻ Requeue** button (re-transcode one file straight from the list, confirm-gated) alongside the existing bulk requeue.
- **Tracearr** — the card gains a **violation-rate** stat (account-sharing violations per 100 plays — the abuse-intensity signal, reported by the **status** action too) and two drawer trend charts backed by a new local sampler: a **violation trend** ("is abuse trending up?" with an "▲ up / ▼ down N this week" caption — answerable now that the rolling 7-day count is sampled over time) and a **fleet concurrency trend** ("peak N · now M" active streams over 30 days, independent of Tracearr's own retention). Two new Admin → Config knobs tune the sampler cadence + retention.
- **Seerr (Overseerr / Jellyseerr)** — the request list in the app drawer becomes actionable: every **pending** request gets per-row **✓ Approve** and **✗ Decline** buttons (decline confirms first), so you can clear the approval queue without leaving OmniGrid. The buttons resolve the exact request, while the existing "approve / decline X" AI / Telegram actions still match by title.
- **UniFi** — the **device** and **client** lists in the app drawer become actionable: each device gets a per-row **Restart** button and each client a per-row **Block** button (both confirm first), targeting the exact device / client by MAC — the one-click drawer equivalent of the existing restart / block AI / Telegram actions.
- **Kavita** — the **libraries** list in the app drawer gains a per-row **Scan** button to re-index a single library, plus a matching "scan the <name> library" AI / Telegram action — alongside the existing scan-everything action.
- **Tdarr** — the live **worker** list in the app drawer gains a per-row **Cancel** button to abort one running transcode (confirms first), plus a matching "cancel the <file> transcode" AI / Telegram action — finer-grained than the existing cancel-all-jobs action.
- **OPNsense** — the **services** list in the app drawer becomes actionable: each service gets a per-row **Restart** button (confirms first) — the one-click drawer equivalent of the existing "restart the <name> service" AI / Telegram action.
- **Nginx Proxy Manager** — the card becomes a cert-safety dashboard: a **"Next cert"** stat showing how many days until the soonest certificate expires (coloured by urgency), a **"Plain HTTP"** warning when any enabled proxy host is served without an SSL cert, and a drawer **soonest-expiry certificate list** with status dots. Each Let's Encrypt cert gets a one-click **Renew** button (also a "renew the cert for app.example.com" AI / Telegram action), plus a **Renew all expiring certs** action that refreshes every renewable cert about to lapse at once. The status action now calls out the soonest expiry, any already-expired certs, and plain-HTTP hosts. (Custom-uploaded certs can't be auto-renewed — they're flagged, not renewed.)
- **Tautulli** — the card gains an **engagement stat** ("Avg watched 78%" — the mean completion across recent plays, plus the share actually finished, reported by the **status** action), a drawer-only **direct-vs-transcode trend** chart (two lines on one scale over 30 days with an "N% transcoded" caption — the signal that justifies a faster CPU / GPU), and a **Stop all transcoding streams** action (one-click "free the CPU" that ends every transcoding session at once, behind a confirm — also an AI / Telegram action).
- **Plex** — the card gains a **concurrent-stream trend** (a new local sampler drives a streams-over-time sparkline in the drawer + a "peak N · now M" stat, since a bare Plex server keeps no concurrent-stream history of its own), a **"+N this week"** chip counting library items added in the last 7 days (also reported by the **status** action), and a **Scan one Plex library** AI / Telegram action that re-indexes a single named section ("scan the Movies library") alongside the existing scan-all. Two new Admin → Config knobs tune the sampler cadence + retention. (Items-this-week is best-effort — hidden when the probe fails rather than shown as a misleading 0.)
...
- **UniFi** — the card now shows **per-AP client load** (the busiest access point — "AP-LivingRoom — 23 clients" — to spot an overloaded radio, plus a per-AP distribution in the drawer) and a **client-occupancy trend** ("clients over time" sparkline + "peak N clients"), backed by a new local sampler since UniFi's Integration API keeps no client-count history of its own. The busiest AP is reported by the **status** action (AI / Telegram) too. Two new Admin → Config knobs tune the sampler cadence + retention. The card now also shows **live WAN throughput** (↓/↑ Mbps) with a drawer **WAN detail** block (plan speed, uplink utilisation %, latency, public IP), and each per-AP distribution bar gains its **radio channel + channel-utilisation** ("ch 36 (5G) · 42% util", flagged red when congested) — both reported by the **status** action too.
- **Grafana** — the card turns into an observability mirror: it now reports **unhealthy datasources** (probes each datasource's health endpoint — "1 of 4 datasources unhealthy" with the names, the single most useful Grafana-admin signal) and the count of **firing alerts** (a 🚨 chip with the rule names, plus an amber "N pending"), from Grafana's unified-alerting API. Both are shown only when something's actually wrong, and are reported by the **status** action (AI / Telegram) too. (Datasource health + firing alerts need an Admin / alerting-capable token; they're hidden otherwise.)
- **qBittorrent** — the downloading list grows two per-row actions: **Recheck** (re-verify a torrent's files) and **Force-resume** (start it now, bypassing the queue) — also available as AI / Telegram actions.
- **qBittorrent** — a one-click **Toggle alternate speed limits** action ("slow it down / full speed") on the card + AI / Telegram, reporting which way it flipped.
- **qBittorrent** — the card now flags **stalled downloads** (a 🛑 "N stalled" chip — torrents stuck with no peers, the actionable seedbox alert) and shows the **longest active download** with its ETA and % done, so you can see at a glance when your queue will clear. Both are also reported by the **status** action (AI / Telegram). No extra calls — derived from the torrent list the card already fetches.
- **Prowlarr** — the drawer adds an **average response-time trend** (a query-weighted fleet average sampled per tick into a "are my indexers getting slower" sparkline with a "now Xms · peak Yms" footnote) on top of the failure-rate views.
- **Prowlarr** — two upgrades to spot and fix failing indexers faster: the drawer now shows a **per-indexer failure-rate distribution** (horizontal bars of the worst-failing indexers, colour-coded green/amber/red, from stats Prowlarr already returns — no extra calls), a richer view than the single "most failing" line; and a new one-click **"Test all indexers"** health sweep (also an AI / Telegram action) that tests every enabled indexer's connectivity and reports which are unreachable (with the reason and a FlareSolverr hint). The sweep tests each indexer individually under a time budget so it returns a clean result instead of timing out on a large indexer set.
- **Kavita** — the card is now a **reading dashboard**, not just an inventory count: it surfaces **active readers**, **total reading time**, and the **most-read series** (all from data Kavita already returns — no extra calls), the **status** action reports them too, and a new **"Most read"** action shows your most-read series as a **poster wall**. (Reading-activity needs an admin Kavita API key; it's hidden otherwise.)
- **Radarr / Sonarr / Lidarr / Readarr** — four upgrades across the whole \*arr family: a **"disk full in ~N days"** chip on every card (the forward-looking projection of your library's growth, now visible without opening the drawer); a **"Today"** chip counting what releases / airs today (movies, episodes, albums, books); a **download-queue-depth** sparkline in the drawer alongside the library-growth and free-disk graphs; and a per-download-queue-row **"Blocklist & search"** button (and AI / Telegram action) that, for a stuck grab, removes and blocklists the bad release then kicks off a fresh search — the one-click fix operators reach for on a stalled download.
- **Version-check cadence is now tunable** — Admin → Config gains a **"Version-check poll cadence"** knob (default 60s) controlling how quickly the "New version — reload" banner appears after a deploy. Lower it (e.g. 15s) to notice fresh deploys faster, or raise it to cut idle traffic.
- **AdGuard Home** — the card now surfaces the **slowest upstream DNS resolver** (the "why is DNS slow" stat) and your **custom-rule count**, and the drawer adds a **top-blocked-domain distribution** — horizontal bars of the 10 most-blocked domains, merged across every AdGuard instance. All from data AdGuard already returns (no extra calls). The **status** action reports the slowest upstream + custom rules too. New **per-client AI / Telegram actions**: "stop blocking ads for the &lt;client&gt;" disables filtering for one AdGuard client (by name or IP) fleet-wide, and "resume blocking ads for the &lt;client&gt;" turns it back on.
- **Stats dashboard** — a new **Samplers** card (before Total samples) shows how many sample-bearing tables are actively persisting rows out of the full roster, and the **per-table breakdown** now covers every sampler — including the per-app samplers (AdGuard / Pi-hole / ddns-updater / Fing / FlareSolverr / Speedtest / Prowlarr / qBittorrent / Tdarr / Kavita / Seerr / *arr) and per-host baselines / prayer-times — that were previously missing. New `*_samples` tables are now auto-discovered so the list can't silently drift again.
- **Apps nav** — a red down-count badge now sits on the Apps top-nav item (the app-level twin of the Hosts down badge), showing how many pinned apps are **down or degraded** at a glance — primed on every page load so it's accurate even before you open the Apps view.
- **APC UPS** — the card now draws a **battery / output-load / runtime-remaining trend**: a runtime-remaining sparkline (with its lowest point called out) plus battery-% and output-load mini-sparklines, over a tunable window (Admin → Config, default 7 days). No new infrastructure — it reads the SNMP samples OmniGrid already persists for the host, so a UPS card finally shows where the runtime headroom is trending instead of just the instantaneous number. **Follow-up:** the card now also shows **power-quality** readings from the PowerNet-MIB — **input** and **output voltage**, **mains frequency**, the **cause of the last transfer to battery** (brownout / blackout / sag / spike / …), the **last self-test result**, and a **"battery needs replacing"** warning when the UPS flags it. All read-only SNMP (no commands are ever sent to the UPS), persisted alongside the existing battery / load samples so the card renders straight from the database. The same power-quality rows now also appear in the **Hosts → host drawer UPS card** (the live host row), so a UPS host shows input/output voltage, line frequency, last transfer and self-test there too.
- **Nginx Proxy Manager** — the card now shows **access-list usage** (how many access lists exist and how many enabled proxy hosts are protected by one) with a new **"List access lists"** action that names each list and how many auth users / IP rules / proxy hosts it covers. The drawer also draws a **config-drift trend** — a proxy-host-growth sparkline with an "added N this week" caption, plus a plain-HTTP-host drift line when any site is served without SSL (NPM exposes no traffic metric, so OmniGrid samples the config counts over time). Sampler cadence and retention are tunable in Admin → Config.
- **netboot.xyz** — the card / AI can now list the **downloaded boot assets** ("what can I PXE-boot right now": the kernels / initrds / ISOs cached locally, with sizes) via a new **"Downloaded boot assets"** action and a drawer list — surfaced from data the dashboard probe already pulls. A new **"Update the boot menu"** action triggers a boot-menu update to the latest upstream release over netboot.xyz's socket.io control channel (confirm-gated, since it changes what every PXE client boots) — and reports honestly when it can't confirm the trigger rather than claiming a false success.
- **Grafana** — building on the datasource-health + firing-alert counts already on the card: a new **"Firing alerts"** AI / Telegram action lists the currently firing (and pending) alert rules as rows, each firing row with a ⏸ **Pause** button; and **"Pause / resume an alert rule"** actions toggle a rule by name via the provisioning API (pausing is confirm-gated — it stops the rule evaluating). The drawer also draws a **firing-alert trend** — a "monitor of the monitor" sparkline (OmniGrid samples the firing-alert count over time so you can see "alerts have been firing more this week"), shown only once the window has actually seen an alert fire. Sampler cadence and retention are tunable in Admin → Config.
- **Apprise** — the card now shows a **per-service breakdown** ("2 Discord · 1 Email · 1 Telegram") and, when the apprise-api config tags its URLs, **per-tag routing counts** ("#admins ×3" — which tag reaches how many endpoints). Two action upgrades: a new **"Test all endpoints"** one-click skill fans a test out to every configured endpoint ("is my whole alert chain alive?"), and the **"Send a notification"** action now supports **styled types** — lead the message with `success:` / `warning:` / `failure:` (or a ✅ / ⚠️ / ❌ emoji) and the AI / Telegram bot fires a coloured alert instead of a plain info one. (Writing notification URLs from OmniGrid stays deliberately out of scope — a notification gateway is kept read-only.)
- **GitSync Connector** — the card now flags **stale sync pairs** (enabled pairs whose last sync is overdue past a tunable threshold, or that have never synced), names the **stalest pair** and the **longest-running** one, and — in the drawer — draws a **mappings-synced growth line** ("+N this week") plus an **alert-error trend** when the window saw any errors (OmniGrid samples the mirror totals into a new history table; GitSync charts none itself). The **status** action reports the stale-pair count + stalest / longest-run too. Sampler cadence, retention, and the stale-pair threshold are tunable in Admin → Config.
- **Forgejo** — the card now answers "what needs me?" instead of just raw counts: it surfaces **PRs awaiting your review** and the number of PRs / issues **assigned to you** (far more actionable than the total open count), and — in the drawer — draws an **open-backlog burn-down sparkline** with a **"backlog up/down N this week"** caption (OmniGrid samples the open-PR/issue backlog into a new history table; Forgejo charts none itself). Two new AI / Telegram actions: **failing CI / Actions runs** aggregates the failed Actions runs across your most-recently-updated repos ("is my CI red?"), and **sync a mirror** triggers a push/pull mirror sync for one repo by name. Sampler cadence and retention are tunable in Admin → Config.
- **Fing** — new app integration. Pin your Fing Agent / Fing Desktop (Local API key) and the card shows **online / total devices**, **offline count**, **new-device count** (an unknown device that recently joined the network), a **device-type breakdown**, an **online-device occupancy trend** sparkline (sampled locally, since Fing's Local API keeps no history), and a **device list** (online first, with type / vendor / IP + a 🆕 badge for newcomers). AI / Telegram actions: **network status**, **list devices**, and **is &lt;device&gt; online** by name / IP / MAC. **Follow-up patch:** the same integration works with the **Fing Agent Docker container** — its Local API is off by default, so enable it (and set a port + key) from the Fing app / web dashboard, then point the chip at that port (default 49090; the container's published port 44444 is UPnP discovery only, not the API). The catalog template now covers both ports, and a chip discovered as either "Fing" or "Fing Agent" gets the same
  Added a **Scan now** action (card button + AI / Telegram) — the Fing Local API is read-only and the agent scans continuously, so it force-refreshes the agent's latest device state on demand.
  card. The drawer now also surfaces the **new devices by name + MAC** (the "unknown device joined your network" detail, not just a count) and an **offline-but-known** device list (known devices that dropped off); a dedicated **New devices** AI / Telegram action lists the recently-joined devices with their MAC / IP and when they appeared, and the **network status** action now names the newcomers and reports the offline-known count.
- **OPNsense** — new app integration. Pin your OPNsense firewall / router (API key + secret) and the card shows version + **update-available**, **WAN gateway health** (online / total + per-gateway latency & packet loss), **running services**, **CPU + memory + load**, **per-interface throughput** (↓ download / ↑ upload, rate-diffed from the live byte counters), **firewall state-table count**, **DHCP lease count** (Kea or ISC), **uptime** and **temperature**. AI / Telegram actions: **status**, **list gateways**, **list services**, and **restart a service** by name. **Follow-up patch:** corrected the diagnostics API calls that returned 0 — OPNsense's system endpoints use camelCase action names (`systemResources` / `systemTime`, not snake_case, which silently returned the login page), the service search is a GET (not POST), DHCP is now read Kea-first (the modern default) with an ISC fallback, and CPU% + per-interface throughput were added. On boxes where those system endpoints still stay
  unreadable, **memory % and load averages now fall back to parsing the firewall's own `top`-style activity output** (the same source CPU% reads from), so they no longer read 0; the firewall state-table count also tolerates the summary / paginated-list / bare-list shape variants. **Further follow-up:** the running-services list now uses the correct **POST** grid call (it was a GET that silently returned nothing), the DHCP-lease count reads both the modern Kea (numeric active state) and legacy ISC backends over the correct GET endpoints, and the firewall-state / per-interface-throughput counters now coerce OPNsense's string-typed numbers — so services, DHCP, firewall states and the per-interface ↓/↑ throughput list populate instead of reading 0. (If any still read 0, the OPNsense API user needs the matching Diagnostics / Services / Kea page privileges.) **Further follow-up:** the **services** and **DHCP** counts now try every grid shape OPNsense has used across versions (POST bootgrid body, bare GET, and the explicit `current`/`rowCount` query for services; Kea GET + POST and both ISC endpoints for DHCP) and accept the rows under any of the `rows` / bare-list / nested shapes — and when none return rows they log the status + keys of each attempt so a missing page-privilege is diagnosable from Admin → Logs. New **interface throughput** action: ask the AI / Telegram bot for "opnsense interfaces" (or "interface throughput") and it lists each interface with its current **↓ downlink / ↑ uplink** rate, busiest-first.
- Speedtest Tracker — the card and the "latest speed test" action now show a **reliability stat**: how many tests **failed** (and the %) over the history window, so a flaky connection is visible at a glance. As part of this, the long-term median download/upload/ping and the trend sparkline now ignore failed (zero-result) tests, so a few failures no longer drag the "median" down. New **"run a speed test against &lt;server&gt;"** AI / Telegram action picks a named Ookla server (by name / location / sponsor) and runs the test against it.
...
- Jellyfin / Emby — more streaming insight, for both brands at once: while something's playing the card now also shows the **distinct viewers and devices** ("2 viewers · 3 devices"); a **"N added this week"** chip surfaces how much new media landed in the last 7 days; the card footnote shows the **total user count** and the **"N on deck"** (Continue-watching / resumable) count; the drawer draws a **daily peak-streams sparkline** plus a **"peak N streams today / peak over the window"** stat (OmniGrid samples the concurrent-stream count into a new history table — neither server keeps that time series itself); a new **"Scan one library"** AI / Telegram action re-indexes a single library by name (e.g. "scan the Movies library") instead of the whole server; and a new **"Send a message to viewers"** action pops a short on-screen message on every active session ("tell everyone dinner's ready"). Sampler cadence and retention are tunable in Admin → Config.
...
- AdGuard Home Sync — the card now also draws a **sync-reliability trend**: a "in sync N% of the time" headline plus a daily sparkline that dips on any day a replica fell out of sync, with an "M out-of-sync days" caption. OmniGrid samples each AGS chip's replica in-sync count into its own history table (the sync tool keeps no history of its own), so the trend builds up automatically and survives restarts. Sampler cadence and retention are tunable in Admin → Config.
...
- Proxmox VE per-app integration — admin-pinned Proxmox chips get an expanded stat card (nodes online / total, VMs running / total, LXC containers running / total, cluster CPU + memory utilisation, aggregate storage with shared-storage de-duplication, and the PVE version), sourced from a single `/cluster/resources` call. API-token auth (Datacenter → Permissions → API Tokens → Add; paste the whole `user@realm!tokenid=secret` value; a per-instance "Verify TLS" toggle, off by default for the self-signed cert). AI / Telegram skills: Proxmox status, List VMs, List containers, Start a guest, Stop a guest (graceful shutdown; requires a confirm), and **Reboot a guest** (graceful restart; requires a confirm) — with per-row ▶ Start / ⏹ Stop buttons on the VM and container lists. The card also shows the **cluster quorum** state (green Quorate / red No-quorum, for a real cluster) and the **last-backup age** (with a recent-failure count, from the per-node vzdump task log), both reported by the **status** action too; and the drawer gains a **cluster-resource trend** (CPU / memory / storage % over time) backed by a new local sampler since Proxmox keeps no such history of its own, with two Admin → Config knobs for the sampler cadence + retention. Follows the per-app encapsulation pattern (module + extender + editor / extras partials + catalog template + icon + Test-connection).
...
- App posters — title posters and thumbnails (across the *arr family, Seerr, Tautulli, Tracearr, Plex, Bazarr and any app that uses the image proxy, plus the release-calendar and AI-sidebar posters) now show an animated **shimmer while they load** instead of a flat grey box, so it's clear the image is on its way; the shimmer clears the moment the image paints (and respects the reduced-motion setting).
- Seerr (Overseerr / Jellyseerr) — the card now shows a **request-fulfilment speed** stat ("fulfilled in ~2.1 days median" — the median time from request to available over recent requests), the drawer adds a **request-state composition** chart (stacked available / approved / declined per day over the sampler window), and a new **Approve all pending** action (card button + AI / Telegram, confirm-gated) approves the whole pending queue in one shot.

### Changed

- **Large-fleet rendering and refreshes are lighter.** Inactive top-level views and settings/stat tabs are no longer kept in Alpine's reactive graph, only the active Hosts desktop or mobile layout mounts, and repeated item, stats, host, and sparkline requests are coalesced instead of overlapping. Stats and host updates now reconcile in place, while unchanged nested rows keep their existing DOM. The dashboard keeps the same views, filters, drawers, and live-update behaviour with less redraw work.
- **The Update confirm opens with its release notes already there.** It used to open on "Loading release notes…" and make you wait out the lookup — the registry, then GitHub — for an update the dashboard had usually known about for hours. The notes are now fetched in the background the moment an update is found: the server looks them up when its scan spots the new image, and the browser picks them up as soon as the list shows the update, so by the time you click they are normally just rendered. You will only see the spinner for an update that appeared seconds ago. It is careful with GitHub's hourly allowance: each new image is looked up once, a lookup that finds nothing is not retried in the background, and a browser asking at the same moment as the background lookup shares it instead of doubling it. Notes are tied to the exact image being offered, so a second push to the same tag fetches fresh notes rather than showing the previous ones.

- **A machine client with an API token can now fetch the dashboard page itself, not just the data behind it.** The page was gated on the browser session cookie alone, so anything holding a valid token — a monitoring script, a health check, anyone verifying a deploy — was redirected to a login form it has no way to complete. That mattered more than convenience: the page is assembled server-side from partials, and when that assembly goes wrong its failure mode is to drop the missing pieces silently, leaving one line in the log. Fetching the assembled page is the only way to see that, and it was the one thing a token could not do. Access is unchanged in substance — a token holder was already able to read the data the page displays, and the page itself is markup with none of it baked in; an invalid or absent token is still turned away exactly as before.

- **The trend graphs in an app's drawer now say what they are showing.** They drew a shape and nothing else — no scale on the side, no baseline, no indication of how far back they went — so a rising line told you something had grown but not from what, to what, or over what period, and a total printed beside the chart could not be related to any point on it. Each one now carries the value at the top and bottom of the plot, in that chart's own unit, and says which end is the oldest reading. 70 charts across 31 apps have it. The one remaining unlabelled frame is the small-tile speed-test sparkline, where a chart 32 pixels tall genuinely has no room for tick text — every tile size with the space for it shows the labelled chart instead. A confidently wrong axis is worse than the blank one it replaced, which is not hypothetical: the first attempt labelled every chart's floor as zero, and most of these charts do not start at zero. On a disk-space chart that never drops below 4 TB it would have drawn a floor the line never approaches.
- **The speed test's long-term trend was drawing its download and upload lines at different scales.** Both are megabits per second, but each was stretched to fill the plot against its own highest reading — so an upload a fifth the size of the download still climbed to the top of the chart, and the two lines tracked each other as though the connection were symmetric. They now share one scale, so the gap between them is the real gap, and the chart carries a proper axis: megabits on the left, ping in milliseconds on its own axis at the right, and the dates it spans along the bottom. That is the same treatment the recent-tests chart in the same drawer already had. A test now guards every app's charts against the same mistake — several lines in one frame each scaled to themselves.

- **Paired lines in one graph now share a scale, so the gap between them is the real one.** Wherever two lines of the same kind were drawn together, each was stretched to fill the frame against its own highest point — so they tracked each other no matter how far apart they actually were. UniFi showed a handful of wireless devices climbing as high as the entire client count; qBittorrent's upload sat level with a download many times its size; Prowlarr's grabs matched its queries; ddns-updater made a single failing record look like every record was failing; and the speed test drew an upload a tenth the size of the download at the same height. Each of those pairs is now measured against one scale. The speed test's ping keeps its own, because milliseconds and megabits are not comparable and pretending otherwise would be a worse distortion than the one being fixed.

- **A read-only API token can now read the logs and the AI call log.** Those were admin-only, so a read-only token -- which is the right shape for an automated or assistive client, since it cannot change anything -- could see every host's telemetry but not the logs explaining why a host was failing. That is backwards: the diagnostic is the part a read-only client is for. Everything that WRITES, including clearing the log buffer, stays admin-only. **Follow-up patch:** one read had been missed and the sentence above was ahead of the code — the live-log view itself still refused a read-only token while the three log-FILE reads beside it allowed one, so the same token could download a whole day's log from disk but not the much smaller in-memory tail of that same content. The protection was inverted rather than merely inconsistent. The live view now matches its neighbours; clearing the buffer is still admin-only.
...
- **"Resume all paused hosts" in the web assistant now actually resumes them.** The assistant replied "Resuming sampling for all paused hosts" and showed **✓ Ran: Resume hosts**, yet every host stayed paused. The action only worked on hosts ticked in the Hosts view, so with nothing ticked it quietly did nothing. With no selection it now resumes every paused host, whether the whole host or just some of its providers were paused, and the toast shows how many were resumed. Ticked hosts still limit it to those.
- **"Why did it fail?" no longer offers a rollback for a failure a rollback can't fix, and no longer calls every exit 137 an out-of-memory kill.** A Tautulli task that exited 137 was captioned "Killed — out of memory" above a red **Roll back to previous version** button — on a service whose running and remote images were identical, so there had been no update to roll back to, and for a cause the classifier itself declares unfixable by any action. Three things were wrong and all three are fixed: a cause named from the exit code now obeys the same action rules as one named from the log (only a genuinely unrecognised failure still offers rollback as a last resort); exit 137 is now reported as **"Killed by a signal"**, because 137 only means the process was killed — by the out-of-memory killer, yes, but equally by a stop that outran its grace period, a failing healthcheck, a drained node, or a person — and the hint now lists those and gives the command that settles which; and the log lines shown beneath a verdict are captioned honestly. Previously a harmless "Disabled HTTPS because of missing certificate and key" warning was printed as the evidence for an out-of-memory kill, picked only because it contains the word "missing". When nothing in the log actually matched, those lines are now labelled as the last error-shaped output rather than as the reason. A log that genuinely says "out of memory" is still named as such, with the higher confidence that comes from having read it.
- **A multi-arch image no longer shows "update available" when the architecture you actually run hasn't changed.** `caddy:2-alpine` was reporting an update on an amd64 Swarm while simultaneously showing healthy and the same running version — because Caddy had republished only the ARM builds. A multi-arch tag has one top-level digest covering every architecture, so it moves whenever *any* of them is rebuilt, including ones your fleet never pulls. When the top-level digests disagree, OmniGrid now resolves both down to the image for the architecture each placement actually runs on and compares those; only a real difference is reported as an update. A service spread across architectures counts as current only when every architecture it runs on is current, and anything that can't be proven identical still reports an update — a missed update is worse than a spurious one. The extra lookups happen only on rows that would otherwise claim an update, so a fleet that is up to date costs nothing extra. `/api/items` marks these rows with `digest_differs_other_arch` so the reason is visible.
- **UnifiedSSO's brand icon never rendered, despite shipping with one.** The artwork was in the icon set but the resolver was never told about it, so every row fell through the "does this slug exist?" gate and drew a blank slot. The stack and all three of its services (`unifiedsso_web`, `unifiedsso_caddy`, `unifiedsso_auth-service`) now show the mark, as does a host whose icon is set to `unifiedsso` / `unified-sso` / `unified_sso`, or an item simply labelled "Unified SSO". The artwork was also refreshed from the app's own current icon (blue, was indigo). The wiring is now covered by tests, including two standing checks that catch the same class elsewhere: every registered icon slug has a file on disk, and every brand with a dark-theme variant actually ships the `-dark` file (a 404 that only appears on dark theme, so nobody working on light would see it).
- **Asking the assistant to find something and then act on it did the finding and skipped the acting.** "Find the MAC address on this switch and bounce its port" ran the lookup, reported the port, and stopped — no bounce, no confirmation prompt, no error. Asking for the same bounce *without* a lookup worked perfectly, which is what made it look intermittent rather than broken. The cause was that a reply arriving after a diagnostic step is assembled by different code from a reply that needs no diagnostic step, and only the second knew what to do with an instruction. So the instruction was read, understood and thrown away — a request whose whole point was the second half could never perform it. Both paths now behave the same: the instruction is carried out, and if it is something destructive you are asked to confirm it exactly as you would be had you asked for it directly. This also explains the "(empty response)" replies reported alongside it — when the entire reply was the discarded instruction, there was nothing left to show either.
- **A tenth of everything written to the log was the same DNS failure, repeated for hosts already known to be unreachable.** The port scanner looked each target up once, then handed the *hostname* to all 283 per-port probes, so every port repeated the same failing lookup. Each probe gave up after two seconds while the container's resolver was still working through its search domains, and the abandoned lookup surfaced later as an unretrieved error — arriving in bursts as wide as the scan runs ports in parallel. That was 7,809 lines in one day, for hosts the startup check had already reported as unresolvable, plus around seventeen seconds of scanning per host to prove it 283 more times. A name that does not resolve is now not scanned at all: one line saying so, and the same "does not resolve" result the caller already read. Nothing is invented in place of the scan — the port list comes back empty rather than as 283 ports reported shut, because a lookup that never reached a packet did not observe anything.
- **The assistant replied "(empty response)" instead of saying what it was about to do.** When a reply is nothing but an instruction to run something, the instruction is stripped out before the text is shown — so a model that answered with the instruction alone, and no sentence around it, left nothing to display. The request had succeeded and the action had been recognised; only the chat looked blank. It now says what it is about to do, using the action's own name. **Follow-up patch:** that was true of a reply given straight away, but not of one given after a diagnostic step — there the instruction was being discarded as well as hidden, so nothing ran either. See the separate entry above; both paths now say what they are doing AND do it. A destructive one reads as *proposed* rather than as done, because it has not run at that point and still waits on the confirmation beneath it.
- **Gemini's fast models started thinking again after a version bump.** Flash models are meant to skip the internal "thinking" phase so the palette answers quickly, but the rule that switched it off tested for the literal text `2.5` in the model name — so moving to `gemini-3.8-flash` silently opted out, and every call has been thinking since. The test now reads the version as a number, so a new version line keeps whatever behaviour its family already had. Pro models are untouched (they only operate in thinking mode and reject the setting), and a model whose version cannot be read is left alone rather than guessed at.
- **Newer OpenAI models refused every request, and the only visible sign was the Test button.** OpenAI stopped accepting `max_tokens` on its recent models and wants `max_completion_tokens` instead, so a ChatGPT provider set to one of them answered `Unsupported parameter` to the connection test — and would have answered the same to every actual question, since both paths sent the same thing. The obvious fix is the wrong one: DeepSeek shares this code, and the Base URL field exists so you can point a provider at a self-hosted server, and those accept only the old name. Matching on the model id is no better, because the names that changed share no pattern with the ones that did not, so it would need editing again for every future model and would misjudge a self-hosted one in the meantime. So OmniGrid asks instead of guessing: it sends the widely-accepted form, and if an endpoint refuses and names its replacement, it sends it the other way and remembers that endpoint's answer. One extra call the first time a provider is used, never one per question, and a self-hosted server is never handed a parameter it would reject. **Follow-up patch:** fixing that surfaced the next problem behind it — the parameter was accepted and the model then answered "Could not finish the message because max_tokens or model output limit was reached". These models reason before they write, and the connection test deliberately asks for a single token, so the model spent that one token thinking and produced nothing. Raising the test's budget would have been the wrong answer: it would burn real reasoning tokens on every click to learn nothing, because getting that far already proves the credential and the model id are right — a bad key still answers 401 and a wrong model still answers 404. The test now passes in that case and says why, rather than reporting a failure that isn't one. On real questions the same thing is a genuine failure, and it used to arrive as an empty reply you could not tell apart from the model having nothing to say; it now names the cause and points at the Max response tokens setting. That setting's help text also covers this now — it warned about Gemini's thinking mode but not about OpenAI's reasoning models, which hit the identical wall — and notes that the Test button is deliberately exempt from it.
- **The AI dashboard looked like it had ignored a model change.** After switching a provider's model, the summary line correctly showed the new one while the only row beneath it showed the old — which reads as a contradiction, though both were right: the row is a record of what the calls in that window actually ran on, and no call had used the new model yet. The rows now say which is which, so a model that is no longer configured is marked `previously used` rather than sitting there unlabelled next to a heading that disagrees with it. The explanation had existed only in a tooltip on the heading, which is not where anyone looks when two numbers appear to conflict.
- **Roughly a third of the log was two lines repeating themselves.** Reading the running system's own logs: one line per SNMP probe per host was 16% of everything written, and for network gear it said the same thing every time — a switch has no CPU or memory or disk to report, so it printed three blanks on every probe it will ever answer. Two internal "an update was dispatched" notices were another 15%, duplicating notices the samplers already write with the actual measured value beside them. The SNMP line now appears when a host is first seen and whenever what it reports *changes* — a switch that stops listing its interfaces, a server that stops reporting memory — which is the thing worth noticing rather than the reading itself. The two dispatch notices are silenced, joining others suppressed for the same reason; the updates themselves still reach the page exactly as before, only the log line is dropped. Combined with the naming fix in the previous release, the log should be around half its former size, and what remains is more likely to be worth reading.
- **When the assistant failed to find a MAC address, the explanation ran off the side of the sidebar.** The little note under a reply comes in two forms — one for something a tool worked out, one for something it couldn't — and only the first had been told it was allowed to wrap. The second is the longer of the two, because it quotes the switch's own words back at you, so it was the one that spilled out of the panel and got cut off mid-sentence. Both now wrap, and both can break a long address or hostname rather than pushing the box wider than the panel.
- **The Apps page was reading a week of history to draw two hours of it.** The little uptime sparkline on each app tile shows the last two dozen checks — about two hours' worth — but the query behind it read everything kept, seven days by default, and sorted the lot before throwing almost all of it away. It was the slowest query on the system. It now reads a window sized to what the chart can actually display, with a wide margin for gaps: on a test set of nearly a million rows, 521ms became 115ms for byte-identical results. The margin is calculated rather than fixed, so raising the check interval widens the window instead of quietly cutting charts short. One deliberate consequence: a service that has not been *checked at all* for longer than that window now draws an empty sparkline rather than a week-old one — the tile still tells you when it was last checked, right beside the chart. A service that is checked and *failing* is unaffected and keeps its full line, since a failed check is still a recorded result.
- **The UPS input-voltage graph was showing you a straight line regardless of what the voltage did.** It was drawn from zero upward, and mains never goes near zero — so a supply moving between 200 V and 231 V was squashed into the top eighth of the box and came out flat. Simply starting the graph at the lowest reading instead would have swapped one wrong picture for another: a rock-steady supply drifting three volts would then have filled the whole box and looked alarming. The graph now sizes itself to the readings when they actually move, and holds a fixed twenty-volt window when they don't, so a real sag stands out and ordinary wander still looks like ordinary wander. Battery, load and runtime graphs deliberately still start at zero — for those, zero is the thing you are watching for, and a line sitting well above it is the reassurance.
- **Three graphs stated a number without saying what it measured.** The UPS runtime graph was labelled `77` and `0` while its own caption alongside read `low 57m`; the two Prowlarr graphs gave a bare scale where the text around them was talking about milliseconds and percentages. All three now carry their unit. (Graphs that count things — streams, devices, waiting items — keep a plain number, which is correct for a count.)
- **A diagnostic that was a fifth of the log, and could not tell you which host it was about, now names its host and speaks only when something changes.** Reading the running deployment's own logs turned up `[beszel] efs-aggregate ?:` — 360 occurrences in an eleven-minute sample, 18% of everything logged, and every single one said `?` where the hostname belonged. The host key is not present at the point that line is built, so it never could say. A line a few above it had already been deleted for this exact pair of reasons, and the comment recording that deletion sat directly over a line that had inherited both. The host is now passed in, and the line reports on first sight of a host and whenever its totals actually move — which is when a disagreement with the disk chip would be worth noticing — rather than once a tick forever.
- **Per-chip port results open faster, and one of the queries behind them was showing up in the slow-query log.** The fleet-wide "newest sample per group" reads were rewritten some time ago to let the database seek an index instead of sorting everything it touched; the three single-host versions were missed. One of them turned up as the slowest query in a sample taken from the running system, on a path the interface uses to draw a chip's per-port detail — though those warnings come and go with load, so the rewrite rests on the query shape rather than on that one reading. All three now use the same form as their fleet-wide siblings, with the two results proved identical row-for-row, including where the newest row's columns are empty and where timestamps tie. The Apps sparkline rollup also gains an index matched to what it filters on, so it stops reading every per-port row only to discard it — its remaining cost was a different problem, since fixed — see the Apps sparkline entry above.

- **Asking the assistant to find and bounce a port now shows you what it found and what it did.** Three things were missing and all three are fixed. The switch port it resolved was already being sent to the browser but never displayed, so it appeared only if the reply happened to mention it — it is now stated plainly under the answer as "&lt;MAC&gt; is on &lt;port&gt; of &lt;switch&gt;". The bounce reported itself finished the moment it was dispatched, which is wrong: the port is deliberately held down for about thirty seconds first, so the chat said "Ran" while the port was still down and never spoke again. It now says "Bouncing &lt;port&gt; — down 30s, waiting for it to come back", follows the operation, and then says the port is back, or that it failed and why. And when the assistant announces a lookup but doesn't actually start one — which it still does occasionally — the reply no longer just ends in silence: it says so and offers to ask again. **Follow-up patch:** none of that helped when the lookup itself failed, which turned out to be what was actually happening — the assistant asked the switch, the switch did not answer with a port, and every path that could have said so was closed at once. The chip under the answer was only ever drawn on success, and the "didn't start it" notice stood down whenever a lookup had run, on the reasoning that running one counted as acting. It does not: fetching something is not acting on what came back. A failed lookup now says so under the reply, in the switch's own words — "Couldn't find &lt;MAC&gt; on &lt;switch&gt;: …" — and the same applies to every other thing the assistant can go and fetch, so a failed fetch can no longer reach you looking like an answer. The lookup also kept no record of why it failed; it now writes one either way, so the next failure is readable in Admin → Logs instead of having to be reconstructed. **Follow-up patch:** the assistant now gets one corrective re-ask when it narrates instead of acting — it is shown its own reply back with the results still in hand and asked to finish or explain, once, never in a loop. Running the matcher that decides this over the replies the switch work actually produced caught it firing on "The port is gi3. Let me know if you want me to bounce it." — a reply that had already answered and was politely handing the next step back. That is the most ordinary way to close such an answer, so a large share of perfectly good turns would have been re-asked and paid for twice. "Let me know" is now read as the hand-back it is rather than an announcement, on both sides of the wire.
- **"Find the port holding this MAC and bounce it" now finds the port.** It had been reporting the address as absent from a switch it was plainly on. The switch — an SG300 — answers `show mac address-table` one screen at a time and then waits for a keypress, so what came back was the banner, the command, and the table's heading, with the row underneath it never sent. Every reply stopped in the same place, one character into the word "Port". Paging is now turned off before the query, and the commands are sent one per line rather than joined with semicolons, which a switch reads as a single command it doesn't recognise — that had no effect while only one command was ever sent, and would have broken the moment a second was added. Nothing next to this caught it: the bounce commands sharing the same connection print no output at all, so they never meet the pager. **Follow-up patch:** the command was also being typed while the switch was still printing its login banner, arriving spliced onto the end of it. This switch echoes and buffers, so it accepted the command every time; one that ignores input while printing would have dropped it, and the result would have looked like a device that answered nothing rather than one that was never asked. The banner is now read and set aside before anything is typed, and a device that stays quiet is not waited on. A side effect worth naming: whether the command "worked" is now judged on its own reply instead of on the connection having printed a welcome, which previously counted as an answer on its own.
- **OmniGrid no longer reports an update to the build it just deployed.** For about ten minutes after deploying itself it showed an update as available, against the image it had that moment finished rolling out. Two things had to line up. The values shown after a restart come from a snapshot the previous container wrote, which exists so the page paints immediately instead of waiting on a fresh reading — but it carried the previous container's timestamp with it, so the new one counted its predecessor's reading as its own and saw no reason to take another. And the record of what each image's latest version is, which is kept so a restart doesn't have to re-check every image, was reloaded including the entry for OmniGrid's own moving tag — the one tag that had certainly just changed, since changing it is what caused the restart. The snapshot is now treated as what it is, last-known values rather than a current reading, so a fresh one is taken straight away; and tags that get republished in place are re-checked after a restart instead of being assumed. Pinned versions are still remembered, so a restart is no slower for everything else.
- **A tool call the assistant ran and lost is now recorded as failed, not as a success.** Every lookup the assistant performs writes a row to History. The row's wording already said what happened — "… → error" — but the status beside it was filed as a success regardless, because the outcome was worked out and then only used to build that sentence. Filtering History for failures therefore hid the exact calls worth finding, and the count of failed lookups read as zero however many there had been. The status is now the real one, and the row carries the reason as well.
- **Every host showed an error after the last deploy; fixed.** Clearing out a spent diagnostic dump in the Proxmox Pulse reader left behind the loop that read one of the values it had removed, so the reader raised on every run. Nothing was actually wrong with Pulse or with any host — but the per-host endpoint had no protection against its own reader failing, so one broken line returned an error for the whole page rather than for one provider. Both halves are fixed: the leftover loop is gone, and the Pulse reader now honours the promise its callers were already relying on — if it ever fails again it reports Pulse as down, with the reason in Admin → Logs, and every other provider and host keeps rendering. Storage totals on pooled hosts had not corrected either, because the endpoint that recalculates them was the one erroring; they update on the next refresh.
- **A busy AI provider is now retried instead of surfacing as a failure.** When the provider answers "currently experiencing high demand", that is it declining to start rather than struggling to finish -- but the retry rule treated a slow reply as a reason not to try again, and declined. The fallback provider could not step in either, because the size limit for handing a prompt to a backup provider was fixed at a value the AI sidebar's prompt has always exceeded, so a configured fallback could never engage there at all. Refusals are now retried, and that size limit is a setting in Admin -> Config with a default high enough to be a guard rather than an off switch.
- **Routine host-sampling traces no longer appear as errors in the log.** One provider's per-host diagnostic listed field names, one of which contains the word "error", and the log classifier read that as the line reporting one. Around twenty hosts every few minutes were filling the error view with something that was working as intended.
- **A NAS no longer reports several times the storage it has.** A TrueNAS host showed 24.5 TiB against a real 6.5 TiB. Pooled filesystems like ZFS quote the whole pool's free space inside every dataset's size, so adding the sizes up counts that one number once per dataset -- this host had eighteen datasets on one pool, each repeating the same 1061 GiB. Each pool's free space is now counted once and only the datasets' own usage is added. Hosts without pooled storage are unaffected: their totals come out exactly as before. The per-mount list still shows each dataset's own figures. The same roll-up on the Beszel side, used when an agent is configured with a list of extra filesystems, is corrected the same way. The same correction applies to a step that ran afterwards and re-added the per-mount sizes to cross-check them: on a pooled host that sum is always inflated, so it was replacing a correct figure with a wrong one — a firewall running ZFS reported 8.6 TiB of storage against a real 0.86 TiB. Hosts without pooled storage are unaffected; verified against every mount on the live fleet.
- **When an AI request fails, the sidebar now says why.** It showed a bare "Failed" whenever the response was not JSON -- which is exactly what a reverse-proxy timeout or gateway error looks like, since those answer with an HTML page no matter what was asked for. The reason was read, found unparseable, and thrown away. Both AI paths now run the response through the same formatter the rest of the admin screens use, so a gateway timeout reads as a gateway timeout.
- **The AI no longer says it will look something up and then stop.** Asked to find a device by MAC address on a switch and bounce its port, it would reply that it was going to check which port -- and nothing further ever arrived. It was following its own instructions: everything telling it when to go and fetch data described a troubleshooting question, and this was an action, so it never actually asked the switch. It is now told that an action whose target has to be looked up first needs the same lookup, and that announcing a lookup it has not started is the worst answer it can give.
- **The two-factor QR code on the login page now renders as a QR code.** When an account is required to set up two-factor authentication, the enrolment step shown at sign-in was falling back to printing the raw setup link as text instead of a scannable code, because it pointed at a file the QR library does not publish. The same step reached from Profile -> Security was always correct. A new check now refuses to ship a page that points at a vendored file which is not there.
- **Two front-end defects surfaced by the newer linter.** A duplicated field in the AI sidebar's saved-conversation shape, and an unused argument on one live-updates handler.
- **A NAS now shows the disk space across its pools, not just its system dataset.** Where several sources report a machine's disk, the one that measures the whole machine now wins over one that only sees a single filesystem, regardless of which reports last. Previously a TrueNAS host showed 394 GB (its root dataset) while the pools it actually stores data on — 13.4 TB — had already been reported and were being overwritten. Nothing to configure; hosts with only one source are unaffected.
- **A Docker node added directly (not through Portainer) now shows the same CPU, memory and disk as the Hosts page.** Its card showed only a core count and Docker's own disk usage. Two causes: those cards are built at the very end of a refresh, after everything that gathers host figures has already run, so nothing filled them in; and the lookup that matches a node to its configured host was case-sensitive, so a machine calling itself `TrueNAS` never matched the host set up as `truenas`. The cards are now filled from the last successful reading for that machine, and the match ignores case like every other one does. Separately, the Nodes and Hosts views had also drifted apart in how they resolve which address to monitor — only the Hosts view understood the host's Address field — and they now share one resolver.
...
- **Rebooting a host from the web assistant could come back as an HTTP 502.** The request used to stay open for the whole conversation with the device — up to about 25 seconds for a switch — which is long enough for a reverse proxy to give up on it even though the reboot was fine. It now runs in the background like a port bounce does: you get an immediate confirmation, the device’s own output appears in the running-operations panel and the history row, and a notification tells you how it went.
- **"Mark all read" in notifications gave no sign it was working.** The badge, the rows and the button itself all stayed exactly as they were until the server replied, so the click looked like it had done nothing and people clicked again — each time sending another request. Everything now marks itself read the moment you click, the button shows a spinner and won't fire twice, and if the request fails it puts the unread marks back.
- **"Stacks 1" with no Update button anywhere.** The badge counts every item running a stale image, but two kinds of item had no button to go with it: a direct-Docker compose stack that you'd expanded (its services update with the whole project, so they have no button of their own — and the stack's own Update button was hiding because the stack was open), and a service deployed outside any stack OmniGrid can update. The first now keeps its Update button while expanded. The second — and anything else counted but not clickable — now says why on the row, with a tip on what to do instead.
- **The status and health filter chips on Stacks / Services actually filter again.** Clicking **Offline**, **Degraded**, **Updates**, **Up-to-date**, **Errors**, **Unknown** or **Ignored** highlighted the chip (and the counts on the chips were always correct) but the table below kept showing every row — only a search, a sort, or a page reload appeared to apply it. An internal caching optimisation on the filtered-row list meant the table stopped being notified when the filter changed. The cache is gone; the chips now narrow the table immediately, and **All** restores it. Two related fixes came out of the same area: the **Updates** chip and its filter now agree (the chip counts only updates you can actually act on — a stopped/orphaned container with a stale image was excluded from the count but still appeared when you clicked it), and the header **select-all** checkbox no longer mis-reads as "all selected" when you have the same *number* of rows selected from a different filter (which also made select-all clear the selection instead of selecting).
- **Bulk actions no longer act on rows hidden by the current filter.** Selecting rows and then filtering used to leave the bulk bar offering, say, "Remove (5)" while the table showed three unrelated rows — and running it would have hit all five, including the ones you could no longer see. Update / Restart / Remove now apply only to the selected rows that are **currently visible**, and the bar shows "N hidden by filter" when the filter is hiding part of your selection. Your selection itself is never discarded — clear the filter and it's all still there.
- **The web AI assistant can now reboot a host by name** (e.g. "reboot ubuntu") from the command palette and the AI sidebar — matching the Telegram assistant, which already worked. Previously the web assistant correctly understood the request and named the target, but the host it named was never passed to the reboot action, so it aborted with "No host selected" unless that exact host's drawer happened to be open. The AI-named host now flows through every dispatch path (sidebar approve-then-confirm, sidebar autonomous, and the Cmd-K palette). The destructive-action confirmation is unchanged — you still confirm before anything reboots. This also works for a host that isn't currently on screen or is named by an alias/label (e.g. "reboot dns01" when its telemetry is stale) — the target is resolved against your full host list, not just the hosts currently visible to the assistant. Typing the host name directly into the Cmd-K palette — `/reboot <host>` → **Reboot a host** — now targets that host too (previously it ignored the typed name and said "No host selected"); a name that matches nothing gives a clear "No host matches …" message. And when the assistant confirms a reboot but only names the host in its written reply (some models skip the internal target directive), OmniGrid now recovers the target from your request / the assistant's reply instead of stalling on "No host selected".
- **Stacks / Services now reflect the real state promptly after a Refresh, a Cleanup, a write op, or an OmniGrid restart** — instead of showing old versions/status and only catching up a poll cycle (or ~30s) later. The items view serves the last snapshot instantly and rebuilds the cache in the background; that background rebuild now emits a completion signal so the view updates on its own within about a second of fresh data landing (the "Refreshing…" indicator clears when it arrives) rather than sitting stale until the next poll. After an OmniGrid restart the first rebuild is also faster: the resolved remote-image digests are now persisted and reloaded on boot, so the first gather reuses them instead of re-checking every image's registry from cold.
- **Direct-Docker nodes no longer show a false "all host-stats sources failed" error.** A node managed directly over SSH/TLS (not via Portainer, and with no Beszel/node-exporter/SNMP/etc. provider mapped to it) was flagged with a red "N SOURCES ✗ — all configured host-stats sources failed" badge, because the check assumed every node should report host stats once any provider is enabled fleet-wide. Those nodes run on Docker-derived signals by design, so they now show a neutral **"Docker"** chip (hover explains it) instead of the red error. A curated host whose providers genuinely failed still shows the red ✗.
- **Updating Portainer itself no longer shows a scary false failure.** Because OmniGrid manages containers *through* the Portainer API, recreating the **Portainer** container restarts that API mid-operation — your reverse proxy returns a `502 Bad Gateway` and the update used to report `inspect HTTP 502: … Container update failed` even though Portainer had (almost certainly) updated and come back. OmniGrid now recognises a Portainer self-update: the dropped connection is reported as **success** with a clear "Portainer is restarting to apply its update — refresh in ~30s to confirm it's back on the new version" message, instead of the red error. (To update Portainer with no blip at all, manage its host as a **direct-Docker node** — that path recreates over SSH and doesn't depend on Portainer staying up.) Updating any other container is unchanged, and a *genuine* Portainer error still falls through to the normal recovery path.
- **SSH to legacy network devices now works** (old Cisco / managed switches, PDUs, IPMI / iDRAC / iLO BMCs, ESXi shells, some NAS firmwares). These devices often speak only the older SSH crypto that modern OpenSSH disabled years ago, so an SSH **Run command** or **Telegram restart** failed before authentication with a bare *"No matching key exchange algorithm found"*. OmniGrid now appends those legacy algorithms (SHA-1 key exchanges, CBC ciphers, SHA-1 / MD5 MACs, and DSA host keys) to the SSH negotiation **as a fallback only** — a modern Linux host still negotiates modern, strong algorithms unchanged, and the legacy set is used solely when the device offers nothing better. A device too old even for that fallback now gets a clear, actionable error instead of the raw library dump.
- **Nginx Proxy Manager** — the "days until expiry" on certificates is no longer off by one at the day boundary. A certificate valid until the end of today now reads "expires today" (0 days) instead of "expired 1 day ago"; every near-boundary count is correct.
- Asset inventory no longer shares the same icon as Services (both rendered a box/package glyph). Asset inventory now uses a distinct clipboard-list icon in the admin sidebar nav, the admin page header, and the stats dashboard card.
- Apps — per-row action buttons (e.g. Bazarr's **search subtitles for this title** lens, qBittorrent recheck / force-resume, GitSync per-pair sync) now show a **spinner and disable while the action runs** instead of looking inert until the toast lands seconds later. Dispatching the same action against two different rows in quick succession also no longer silently does nothing on the second click.
- Stats — the **Samples** icon no longer collides with the **Apps** icon (both rendered the same grid glyph). Samples now uses a distinct layers icon in the stats sub-tab nav, the dashboard, and the Samples page.
- First page-load after a new release no longer throws `Alpine Expression Error: app is not defined` and freezes on the loading skeleton until a manual refresh. The Alpine bootstrap gate waited up to 2s for the `app()` factory and then started Alpine regardless; when the `app.js` ES-module import graph transiently fails to finish on that very first cold load (one freshly-versioned module file momentarily unavailable mid-deploy), the factory was never defined and Alpine threw against it. The gate now performs a single automatic reload in that case — invisible auto-recovery instead of a manual refresh — guarded so a genuine persistent failure surfaces the error rather than looping.
- Stats dashboard — the Apps card now reports **app-level** health (e.g. "1 degraded") so it agrees with the Apps page instead of showing the instance/chip-level count (a single degraded app with 2 down chips previously read as "2 down" on the dashboard but "1 degraded" on the Apps page). The instance count moved to the card's sub-line.
- Admin → Apps — the per-app editor fields (API key / credentials / Test connection) now appear for an app instance even when the chip isn't linked to a catalog template, as long as its name identifies the app. Previously the editor matched the app by catalog slug only (while the card's expanded section matched by name too), so an app pinned/named without a catalog link showed its card extras but no editor fields.
- Apps — an app that needs credentials (e.g. RustDesk) no longer fires a failing background data request when it hasn't been configured yet. A wide app card shows its expanded section by default, which was kicking a fetch even with no username / password entered — surfacing a console error and a brief failure flash. The card now shows its "no credentials set — add them in Admin → Apps" hint straight away and only fetches once the credentials are saved.
- Telegram `/upcoming` — a release list with many entries produced a message over Telegram's 4096-character limit, which Telegram rejected and the bot silently dropped (no reply). The reply is now built under a safe length budget — it shows as many releases as fit, then a "…and N more — narrow the window or filter by type" hint — so the command always replies. Telegram send failures are now also logged with the API's own reason (e.g. "message is too long") so any future delivery problem is traceable.
- Custom-dashboard release-calendar widget — the refresh spinner now reliably shows when you step to the previous / next month (previously it could flash imperceptibly on an already-loaded month, looking like nothing happened). **Follow-up:** the spin flag now reassigns the whole refreshing-state object rather than mutating a nested key, so the pill spins dependably on every month change (a nested-key update wasn't always picked up by the reactive binding).
- Admin → Logs — informational app status lines that happen to include a count label like `failed=5` (e.g. Tdarr / Seerr "fetched …" summaries) are no longer mis-coloured as errors. The log classifier now trusts a line's own declared level (`[app] INFO …`) over stray keywords in the body.
- Performance — the schedule-queue / audit-history views ran an un-indexed `COUNT` over the full history table (a ~290 ms slow query on a large audit trail). Added a matching index so the count and the listing both seek directly.
- Custom-dashboard release-calendar widget — fixed a `_arrCalSpin is not defined` console error that could fire when stepping months in some load orders (the per-widget module's spinner helper hadn't bound yet when the tile mounted); the widget now guards the call and falls back gracefully until it's available.
...
- **Tautulli card no longer crashes the dashboard.** The Tautulli app card's "plays by day" / "plays by hour" bar charts were drawn with a template loop nested inside the SVG, which the browser mishandles — it threw an uncaught error that could blank the rest of the page. The charts now render as a single SVG path (the same technique every other chart uses), so they draw correctly and can't take the UI down with them.
- **Plex / Tautulli / Seerr avatars now load.** A now-playing or requester avatar served from `plex.tv` redirects to wherever that avatar actually lives (a Plex CDN, gravatar, S3, …); the image proxy was refusing to follow that redirect and showed a broken image. It now follows the redirect to any genuinely public host while still blocking redirects to internal / LAN addresses.
- **Admin → Docker Nodes** — the per-node "Test connection" button now uses the same styled control as every other integration panel (it was a plain mismatched button), and a missing translation key on the Save button is fixed.

### Security

- **Every dependency refreshed; the five open advisories are gone.** They were all in the linting tools used while developing, never in anything the dashboard runs or serves to a browser, so nothing was exposed — but a quiet advisory list is what makes the next one visible.
- Cleared a static-analysis weak-hashing alert on the RustDesk integration. The pseudo device-id sent in the RustDesk login is now seeded from the app chip's URL field, read directly from the chip — never from the credential-bearing values the taint analyzer follows — so no credential reaches the hash. (Earlier attempts that dropped the username and then used the resolved base URL didn't satisfy the analyzer, because it treats every value returned alongside the password in one tuple as equally sensitive.) No credential has ever been hashed here, and RustDesk auth is unaffected (the id only needs to stay consistent within a login). The non-security SSH resolve-trace cache key (MD5) and the host-provider cache-bust key (SHA-256) are also marked `usedforsecurity=False` to reflect their intent. No behaviour change.
- The per-app image proxy (used to load app posters / thumbnails server-side) now re-validates every redirect hop instead of following redirects blindly. A redirect off an allowed image host to an arbitrary address — a LAN host or a cloud-metadata IP — is rejected rather than fetched and cached; same-host redirects (http→https) still work, and the legitimate cover-art redirect chain (coverartarchive → archive.org) is preserved via a per-app allowlist. App credentials are never sent to a cross-host redirect target.

### Internal

- **Python and npm dependencies were refreshed to current compatible releases**, including the committed npm lockfile and served vendor tree. This repository has no Composer package set.
- **OpenCode now has the same `/release` workflow as Claude**, with compact repository guidance in `AGENTS.md` and native Windows `python`/`npm` instructions.
- **The agent rule catalogue moved out of `CLAUDE.md` into `.claude/conventions/*.md`, one file per area.** `CLAUDE.md` is re-read in full at the start of every agent session, and at ~290 accumulated conventions the catalogue had grown to roughly a sixth of the context window — spent before any work began, most of it on rules irrelevant to the task at hand. All 294 rules are unchanged and verbatim in their topic file (backend core / backend data / hosts & providers / apps / AI & Telegram / ops & notifications / CSS / accessibility / i18n / Alpine / docs & deploy); `CLAUDE.md` keeps the architecture facts plus a one-line index carrying each rule's invariant and the file to open for the rest. `CLAUDE.md` went from 626 KB to 164 KB. Nothing ships differently — `.claude/` is excluded from the image build context.
- **A partial that names itself in an include marker is now refused before it can quietly delete page content.** Adding the shared chart-axis component introduced exactly this: its header comment documented its own call site, and the include expander is a plain search over the whole file with no idea what a comment is — so the example counted as a real edge and the partial included itself. The expander looped to its depth limit, and its response to markers it could not resolve is to strip them, so markup silently vanished from the assembled page while it grew by tens of kilobytes of duplicated content on the way there. The only outward sign was one line in the log. A lint rule now rejects a partial carrying a marker that names itself, and a test walks the whole include graph for cycles of any length.

- **Four notes that had quietly stopped being true, corrected where they were written.** All four were read as current within a single session and three of them produced a wrong conclusion before being checked: the engineering notes claimed an image-digest cache did not exist when it had shipped, claimed neither of two "still refreshing" flags reached the browser when both did, and the review backlog claimed the commit hook skipped the project-wide audits when it has run all ten of them, failing on warnings, since a second stage was added. The saved working notes still instructed future work to maintain the retired task list; those lines are gone, and the ones carrying advice worth keeping now point at this changelog. A record that reads as current is more dangerous than a missing one — it gets quoted.

- **New `/release` command, and a release is now one commit.** The release runs as ten named steps -- walk every changed file, sync the changelog and the docs that drift silently, then the gates cheapest-first, then one commit and a push that is confirmed to have moved the branch. It carries the traps this project has actually hit: a scoped lint is a DIFFERENT gate rather than a smaller one, since ten project-wide audits run only in full mode; and pushing is deploying here, so there is no later point of no return. Splitting a release across several commits is now explicitly against the agreement -- it produces a separate build and deploy for each piece of one change. **Follow-up patch:** each step now reports as `STEP n of 10` rather than a bare `STEP n`, so the progress line says how much is left instead of only where it is. The denominator is fixed at ten whatever happens -- a step that does not apply still counts, and a step that has to be retried is still that same step -- because a total that shrinks to flatter the remaining work is worse than no total at all.
- **`/pending` no longer reports two settled items.** The release-cycle document's steps that prune the retired task ledger, and the replacement tracking mechanism that is deliberately not being designed yet. A row nobody can act on, restated every run, is what stops a pending list being read.
- **Cut the log down to what is worth reading.** A sample of live steady state was 1,632 lines in eleven minutes, and two spent debugging aids accounted for a fifth of it: one announced its own existence on every host probe to prove a long-shipped fix had deployed (while printing `?` instead of the hostname, so it could not say which host it meant), and another dumped a raw JSON record every tick to work out which fields a monitoring version carries. Both questions were settled long ago. Also fixed a notification-delivery line that was filed as an error because it reports the severity of the notification it just delivered successfully, and a failure message that printed the word "error" followed by nothing at all when the underlying exception carried no text.
- **Stopped treating monitoring alerts as if they were virtual machines.** Pulse returns alert records inside the same arrays as guests, and they were being collected as though they were hosts. They are now recognised and skipped by kind. The check that decides what looks like a guest is deliberately left permissive -- tightening it previously excluded Docker containers and paused monitoring across the fleet -- so this only excludes record types that are explicitly not guests, and a test pins that it can never catch a container.
- **The commit-message hook that blocks AI attribution had a hole, and now has a test behind it.** The hook refused two of the three trailers in every environment, but the third -- the one prefixed with an emoji -- slipped through under a UTF-8 locale, which is exactly what git uses when it runs hooks. It now matches bytes rather than locale-decoded characters, so the result is the same everywhere. The new test found this on its first run inside a real commit; it also pins the carve-outs that stop the hook being routed around -- a genuine human co-author passes, and so does any message that merely mentions the CLAUDE.md file.
- **The task ledger is gone.** It had grown until every entry was a paragraph nobody read, costing more to maintain than it returned — first retired, then deleted outright along with every rule, agent instruction and pointer that referenced it, so nothing is left telling a future reader to update a file that no longer exists. This CHANGELOG is now the durable record of what shipped; anything code-complete but unverified is said when the work is handed over rather than filed. A replacement way of tracking releases is planned.
- **New `/pending` command.** Produces one numbered table of everything still open plus a single recommended next step, derived fresh on every run from uncommitted work, the unreleased CHANGELOG block, and the review notes -- never from memory. It is explicitly barred from suggesting a commit, a push or a deploy.
- **Type-checking tightened where it can be trusted**, which immediately surfaced two latent crashes, one of them already deployed. (An earlier attempt in this same unreleased block to keep the working agreement as a document in the repo has been removed — enforcement lives in the lint rules and the git hooks, which are checkable, rather than in prose that has to be read.)
...
- Dev tooling — the in-repo linter now runs EVERY rule by default (nothing opt-in). A new `py-chained-comparison` check flags `a < b and b < c` (chain it as `a < b < c`), and the three large-backlog rules (broad-except, missing-docstring, sibling-label inputs) are now default-on with their existing findings grandfathered via a per-file/rule baseline (`scripts/.lint-baseline.json`) — so the full-tree report stays clean while any newly-introduced instance is caught immediately. Regenerate the baseline with `--update-baseline`; see the whole backlog with `--no-baseline`. (the missing-docstring backlog was then fully written out — 156 real one-line docstrings across ~55 files — and the broad-except backlog was then cleared too — 71 async sites that could swallow a task cancellation got a real CancelledError/KeyboardInterrupt carve-out, the rest got the sanctioned inline marker — so only the sibling-label (accessibility) backlog remains grandfathered).
- Dev tooling — added a default-on linter rule (`py-none-or-attr`) that catches the recurring IDE warning where a loop-mutable variable is dereferenced (`.attr` / `[...]`) across an `x is None or …` short-circuit — runtime-safe but the type checker can't narrow it, so it warns "Member None of `X | None` has no attribute". The rule is scoped to loop-assigned names guarded by an explicit `is None` (so safe string/list emptiness checks aren't flagged), and the existing instances were fixed by hoisting the compared value to its own local. Full-tree lint baseline stays at zero.
...
- Dev tooling — the in-repo linter's four weak-warning rules (enclosing-name shadowing, unused parameters, bare `str(.get())`, duplicate code blocks) are now first-class default-on checks in every mode, including the full-tree scan, after the entire pre-existing backlog was cleared to zero. Fresh instances of these are flagged on every per-edit, pre-commit, and full-tree run rather than only on touched files. The shadowing rule now correctly ignores `global` / `nonlocal` rebinds, and each rule gained a per-function `# noinspection` opt-out for the handful of sanctioned-pattern exceptions (contractually-fixed signatures, intentional shape-similar blocks, meaningful nested-helper name reuse). Also added sixteen new default-on checks mirroring common IDE inspections — real-bug guards (mutable default arguments, `is`/`is not` against a literal or collection, always-true `assert (cond, "msg")` tuples, duplicate dict-literal keys, `return`/`break`/`continue` inside `finally`, bare `except:`,
  unreachable code after a terminal statement, `x = x` self-assignment, `raise NotImplemented`) and style/redundancy warnings (placeholder-less f-strings, `== None`, `== True`/`== False`, negated `in`/`is` comparisons, `type(x) == T`, unused `except … as e` bindings, redundant `.keys()` in for-loops). No runtime behaviour change.
- Dependencies — bumped the in-browser Tailwind runtime (`@tailwindcss/browser` 4.3.0 → 4.3.1), the dev-only JS linter (`eslint` 10.4.1 → 10.5.0), and its globals table (`globals` 17.6.0 → 17.7.0). `package.json` keeps its `"*"` ranges (the lockfile pins the resolved versions); the committed `node_modules/` subtrees were updated to match.
