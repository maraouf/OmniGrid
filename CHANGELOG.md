...
- "Unsaved" pill text now flashes subtly on a slow 3.5s opacity cycle (1 → 0.55 → 1, ease-in-out) instead of Tailwind's faster 2s `animate-pulse`. New `@keyframes subtle-flash` + `.unsaved-flash` utility class in `static/css/style.css`; applied to every unsaved-pill text span across 16 admin/settings sites (Profile, Notifications, Authentication, Apprise, OpenMeteo, Portainer, OIDC, Debug, Hosts editor, Host groups editor, Config, Version). The animated dot keeps its faster Tailwind pulse so the eye lands on the dot first then notices the gently-breathing word (#386).
