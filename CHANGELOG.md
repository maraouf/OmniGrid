...
- Cmd-K / Ctrl-K palette now opens reliably (fourth follow-up patch). Diagnostic data revealed Ctrl+K was firing the toggle TWICE per keystroke (false→true→false in one press, leaving the modal hidden). `window.__omnigridOpenPalette()` worked because it bypassed the hotkey path. Cause: the bubble-phase `handleHotkey` also matched the same combo and re-toggled despite the capture-phase listener's `preventDefault() + stopPropagation()`. Fix: stamp `e._cmdpal_handled` sentinel on the event in capture-phase; any subsequent listener bails out cleanly. Also added `e.stopImmediatePropagation()` for defence-in-depth.
