...
- SNMP-only host stat-bars + sparklines on the main Hosts list. Pre-fix `hostHasTelemetry()` only matched the four unix-style providers (Beszel / Pulse / NE / Webmin), so SNMP-only hosts (managed routers, switches, DD-WRT boxes, etc.) had their CPU / Memory / Disk stat-bars AND inline sparklines hidden on the main row even though the per-host probe had populated `cpu_percent` / `mem_percent` via UCD-net-snmp + hrProcessorLoad. The gate now includes `(h.snmp_name && h.snmp_enabled === true)`, lighting up bars and sparklines for SNMP-enabled hosts to match the other providers. Disk reads 0% honestly when the SNMP agent doesn't expose dskTable (e.g. dd-wrt) — accurate "no disk telemetry from this provider" rather than hiding the whole row's signal.
