...
- **The assistant replied "(empty response)" while quietly doing exactly what you asked.** When a reply is nothing but an instruction to run something, the instruction is stripped out before the text is shown — so a model that answered with the instruction alone, and no sentence around it, left nothing to display. The request had succeeded and the action had been recognised; only the chat looked blank. It now says what it is about to do, using the action's own name. A destructive one reads as *proposed* rather than as done, because it has not run at that point and still waits on the confirmation beneath it.
