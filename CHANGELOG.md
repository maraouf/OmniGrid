...
- **The AI no longer reports every "problem" host as "Down".** When asked about host status, the assistant grouped down + unknown + paused hosts together and labelled them all "Down" — so a fleet with 1 truly-down host and a handful of unreachable/unknown ones read as "7 Down". It now groups problem hosts by their real status (separate Down / Unknown / Paused sections) and the "Down" count matches the dashboard exactly.
