...
- Host-drawer port-scan detected-ports list now sorts by port number ascending across both protocols (TCP + UDP interleave) instead of the backend's response order, so the chip strip reads as a numeric sequence (`22/tcp · 53/udp · 80/tcp · 443/tcp · …`) for at-a-glance scanning. Stable tie-break on protocol (TCP before UDP for duplicate port numbers — rare but possible, e.g. dual-stack DNS on 53). UDP chips additionally render with a `chip--udp` modifier — thick info-blue left border — so the protocol is visually distinct from TCP at a glance while preserving the existing `pill-ok` / `pill-warning` curated/unknown green/amber semantics. New SPA helper `sortedDetectedPorts(host)`; new CSS rule `.chip.chip--udp` placed AFTER the `.chip.pill-*` variant grid so the source-order + specificity (0,2,0) cascade lets the explicit `border-left` and italic font win over both `.chip`'s `border` shorthand and `.chip.pill-warning`'s `border-color` shorthand.
