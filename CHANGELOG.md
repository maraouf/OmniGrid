...
- netboot.xyz per-app integration — admin-pinned netboot.xyz chips get a compact status card (reachable + running version, with an "update available" note when its webapp reports a newer release). No authentication needed (just set the chip URL to the webapp root, e.g. http://host:3000). AI / Telegram skill: netboot.xyz status. Follows the per-app encapsulation pattern (module + extender + editor / extras partials + Test-connection).
