...
- New developer-discipline audit scripts under `scripts/`. `scripts/audit_tunables.py` does a bidirectional drift check between the TUNABLES dict (declared knobs) and every `tuning_int(...)` call site (consumers); exits non-zero on either declared-not-consumed (dead TUNABLES) or consumed-not-declared (KeyError at runtime) drift. Recognises the typed `Tunable.X` enum-member shape, the bare-string shape, and a small catalogue of expected f-string dynamic-key patterns (`tuning_portainer_op_timeout_{tier}_seconds`, `tuning_snmp_walk_concurrency_{vendor}`). `scripts/audit_create_task_bypass.py` greps every `main.py` + `logic/*.py` source line for `^\s*asyncio.create_task(` patterns that bypass `spawn_background_task`; exits non-zero on any hit. Both scripts have zero dependencies beyond the standard library so they run in the bare image without `pip install`. Operator can run on demand; future CI step can fail-on-non-zero. Closes the recurring drift class that produced two dead TUNABLES (2026-05-12 Beszel pair + this round's service_probe pair) and nine bare `create_task` bypass sites.
- Background-task strong-reference wrapper now covers nine previously-bypassed fire-and-forget sites. The login-event notify path in `main.py`, the host-paused notify path in the host metrics sampler, three schedule-runner spawns (`_run_prune_node`, the `_run_prune_all_nodes` per-host fan-out, and the `_await_and_record` waiter), and four Telegram dispatcher spawns (cleanup, single-stack update, standalone-container update, service-via-stack update) all called `asyncio.create_task(...)` directly. Bare `create_task` stores the task ref only in a local-scope variable that goes out of scope on return; asyncio's GC can then collect the running task mid-execution, manifesting as silent failures (op started, history row never written, no error in logs). Routes every spawn through `_spawn_background_task(coro, label=...)` (in `logic/` modules via a lazy `import main as _main` to dodge the circular dependency), which adds the task to the process-level `_BACKGROUND_TASKS` set + attaches a done-callback that logs success / exception. The 5-node `_run_prune_all_nodes` fan-out gets the same wrapper per child task so the multi-task-amplification GC-collection risk surface (5 tasks × bare-create_task = 5× the original drift class) is closed at the same time.
- DUP-001 Phase 1 + 2a-c — extracted `iter_curated_hosts(*, require_enabled=True)` generator helper in `logic/db.py` and migrated 6 sampler / consumer sites (`host_baseline_sampler` / `ping_sampler` / `host_beszel_sampler` / `host_pulse_sampler` / `host_webmin_sampler` / `host_metrics_sampler._refresh_host_provider_config_cache`). Replaces ~70 LOC of copy-pasted `raw = get_setting("hosts_config") → strip → json.loads → isinstance(list) → iterate → isinstance(dict) → id-empty-skip → enabled-gate` boilerplate. The helper guarantees every yielded row is a non-empty-id dict with the enabled-gate applied; per-provider field filtering (different samplers care about different name fields) stays at the call site per the helper's docstring contract. `_walk_hosts_config()` kept as a thin back-compat wrapper. Future samplers consume the canonical helper from day one. Phase 2d-e (gather.py's in-band walks + further reduction of db.py's 4 curated_*_hosts() helpers) deferred to a follow-up batch — diminishing returns vs the merge-order discipline risk.
- `main.py` lint Phase 4 — refactored 139 FastAPI route handlers from `_admin: auth.User = Depends(auth.require_admin)` to the `Annotated[X, Depends(callable)]` form via two new type aliases (`AdminUser` / `CurrentUser`) at module-top. Closes the `Member 'None' of 'Any | None'` IDE-warning chain on every `_admin.username` / `_admin.id` access that comes from FastAPI's `Depends()` Generic widening to `Optional[Any]` in PyCharm's stubs. 22 handlers also needed a `*,` keyword-only separator inserted before `_admin: AdminUser` to satisfy Python's parameter-ordering rule (FastAPI rejects `Annotated[X, Depends(...)] = Depends()` so the bare default-sentinel workaround isn't available). Zero behaviour change — FastAPI's dependency resolution treats both forms identically; verified by walking every route's dependant tree post-refactor.

### Added

- Admin → Config tunables now carry a "no live consumer" badge next to any knob whose value silently no-ops at runtime. Pre-fix operators could spend time tuning a knob that no `tuning_int(...)` call site reads (BUG-003 drift class — caught two dead TUNABLES in this batch alone). New `effective_state()` resolver in `logic/tuning.py` scans `main.py` + `logic/*.py` once per process for both bare-string (`tuning_int("tuning_X")`) and typed-enum (`tuning_int(Tunable.X)`) consumer patterns plus the documented f-string dynamic-key catalogue (`tuning_portainer_op_timeout_{tier}_seconds` / `tuning_snmp_walk_concurrency_{vendor}`), then stamps `unused: True` on each declared knob whose key never appears on the consumer side. SPA's tunable form renders a small grey `no live consumer` chip next to the field label when set — operators see at-a-glance which knobs are decorative.
- HTTP / TLS-cert / DNS health probe — seventh host-stats provider. Auto-covers every curated host's `url` + `services[].url` with zero per-monitor configuration (one of the operator's frustrations with Uptime Kuma is the per-monitor manual setup overhead). New `logic/http_probe.py` runs three sub-probes per URL in one call: HTTP GET (status code + optional content-match substring), TLS handshake (extract leaf cert + compute `notAfter` → days-until-expiry), DNS resolution (`socket.getaddrinfo` in a thread executor). Sub-outcomes surface independently on the merged host row (`host_http_status_ok` / `host_http_status_code` / `host_http_content_match_ok` / `host_http_tls_expires_in_days` / `host_http_tls_subject` / `host_http_dns_resolved` / `host_http_latency_ms` / `host_http_error` / per-URL `host_http_urls[]` array) so the drawer card can pinpoint which layer broke. Lifespan-managed `host_http_sampler` writes per-(host, url, ts) rows into the new `host_http_samples` table; same skip-don't-synthesize discipline + auto-pause contract + per-host success / failure caches as the other six providers. New tunables under `tuning_http_probe_*` cover per-URL timeout, fan-out concurrency, sampler cadence (0 = inherit global), auto-pause threshold, DNS sub-probe timeout, TLS cert warning days, and per-host success / failure cache TTLs. Master toggle (`http_probe_enabled`) + alias CSV (`http_probe_aliases`) lives under Settings → Host stats → HTTP probe; per-host opt-in via `hosts_config[].http_probe = {enabled, urls?, content_match?, accepted_status_codes?, verify_tls?}` set from Admin → Hosts. One-shot test endpoint at `POST /api/http-probe/test`. Test-before-Save gate on the admin section follows the canonical Portainer / OIDC pattern. **Follow-up patch:** four user-facing UX surfaces landed on the same enrolment — drawer card under SYSTEM (per-URL status pill strip + TLS-expiry pill with three-state colouring against `tuning_http_probe_cert_warning_days`, DNS pill, TLS subject / issuer + latency rows, Resume button on the auto-paused chip), per-host editor block in Admin → Hosts (multi-line URLs textarea + content-match input + accepted-status-codes CSV + verify_tls toggle with the `_clean_host_http_probe` backend validator), `providerStates(h)` extension so the http_probe chip joins the per-row provider strip with the canonical `'down'` self-status when `host_http_status_ok === false`, and the existing `POST /api/hosts/{id}/provider/{provider}/resume` endpoint extended to clear the per-host `_http_probe_host_cache` on resume (the probe targets are operator-curated URLs, not credentialed APIs — no auth-cooldown timer to clear, just the success / fail cache). Chip iteration arrays in `static/index.html` (toolbar filter) and `static/_partials/admin/hosts.html` (per-row test result) extended to include `http_probe`. SPA save / load / import paths preserve the `http_probe` sub-dict end-to-end so a fleet-export → fleet-import round-trip keeps the per-host configuration intact.
- Switch-to-tag affordance now works for Portainer-deployed Swarm services (was previously container-only). Pre-fix `canRetagToLatest` returned `false` for `item.type === 'service'`, so the drawer button was greyed out on services even when the underlying stack supported the operation. The exclusion now keeps orphans out and services WITHOUT a `stack_id` out (rare — Swarm services deployed outside Portainer, where the backend has no compose file to mutate), but services that belong to a Portainer-managed stack qualify. The existing `submitRetagPopover` already routes stack-belonging items via the `/api/update/stack/{id}/retag-latest` endpoint with `image_repo` set to the service's own image — the compose-file mutation touches only that one image line, not every image in the stack. Workflow for the user's example: open `authentik_server` drawer → Switch to tag → type `2026.2.3` → stack-retag fires with `image_repo=ghcr.io/goauthentik/server` + `tag=2026.2.3` → Portainer pulls the new manifest and rolls the task.
- Drawer port-link now learns the LAN suffix from curated hosts before falling back to the browser's location. Pre-fix the SPA naively used everything-after-the-first-dot of `window.location.hostname` — for an SPA loaded at `omnigrid.www.example.com` that promoted bare `worker01` to `worker01.www.example.com`, wrong because `www` is the SPA's own subdomain and the LAN sits on `example.com` directly. New `_resolveLanSuffixFromCuratedHosts(promotingHost)` scans every curated host's `address` / `host_hostname` / `label` for FQDN-shaped strings (multi-label, non-IP, hostname-charset, not URL-shaped), then computes the longest-common-suffix label-wise across every FQDN found — that's the operator's authoritative LAN domain. So `worker01.example.com` + `web01.example.com` + `mail.corp.example.com` → `example.com`. Requires ≥ 2 labels to count. Fallback now takes the LAST TWO labels of `window.location.hostname` (instead of everything-after-first-dot) so both `omnigrid.example.com` and `omnigrid.www.example.com` correctly resolve to `example.com`. Last resort is the bare hostname kept as-is when neither yields a multi-label suffix.
- Drawer port-link now auto-appends the browser's domain suffix to bare hostnames. Swarm reports node names as short hostnames (`worker01`, `web01`) which don't resolve in the browser unless the operator's DNS handles short names natively. When the resolved host has no dots AND the browser is currently at a FQDN (e.g. `omnigrid.example.com`), `itemPortLink` extracts the SPA's domain suffix and appends it (`worker01` → `worker01.example.com`). Skips when the host is an IPv4 literal (DNS suffix on an IP makes no sense) or when the SPA's own location has no domain suffix (single-label hostname like `localhost`). Operators with curated host `address` already set to a FQDN are unaffected — that value wins before the fallback chain runs.
- Drawer port-link host-resolution now walks `item.placements` for Swarm services. Pre-fix `itemPortLink` only consulted `item.node`, which is stamped on standalone containers only — services kept their node info in `placements[*].node` and the lookup fell through to the Portainer public-URL hostname. User-flagged: a service on `worker01` linked to `http://portainer.example.com:9618/` instead of `http://worker01:9618/`. The candidate-nodes list is now built in priority order — `item.node` first, then every running placement, then every other placement (stopped placements still carry a useful last-known node). Each candidate is matched against the curated host list (id / host_hostname / label); when matched, the curated row's `address` field is preferred. When no curated host matches, the raw Swarm node hostname is used directly (typically resolves in the operator's local DNS / hosts file). Portainer-URL fallback only kicks in when no placement-derived candidate resolved.
- Service / container drawer now surfaces every exposed port with a clickable external link. New `ports: list[dict]` field on every item in `_gather_impl` carries `[{published, target, protocol}, ...]` entries extracted from `Endpoint.Ports` for Swarm services and from the container-list `Ports` field for standalone containers. The drawer renders an Exposed-ports section between Placement and the task-error panel, each row showing `<published> → <target> <protocol>` with an `Open ↗` link when a host can be resolved. Host-resolution chain: (1) for containers, match the curated host by `item.node` (Swarm-node hostname) — prefer the row's `address` field; (2) fallback to the hostname from `settings.portainer_public_url` (ingress-mode publishes land on every Swarm node); (3) last resort `window.location.hostname`. Always emits `http://` (Docker port publish doesn't imply TLS). The link is hidden when the port can't be navigated in-browser (UDP / SCTP).

- AI palette backend now re-parses the SECOND-round AI reply for follow-up TOOL directives. Previously only the first-round reply was parsed — when the AI's post-dispatch reply emitted a new TOOL: directive (canonical case: first round surfaced a container ID, second-round emits `TOOL: docker_container_du` with the corrected container_name), the directive lines stayed visible in the chat AND no follow-up dispatch fired. `api_ai_palette` now calls `parse_palette_tool_calls` a second time on the second-round response; cleaned text replaces `out.text` (so `TOOL:` lines never leak into the visible reply) and confirm-required follow-ups are surfaced as a fresh `pending_tool_confirms` envelope. Read-only follow-up tools fire inline immediately and their results merge into the existing `tool_results` envelope. The SPA's `confirmInlineToolDispatch` already re-stamps + auto-chains from this so autonomous mode walks the whole chain without operator intervention.
- Missing i18n key `admin_hosts.ssh_password_override_placeholder` added to `static/i18n/en.json`. The key existed at the path `admin_hosts.groups.ssh_password_override_placeholder` (inside the host-groups editor sub-block) but the per-host SSH editor's markup looked it up at the top-level `admin_hosts.ssh_password_override_placeholder` path, producing a console warning. Added at the top-level scope next to `ssh_section_label`; the existing nested key stays in place for the host-groups editor.
- AI palette ships a baseline `ai_memory` seed teaching the Swarm task-ID-suffix container-naming convention. The `docker_container_du` tool fails with "No such container" when called on a Swarm service's bare service name (`tracearr_tracearr`) because Swarm names running tasks `<stack>_<service>.<replica>.<task_id>` (e.g. `tracearr_tracearr.1.glr5r6sv31fcz8e0p019m1sbm`). New module-level `_AI_MEMORY_SEEDS` tuple in `main.py` carries the canonical text explaining the convention + the mandatory-discovery-first workflow (resolve the full container name via `ssh_diag preset=docker_ps_with_sizes` FIRST, then drill in with `docker_container_du`); `init_db` seeds each entry into `ai_memory` with `source='system'` + `actor='bootstrap'` after migrations apply (idempotent via a duplicate-text guard). The `docker_container_du` tool description in `PALETTE_SYSTEM_PROMPT` now carries a **SWARM CONTAINER NAMING — STRICT** block; the EXAMPLE container-bloat flow teaches a TWO-ROUND chain for Swarm services (round 1 discovers the full name, round 2 drills in) instead of the previous single-round example that would fail on Swarm. Compose single-replica containers can still use the one-round shape.
- AI sidebar autonomous mode now also auto-chains MULTI-ROUND tool dispatches. After the first-round dispatch returns and the AI's second-round reply emits new TOOL directives (common when the first round surfaced a container ID and the AI wants to drill into it), `confirmInlineToolDispatch` re-stamps `turn.pending_tool_confirms` from the new response AND, in autonomous mode, re-fires itself via `$nextTick` so the whole chain runs to completion without any chip surfacing. Approval mode renders the chip again at each round. Chain depth is hard-capped at 5 rounds via `turn.tool_chain_depth` so a buggy model can't infinite-loop the dispatcher.
- AI sidebar autonomous mode now auto-dispatches confirm-required diagnostic tools. The destructive-action chip already honoured autonomous mode (bypasses the inline chip + sets `skipConfirm=true`), but the new tool-dispatch chip (`pending_tool_confirms`) rendered unconditionally — defeating the autonomous contract. `sendAiSidebarMessage` now checks `this.aiSidebarMode === 'autonomous'` after pushing the assistant turn and, when the backend returned a `pending_tool_confirms` envelope, fires `confirmInlineToolDispatch(turnIdx)` programmatically via `$nextTick`. The chip never surfaces; the tool dispatch + second-round AI reply land as if the operator had clicked Yes. Approval mode (the default) keeps the chip — the SSH-touching contract still requires an explicit click in that mode.
- AI sidebar now actually runs confirm-required diagnostic tools. The backend already short-circuited `ssh_diag` / `docker_container_du` dispatches with a `pending_tool_confirms` envelope (per the SSH-discipline contract — operator must approve every host-touching call), but the SPA's `sendAiSidebarMessage` ignored that field entirely. End result: the AI emitted tool directives, the parser recognised them, the dispatcher returned the confirm envelope — and the operator saw no chip, no SSH, no analysis. Three pieces ship together: (1) `AiPaletteIn` gains a `tool_confirm_granted: Optional[bool]` field; `api_ai_palette` stamps `ctx["_tool_confirm_granted"] = True` when set so the dispatcher's short-circuit becomes false on the re-POST and the tools actually fire. (2) New inline-confirm chip under the existing destructive-action chip in the AI sidebar; gated on `turn.pending_tool_confirms`. Carries an alertdialog role + small `<code>` chips showing each tool name + preset / container_name so the operator sees what they're approving. The existing action chip now also checks `!turn.pending_tool_confirms` so the two paths don't render together. (3) New `confirmInlineToolDispatch(turnIdx)` handler re-POSTs to `/api/ai/palette` with `tool_confirm_granted: true` + the original query + prior conversation (sliced at `turnIdx` so the AI doesn't re-see its own pending-state reply); replaces `turn.text` with the second-round reply once it lands, accumulates response_time_ms + tokens across the two-call cycle. Companion `cancelInlineToolDispatch(turnIdx)` clears the pending state. New turn fields `pending_tool_confirms` + `pending_query` (original operator text, so the re-POST has the exact wording, not whatever's in the input box at click time). New i18n keys `ai_sidebar.tool_dispatch_prompt` + `ai_sidebar.tool_dispatch_yes`.
- AI tool-directive parser is now tolerant of orphan `TOOL_ARGS:` lines emitted without the leading `TOOL: <name>` line. The forgiving second pass routes the args through a discriminator map (`preset` → `ssh_diag`, `container_name` → `docker_container_du`, `metric` → `get_host_metrics_recent`, `severity_min` / `tag_prefix` → `get_recent_logs`, `name_prefix` → `get_container_events`, `op_type` / `target_kind` → `get_recent_history`, bare `host_id` → `get_failure_events`) so the AI's diagnostic dispatch still fires when the model forgets the prefix. Same pass also fixed a pre-existing regex bug — the post-name whitespace class was `\s*` (eating the newline the optional `TOOL_ARGS:` anchor needed), so even canonical two-line pairs were being parsed as `TOOL:` + a duplicate orphan; switched to `[ \t]*` (horizontal-only). Added a TOOL DIRECTIVE FORMAT — STRICT block to the system prompt with concrete WRONG / RIGHT examples and a fenced two-line shape so future model output uses the canonical form on the first try.
- AI palette now autonomously diagnoses container-bloat / disk-growth questions via tool-use instead of dispensing shell commands as prose. Pre-fix, asking the AI "tracearr is showing dramatic increase in container size, why?" produced a paragraph explaining what containers store plus the SSH command the user should run themselves. The whole point of having a tool catalogue is so the AI uses the tools and reports actual findings. Three pieces ship together: (1) three new presets in `SSH_DIAG_PRESETS` — `docker_system_df` (`docker system df -v` — total disk used by images / containers / volumes / build cache on the host), `docker_ps_with_sizes` (`docker ps -a -s` — rank every container by writable-layer size, the most-common entry point for "which one is the offender"), `du_root_top` (`du -sh /var/lib/docker /var/log /home /opt /tmp` for fleet-level first-pass triage). (2) New parametric tool `docker_container_du(host_id, container_name, path='/', limit=20)` that runs `docker exec <name> sh -c "du -ah <path> | sort -rh | head -n <limit>"` INSIDE the named container so the path resolves against the container's own filesystem. Container name + path are regex-restricted plus shell-quoted before composition. Added to `PALETTE_TOOLS_REQUIRING_CONFIRM` so it routes through the SPA's inline-confirm chip exactly like `ssh_diag` does. (3) New "AUTONOMOUS DIAGNOSIS — STRICT" paragraph in `PALETTE_SYSTEM_PROMPT` telling the AI it must emit TOOL directives instead of dispensing shell commands; lists the wrong pattern verbatim ("You can SSH in and run `du -ah /config…`") and the right pattern (emit a TOOL, get the result back, compose a finding from the actual output). Carve-outs for pedagogical questions ("how would I check?", "what command would I run?") where descriptive prose is the correct answer. A new EXAMPLE container-bloat flow walks the AI through the canonical two-tool dispatch (`ssh_diag preset=docker_ps_with_sizes` then `docker_container_du`) and the SECOND-ROUND reply shape (concrete paths, sizes, actionable remediation — wire a logrotate sidecar, etc.).

### Changed

- HTTP-probe default-accepted-status-codes range broadened from 200..299 to 200..399 so 3xx redirects pass under the default contract. The httpx client now sets `follow_redirects=True` as well, so a 301 / 302 chain lands on the final response (typically a 200) which then counts under the default range. Operators monitoring Nextcloud / GitLab / Forgejo / WWW-redirect-style targets see them as healthy without needing per-row `accepted_status_codes` overrides. Behaviour for hosts using an explicit-codes override is unchanged. **Follow-up patch:** the LO / HI bounds are now operator-tunable via `tuning_http_probe_default_accepted_lo_code` / `tuning_http_probe_default_accepted_hi_code` (default 200 / 399, range 100..599). Operators on diagnostic-focused deploys can widen toward 100..599 (treat any HTTP response as alive); strict health-check deploys can tighten to 200..299 (2xx-only). Per-host `accepted_status_codes` CSV overrides still win exactly. Resolver swaps LO + HI if accidentally configured in the wrong order so probes don't silently reject every code.
- Telegram `/version` build-time line now honours the sender's date/time format preference and the deployment's configured timezone. Pre-fix the build time was hardcoded as `2026-05-19 21:36 UTC` regardless of who triggered the command — users on non-UTC timezones had to mentally translate every reply. `_cmd_version` in `logic/telegram_listener.py` now resolves the sender's linked OmniGrid username via `_lookup_omnigrid_user(msg.from.id)`, fetches their `ui_prefs.datetime_format` preference via `get_user_datetime_format`, converts the VERSION.txt mtime into the deployment's `scheduler_timezone` (canonical "what day is it for OmniGrid?" knob) via `astimezone(_scheduler_tz())`, then renders through `apply_datetime_format(dt, fmt)`. The reply now reads e.g. `🕓 Built: 19/05/2026, 23:36:00 CET` for a Profile pref of `dd/MM/yyyy, HH:mm:ss` plus `scheduler_timezone=Europe/Berlin`. Falls back cleanly to UTC when `scheduler_timezone` is unset OR the sender isn't mapped to an OmniGrid user (unmapped senders see the deployment-default format). The TZ abbreviation (`CET` / `EEST` / `UTC` / …) is appended as a `<code>` chip so operators across timezones don't have to mentally translate.
- Telegram `/` autocomplete menu now publishes a per-scope command roster — unmapped chats see only the open commands (`/help`, `/start`, `/link`, `/unlink`, `/version`) instead of the full ~20-command roster that would 4xx on auth. Authorised chats configured via the `telegram_chat_id` CSV get the full roster via a per-chat-scope `setMyCommands` override. Without an authorised chat configured (fresh deploy), the default scope still carries the full roster as a fallback — operator linking the first chat sees every command, and the listener's per-loop register re-fires the proper per-scope split on the next iteration. Token-hash gate only re-fires `setMyCommands` when EITHER push fails so transient Telegram errors don't lock the registration out for the rest of the process lifetime.
- Per-host HTTP probe "accepted status codes" field now carries a richer help blurb explaining the default-range fallback contract: blank field defers to the deployment-wide TUNABLES range (200..399 covers the homelab norm), explicit CSV pins a specific code list (e.g. `200` to flag a 302 as suspicious, `200,401` to accept a locked-down API's anonymous unauthorized, `200,301,302,401,403` to widen for diagnostic probes). Placeholder rewrites from `200, 301, 302` to `blank = default · or e.g. 200,301,302` so the empty-is-OK semantic is discoverable at a glance.
- Active-ops floating panel now surfaces a live stack-update convergence summary. While `update_stack` ops are converging (server-side polls Swarm services for up to 5 min by default), the panel's per-op block reads the latest `Convergence poll:` event off the op's log and renders it as a tidy one-liner under the op header: `2 service(s) still updating: nextcloud_web (task xxx failed to start); paperless_app (+1 more)`. Operators no longer stare at a spinner without knowing which service is slow. Falls through to `Waiting for stack convergence…` until the first poll lands so the chip is never blank during the wait. Sourced from the existing `_await_stack_convergence` op.log lines — no new backend plumbing.

### Removed

- Two dead TUNABLES — `tuning_service_probe_host_cache_ttl_seconds` and `tuning_service_probe_host_fail_cache_ttl_seconds`. Declared in the `Tunable` enum + TUNABLES dict and surfaced in Admin → Config, but a bidirectional consumer audit confirmed zero `tuning_int(...)` call sites read either value. Operator editing either knob saw the value persist but the runtime effect was nil. Removed from the enum, the TUNABLES dict, the i18n bundle, the env-example snippet, and the env-example reference table — Admin → Config no longer renders the rows. If a service-probe per-host success / failure cache is wired later, the new tunable should follow the canonical six-step wiring before being declared.

### Fixed

- Per-row HTTP probe Test button reported `1/1 URLs healthy` when the operator had two URLs typed into the textarea but only the first persisted. The test endpoint reads URLs from the body's `http_probe_urls` field; the SPA's `testRow` call was sending `row.http_probe.urls` (the LAST-SAVED array) instead of resolving from the current textarea content. So if the operator typed URL #2 and clicked Test without saving first, the test ran against URL #1 only and reported success on it. Test path now mirrors the save-time serialization exactly — split `urls_text` on newline, trim, drop non-http(s) entries, Set-dedup — so the test reflects what the operator typed. Falls back to the persisted array when `urls_text` was never rendered (load-then-test without textarea interaction) so a save-then-test round-trip stays correct.
- HTTP-probe per-host editor's "clear all URLs" semantic — clearing every line in the Probe URLs textarea and clicking Save now actually drops the persisted URLs. Pre-fix the empty-textarea path gated on `urls_text.length > 0` and fell back to the previously-persisted `urls` array on empty, so Save silently re-wrote the old URLs — the textarea looked empty but the next reload showed them back. Gate is now `typeof urls_text === 'string'` (the textarea has been rendered → bound at least once) so a load-then-clear-then-save round-trip correctly serialises to an empty `urls` array. Defensive hydration block still seeds `urls_text` from the array on first render, so the typeof gate works on every entry path.
- `host_stats_source` POST validator error message omitted `http_probe` — operator typing a typo on the master CSV got a 400 listing six provider tokens (`'beszel' / 'node_exporter' / 'pulse' / 'webmin' / 'ping' / 'snmp'`) when seven were actually valid. The detail string now includes `http_probe` so the seventh provider's CSV token is discoverable from the error message.
- HTTP-probe per-host editor's "Probe URLs (one per line)" textarea swallowed every Enter keystroke. The textarea bound `:value="urls.join('\n')"` + `@input` that called `$event.target.value.split('\n').map(s => s.trim()).filter(Boolean)` and reassigned `row.http_probe.urls` on every keystroke. When the operator typed `https://a.example.com` then hit Enter, the trailing empty element from the split was dropped by `filter(Boolean)`, the resulting `['https://a.example.com']` array got rejoined for the next render WITHOUT the trailing newline, the textarea repainted, and the cursor jumped back to the end of line — Enter looked like a no-op. The `.map(s => s.trim())` also stripped trailing whitespace mid-typing for the same round-trip reason. Fix mirrors the SNMP `exclude_mounts_text` precedent already in the same file: bind via `x-model="row.http_probe.urls_text"` (raw string field, Alpine handles DOM state natively, zero per-keystroke round-trip). `loadHostsConfig` hydrates `urls_text` from the persisted `urls` array on load; `saveHostsConfig` splits + trims + filters + dedupes `urls_text` back to the `urls` array on save. Legacy array shape preserved as the save-side fallback so a load-then-save without textarea interaction round-trips cleanly. Defensive `http_probe: {}` init added to `addHostRow`, the discovery-import row template, and the JSON fleet-import path so the textarea's `x-model` never reads `urls_text` off undefined regardless of how the row entered the editor.
- HTTP-probe provider chip silently missing from the per-host row header in the Hosts view, even when the host had `http_probe.enabled = true` AND a curated `services[].url` configured. Root cause was a SHAPE MISMATCH on the API row: `_shape_host_api_row` stamps `h.services` as the Beszel systemd-services rollup OBJECT (`{total, failed, failed_names}`), but `providerStates(h)` in `static/js/app.js` was reading the third URL-source fallback as `Array.isArray(h.services) && h.services.length > 0` expecting the curated services LIST. `Array.isArray` always returned false on the rollup object, so the chip-gate only fired when `h.url` OR `http_probe.urls` was set — hosts whose probe targets came entirely from `services[].url` saw the chip vanish in the row header despite the sampler probing them correctly. Fix: stamp a single boolean `http_probe_has_targets` on every API row, computed backend-side from the same URL-resolution chain the sampler uses (`http_probe.urls` textarea → `row.url` → `row.services[].url`). The SPA chip-gate now reads `h.http_probe_enabled === true && h.http_probe_has_targets` — one boolean, single source of truth. Added `http_probe_has_targets` to both `CURATED_REFRESH_FIELDS` whitelists (main reconcile + `/api/hosts/list` skeleton overlay) so the in-place reconcile collapses the field cleanly when an operator clears all URL sources.
- main.py lint sweep — ~170 IDE warnings cleared across nine rounds (~68% of the original ~250). **Real bugs**: three dict|None type-narrowing warnings at the tab-activity heartbeat handler — `filters_clean: Optional[dict] = None` made pyright flag every in-loop write under the `isinstance` guard with "Member None of dict | None does not have `__setitem__`"; refactored to populate a `filters_clean_dict: dict = {}` inside the guard and assign the final `Optional[dict]` at the end. Two dead `api_key` + `base_url` locals removed from `api_ai_palette` + `api_ai_sidebar` (creds resolve via `_resolve_ai_fallback_chain` into `provider_creds`; the locals were leftover from a pre-fallback-chain era). Two redundant `timeout=30.0` kwargs dropped at the AI palette `ask_provider_with_fallback` call sites (30.0 is already the signature default). **Shadowed-name renames (~12 sites)**: `_lifespan(app)` → `_lifespan(_app)`; `_seed_caches_bg` inner closure's five `except Exception as e:` → `as boot_err:` with narrowed catch tuples; SSH-terminal WS pumps' `as e` → `as upstream_err` / `as ws_err`; hosts-timeline aggregation `as e` → `as agg_err`; single-letter param renames in `_is_parent_child(a, b)` → `(grp_a, grp_b)`, `_canonical(hid)` → `_canonical(canon_hid)`, `_classify_key`'s `r` / `v` loop vars → `row` / `val`, disk-projection `_rank` helper's `label` → `rank_label`, SSE-subscribe `async for evt` → `async for sub_evt`, host-groups sort `lambda g:` → `lambda grp_row:`, hosts-timeline `lambda e:` → `lambda evt_row:`. **Unused-parameter cleanups (~10 sites)**: seven FastAPI handlers with declared-but-unused `request: Request` parameters silenced via `_request: Request` (`api_ai_memory_add` / `_delete` / `_forget`, `api_admin_reauth`, `api_notify_send`, `api_me_webauthn_register_finish`, `api_admin_disable_totp`); `_provider_sample_intervals(host_id)` silenced via `_ = host_id`; three SPA-shell route handlers (`spa_settings_shell` / `spa_admin_shell` / `spa_stats_shell`) had unused path parameters silenced with `_ = section` / `_ = tab` and gained docstrings explaining the SPA-side handling. **Redundant-import cleanups (~10 sites)**: eight redundant local `from logic import events as _events` re-imports deleted (module-level `_events` already covers every call site); one redundant local `_logs` import deleted; one local `_gather` rebind renamed to use the existing module-level `_gather_mod` alias. **Broad-except narrowing (~10 sites)**: dbstat fallback at line ~5442 + two per-table COUNT(*) loops (`Exception` → `sqlite3.Error`); TUNABLES override-count + stats-sample-interval cadence + canonical-map URL parse → specific tuples per the actual failure modes. **Whitespace + structural**: column-alignment at the host-status fallthrough chain's first branch (~13 lines) normalised to standard single-space syntax; the nested-paren boolean chain on the second branch — which kept tripping the 7-line continuation-indent warning regardless of how the indent was tweaked — extracted into a new `_is_host_unconfigured(active, active_set, h, *, ping_enabled, snmp_mapped, any_provider_enabled) -> bool` helper above `_shape_host_api_row`. The helper splits the two-branch logic (active-set-aware vs legacy whole-row) into two clean returns; the call site is now a single one-liner. **Round-5 docstring batch (~52 sites)**: bulk-added one-line docstrings to FastAPI route handlers + top-level utility functions via a name-keyed regex insertion script — covers `init_db`, the item / op / history / ignore / settings / tuning / OIDC / health / version / logs / auth / user / session / token / backup / schedule CRUD routes, the SPA-shell + sprite + Prometheus-metrics handlers, and the `_seed_caches_bg` lifespan closure. Each docstring is a single descriptive line; existing detailed docstrings on routes that already had one are untouched. **Round-4 broad-except narrowing**: SSH terminal WS pump's keepalive + post-disconnect cleanup blocks (`Exception` → `(RuntimeError, OSError, WebSocketDisconnect)`); task drain in the terminal teardown (`Exception` → `(asyncio.CancelledError, RuntimeError, OSError)`); outer session-error catch (`Exception as e` → `(asyncssh.Error, RuntimeError, OSError, ValueError) as sess_err`); proc + conn close fallbacks (`Exception` → `(RuntimeError, OSError, AttributeError)`). Round-4 also fixed the `row`-shadow regression I introduced in round-3 (`for row in series_local` → `for sample in series_local` — the outer enumeration's `row` lives in the `_shape_host_api_row` caller chain). The remaining ~150 warnings (broad-except across ~60 sites, missing-docstring on ~30 mostly-FastAPI handlers, redundant defaults that document intent, legitimate cross-module access-to-protected-member imports per CLAUDE.md) stay under the LATER lint-phase entry — same shape as the earlier Phase 1-4 lint passes on the rest of the tree.
- Stats range-picker partial threw `Alpine Expression Error: rpLeadingLabel is not defined` (4×, then escalating to an uncaught ReferenceError) on every mount of the Incidents / Network / AI Cost / Samples sub-tabs. Root cause: `static/_partials/_components/og-range-picker.html` guarded three optional variables (`rpLeadingLabel` / `rpExtraClass` / `rpTitle`) via `typeof X !== 'undefined' && X`, which works in plain JS but trips Alpine v3's expression evaluator — Alpine compiles each expression with strict scope semantics where a bare undeclared identifier throws ReferenceError BEFORE `typeof` can guard it. The topbar consumer declared all three optional vars so it worked there; the four stats range pickers only declared the four required vars and crashed on the optional reads. Switched all three optional reads to `$data['<name>']` (bracket-notation property access on Alpine's exposed scope proxy), which returns `undefined` cleanly when the parent doesn't declare the key. Docblock at the top of the partial now documents the `$data` convention so contributors don't reach for `typeof` again.
- Stats charts at the 90d range now rotate x-axis date labels -45° so dense dates stop overlapping. The Samples bucket chart was the visible offender (labels every ~14-week bucket at 90d, `dd/MM/yyyy` labels colliding across a 720px chart); the same treatment is applied to the AI Cost response-time trend and the fleet-wide Network stacked-area chart for symmetry. Charts branch on `range === '90d'` (`_renderSamplesBucketChart`, `_renderAiCostTrendChart`) or `hours >= 2160` (`_renderFleetNetChart`), increase the bottom-axis padding from 24/28 → 60px to fit tilted text, and render each `<text>` with `text-anchor="end"` + `transform="rotate(-45 cx ly)"` so the label's right edge anchors at the bar centre and the text grows up-left from there. Shorter ranges (1h / 24h / 7d / 30d) render horizontally exactly as before — no visual change for the common case. `_renderFleetNetChart` gains a second `hours` parameter; the network partial now threads `statsNetworkHours` through to the call site so the renderer can detect 90d. `_renderDbProjectionChart` and the Incidents heatmap are deliberately untouched (relative `+30d`/`+60d`/`+90d` offsets and a day×hour cell grid respectively — no calendar-date overlap risk).
- Container recreate-success path inspected the wrong container ID on every successful Portainer `/recreate` call — the response body (Docker inspect JSON, typically multi-KB) was truncated to 500 chars BEFORE `json.loads`, so the parser raised `ValueError` and the silently-caught exception left `new_container_id` pointing at the original (now-reaped) container. The follow-up inspect then 404'd on the stale ID, logged the "Post-recreate inspect 404 on new container `<old-12>` — can't verify image-id" warning, and skipped the silent-no-op detection that would have triggered the manual fallback path. Visible to operators as "Recreate ran, success toast lands, but the container's local digest never moves and the row stays flagged needs-update" (reported on `netbootxyz`). Fix in `logic/ops.py:do_update_container`: capture the FULL `r.text` into `recreate_response_full` BEFORE the 500-char log truncation, parse `Id` from the full body, then truncate `recreate_response_body` only for the operator-facing log lines. A new warning log line fires if the JSON parse itself fails (was a silent `pass` before) so the next failure mode is visible without a code dive. **Follow-up patch:** once the no-op detection started firing correctly, the FALLBACK path then 404'd because it inspected the OLD `container_id` (which Portainer had already reaped when it spawned the new container during its silent-no-op recreate). Hoisted `new_container_id` to the outer scope of `do_update_container` so the silent-no-op detection AND the fallback dispatch see the same value; when Portainer no-op'd and `new_container_id != container_id`, the fallback now targets the live new container instead of the dead old one. The fallback's "falling back to manual…" log line now includes `(against new container <new-12>)` whenever the target was rerouted, so operators can see in the History entry which container the manual pull + stop + remove + create + start chain actually operated on.
- UI busy-state never extended past 3 s — every per-item Update / Recreate / Restart button re-enabled itself ~3 seconds after click regardless of whether the backend operation was still running. `_markBusy(key)` in `static/js/app.js` armed an unconditional 3-second auto-clear `setTimeout` as a safety floor; the completion path only fired for operations detected as `justDone`, so currently-running operations never cancelled the floor timer. Any update taking longer than 3 s (the common case for stack pulls + recreates) re-enabled the button while the backend was still mid-pull, and a fast double-click could spawn a second operation against the same target. Fix: `pollOps` now calls `_holdBusy(this._opBusyKey(o))` for every operation it observes in the `running` state, cancelling the floor timer the moment we see the op on the wire. The button stays disabled for the actual operation duration (stack pull on a slow registry can take 60+ s) and only re-enables when the existing `_clearBusyFromOp` path fires on the running → terminal transition. The 3 s floor stays in place for the genuine "POST silently dropped, operation never appeared on `/api/ops`" case. **Follow-up patch:** the round-1 fix held the busy state correctly through the operation's lifetime, but the operation's lifetime didn't match the actual update duration. Portainer's `PUT /api/stacks/{id}` returns 200 in ~5 s while the actual `Prune + PullImage` runs asynchronously on the docker daemon for 30-60 s+; pre-fix `do_update_stack` called `op.done("success")` as soon as the PUT returned, the SPA's SSE `op:completed` handler fired `_clearBusy`, and the button reverted to "Update" while the daemon was still rolling. Operators had to refresh manually to see the actual degraded state mid-rollout. New `_await_stack_convergence` helper in `logic/ops.py` polls Portainer's service list after the PUT returns: for every service whose `com.docker.stack.namespace` label matches the stack's name, it checks `UpdateStatus.State`. While any service shows `"updating"`, the helper keeps waiting. Two consecutive clean polls debounce against the brief gap between services rolling out one at a time. The op stays in `"running"` state for the entire actual update duration, so the SPA's existing busy-state machinery + SSE-driven completion path stay correct without further frontend changes. Two new TUNABLES gate the wait: `tuning_stack_update_observe_timeout_seconds` (default 300 s, range 30..1800 — covers a typical N-service rolling update on a home-lab daemon; raise for large stacks / slow registries) and `tuning_stack_update_observe_poll_seconds` (default 15 s, range 5..120 — drop for snappier feedback or raise for very large fleets). Both surfaced via Admin → Config + the env-example reference table.
- Container update via Portainer's `/recreate` endpoint silently fails for containers Portainer didn't itself deploy — Komodo-managed containers, raw `docker run` containers, external compose stacks. The endpoint returns 4xx for these and the operation landed with `error` status across every caller (SPA single-item Update, SPA bulk update, AI palette `update_all_updatable`, Telegram `/update all`, Telegram `/update <name>`); the Telegram dispatch in particular reported "✅ Spawned N Operations" up-front but the operator only discovered failures by opening Admin → History. `do_update_container` now wraps the Portainer call in a try/except: on any HTTP 4xx/5xx or network error from the recreate endpoint, it logs the original Portainer error as a `warning` operation log line and falls back to a new `_recreate_container_in_place(op, container_id)` primitive that does the inspect → pull → stop → remove → create → connect-extra-networks → start dance manually (same recreate flow the operator-confirmed "Switch image tag" path already uses, just without the retag step — the container's current image ref is kept, only the `@sha256:…` digest is stripped so the pull resolves a fresh manifest under the same tag). Volumes / networks / env / restart policy survive via the captured Config + HostConfig + NetworkSettings.Networks copy; anonymous volumes are lost on the stop+remove step — same risk as Portainer's own `/recreate` endpoint, no new risk. The operator-confirmed retag path was deliberately left untouched (conservative approach — code duplication accepted so the working manual recreate can't regress). Every fallback log line carries a `[fallback]` prefix so the operator can immediately distinguish which path completed the recreate when reading the History entry.
- Host status taxonomy false-positive on the `/api/hosts/list` cold-load path — a healthy curated host whose persisted snapshot didn't carry one of four specific runtime fields (`host_cpu_percent` / `host_mem_total` / `host_disk_total` / `host_uptime_s`) fell through every earlier precedence branch and landed on `else: host_status = "unknown"` (red — flagged in `isProblemHost` and the Stats Problem-hosts tile) even when the snapshot contained other host_* fields proving the host was reachable. The `_stale_fields` gate in `_shape_host_api_row` (`main.py`) is now `if s.get("_stale_fields"): host_status = "up"` — any restored snapshot field is sufficient evidence of past liveness, the four-field whitelist is gone. The snapshot fallback's own `_is_meaningful` filter already prevents zero / empty values from polluting `_stale_fields`, so the gate is still defence-in-depth, just not over-narrow. Subsequent `/api/hosts/one/{id}` probes overwrite this provisional "up" with the live status as usual. Reported on a healthy monitoring host (Pulse online + NE + Beszel all responding, every `provider_pause_state` row `paused: false` with recent `last_ok_ts`).
- Host status taxonomy false-positive on curated rows with stale per-row provider fields. `_shape_host_api_row`'s `snmp_mapped` predicate counted any non-empty `snmp` sub-dict as "mapped", so a row that had previously typed a community / version / port override and was then toggled OFF (user unchecked the SNMP enable box, every provider field empty, only the shared `address` field set) still classified as `status: "unknown"` (red — "real outage") instead of `unconfigured` (grey — "nothing to probe"). Mismatched the rest of the codebase: `_merge_one_host` and the debug-panel `host_active` computation both gate on `snmp.enabled === True` AND a resolvable target (`snmp_name` OR shared `address`). The shape helper now matches: a row reads as SNMP-mapped iff (a) `snmp.enabled === True` AND (b) `snmp_name` OR `address` is non-empty. Address-only inventory rows (e.g. an appliance carrying `appliance.example.com` in `address` with no provider opt-ins) now correctly render `unconfigured` — excluded from the Problem-hosts filter chip, excluded from the Stats dashboard Problem tile, no "no data" banner in the drawer. **Follow-up patch — extended the gate from SNMP-only to ALL six providers (Beszel / Pulse / Webmin / NE / Ping / SNMP):** user-reported a second host showing the same false `unknown` status with all SNMP fields clean — root cause was that the OTHER four provider fields (`beszel_name` / `pulse_name` / `webmin_name` / `ne_url`) had identical drift class: a stale per-row value counted as "provider mapped" regardless of whether that provider was globally enabled. Threaded `active: Optional[Iterable[str]] = None` through `_shape_host_api_row`; when supplied, each per-row field must align with a globally-enabled provider for the field to count as "mapped" (`"beszel" in active and beszel_name`, `"pulse" in active and pulse_name`, etc.). Updated all four call sites (`api_hosts_list`, `api_hosts_one`, legacy `api_hosts`, `api_hosts_debug`) to pass `active=state["active"]` (or `active=active_host_stats_providers()` for the legacy path). Legacy `any_provider_enabled: bool` retained as back-compat fallback when callers don't pass `active`. When the provider IS globally enabled but the alias doesn't resolve, the row still correctly renders `unknown` — that's a real "user asked for monitoring, it didn't work" signal.
- `main.py` lint Phase 1 — real bugs only: duplicate dict key `network_ifaces` removed (one of two identical writes silently overwrote the other); two trailing semicolons (E703) at `cleaned.append(p);` + `sx = sum(xs);` replaced with newlines. Stylistic + PyCharm flow-analysis false-positives (`# noinspection` directives on walrus-in-conditional + raise-terminal + FastAPI route + shadow-builtin parameter names like `range` and `id` that are URL-contract names). Phase 2 lands the high-leverage PEP8 reformats (`if X: Y` one-liners split, compact `if`/`add` cluster expanded). Phase 3+ (~150 broad-except + missing-docstring + argument-equals-default + shadow-name items) is deferred to a follow-up batch — Phase 1+2 covers the actual bugs + highest-leverage stylistic items.
- Telegram `/update` preview no longer lists orphan-type containers as "updates available". Pre-fix the filter `i.status == "update"` matched Swarm task containers left over from the PREVIOUS image (status stays "update" until the operator runs `/cleanup` to reap them) — operators saw entries like `omnigrid_omnigrid.1.<task-id> (orphan)` in the `/update` reply and were confused whether `/update all` would re-update them (it would have routed to a stack-update, which would have been wasteful but not harmful). Fix in `logic/telegram_listener.py:_cmd_update` adds an `i.type != "orphan"` clause to every filter site (main preview, partial-name-match fallback, `/update all` fan-out, AI context builder `_build_telegram_ai_context`). Same drift fixed in `static/js/app.js`'s blast-radius modal — the "update available" chip no longer renders on orphan items.

### Added

- Tab-activity popover device hint — each sibling-tab row now renders a small device chip ("📱 iPhone · Safari" / "🖥️ Mac · Firefox" / "💻 Windows · Chrome") between the view title and the rich-state line so operators with tabs open across machines can tell which device the other tab is on without scrolling. Backend `_parse_tab_activity_device(ua)` (`main.py`) parses the heartbeat request's `User-Agent` header into `{form_factor, platform, browser, ua}` — closed-set tagged strings so the SPA's popover can render via i18n keys without further string juggling. Form-factor heuristic distinguishes mobile / tablet / desktop; platform covers iOS / Android / Windows / Mac / Linux / BSD / ChromeOS / Other; browser covers Chrome / Firefox / Safari / Edge / Opera / Other with the canonical order-of-checks (Edge / Opera before Chrome because both carry `Chrome/` in their UA; Safari last because every WebKit browser carries `Safari/`). Hover-title surfaces the raw UA string (capped at 200 chars) for two-laptops-of-the-same-OS disambiguation. New i18n keys under `topbar.tabs.device.platform.*` + `topbar.tabs.device.browser.*` so non-en locales get localised labels. New SPA helpers `_tabActivityDeviceEmoji(device)` + `_tabActivityDeviceLabel(device)`. New CSS rule `.tabs-pill-row-device` (inline-flex, gap consumed from `--s-1`). Backwards-compat: heartbeat entries minted by pre-fix tabs lack the `device` block — the chip's `x-show` gate (`tab.device && tab.device.platform`) hides cleanly for those rows until the next heartbeat fires.
- AI palette — new `send_notification` action lets the operator route a custom (operator-typed) message to ONE specific notification channel. Distinct from the per-medium Test buttons (which fire a fixed test payload). Operator phrasings: "send to telegram Backup finished", "tell apprise restart complete", "notify the team via telegram that opnsense is fine". The AI emits `ACTION: send_notification` + `ACTION_DATA: {"medium":"telegram"|"apprise"|"app","body":"<text>","title":"<optional>"}`; the SPA dispatches via the inline-confirm chip (destructive — the message goes to real subscribers) to a new `POST /api/notify/send` endpoint that constrains `notify()`'s fan-out to ONE medium via a new `logic.ops.notify_one_medium(...)` helper. Admin-only + audited under new `op_type='notify_send'` in `OP_TYPES`. Per-medium master switch is still honoured server-side (a disabled medium short-circuits with a clear `{ok:false, detail:"medium 'X' is disabled"}`). Body length capped at 4096 chars to match Telegram's per-message wire limit. New i18n keys under `command_palette.action.send_notification(_sub)` + `toasts_extra.send_notification_*`.
- Notifications admin tab — per-channel Test buttons. Apprise and Telegram now each have their own inline "Send test" button inside their respective tab (alongside the existing channel form fields), backed by a new `/api/apprise/test` endpoint that fires ONLY Apprise (`/api/telegram/test` already existed). Replaces the pre-fix shared page-level button that fan-out to BOTH enabled channels via `/api/notify-test` (kept registered for back-compat / scripted callers). Each channel renders its own pass/fail result chip inline below the button so operators see per-channel outcomes side-by-side instead of an aggregated message. The page-level Save row keeps only the Save button + the Save result chip — pre-fix the row carried a redundant combined Test button AND a merged Test/Save chip that became misleading once per-channel chips existed.
- Telegram bot — `telegram_chat_id` setting now accepts a comma-separated list of chat IDs. Inbound commands are accepted from any listed chat (so one bot can serve a group AND 1:1 DMs simultaneously); replies route back to the chat the command came from via a `ContextVar` set by the dispatcher BEFORE handler invocation, so the ~50 reply call sites needed no signature change. Outbound notifications fan out to every listed chat sequentially, with aggregate `ok=True` when at least one chat accepts the message (partial-success detail strings name the failed chats so operators can fix them in Admin → Notifications). Backward compat: a single-value setting (no comma) parses as a one-element list and behaves identically to today. Closes the operator-reported "bot not responding to 1:1 chats" gap that surfaced when the bot was originally configured against a group chat ID.
...
- `host_failure_events` (incidents) — switched from "keep every row forever" to a tunable 90-day default retention. New `tuning_incidents_retention_days` knob (default 90; range 0..3650; 0 = disable pruning entirely, restoring the legacy keep-forever behaviour) gates a `DELETE FROM host_failure_events WHERE ts < ?` pass in the existing `host_metrics_sampler` hourly prune loop. Decoupled from `tuning_stats_history_days` (which still drives the time-series sample tables) so incidents can carry a longer audit window than raw samples while still bounding table growth on noisy fleets. Surfaced in Admin → Config with the standard label / help i18n entries.
...
- Telegram `/update` on a Swarm service-replica or orphan task container failed with `HTTP 500: failed to set up container networking: network <id> not manually attachable`. Root cause: the Telegram update handler routed both `kind=="container"` AND `kind=="orphan"` through `do_update_container`, which uses Portainer's `/containers/{id}/recreate?PullImage=true` endpoint — that endpoint can't attach to Swarm overlay networks unless flagged `attachable=true`. Fix: container kinds split into two branches in `logic/telegram_listener.py:_cmd_update` — plain `container` keeps the direct-recreate path for standalone non-Swarm containers; `service` AND `orphan` items now route to `do_update_stack` on their parent `stack_id` (deduped via a `spawned_stacks: set[int]` so `/update all` on a stack with multiple replicas + orphans fires ONE stack-update Operation, not several racing against the same compose file). Items without a parent `stack_id` (rare — bare standalone with `com.docker.swarm.service.id` but no `com.docker.stack.namespace`) skip gracefully with a `(N skipped)` annotation in the reply.
- Telegram `/host <target>` placeholder ("🔄 Probing live providers…") sometimes stuck forever when the listener's first `_cmd_host` invocation raced the main module's lifespan startup — the lazy `from main import _merge_one_host, _get_host_provider_state` raised ImportError, the caught exception just logged to stderr, and the placeholder message never got edited with an actionable reply. Now caught separately and surfaced as "⚠️ Backend still warming up. Try /host <target> again in a moment." so the operator knows to retry instead of staring at a frozen spinner.
- Telegram Markdown→HTML converter mis-parsed two edge cases: (a) `**bold then *italic*** here` (asymmetric italic-inside-bold with no closing space) used to leak a stray `<i>*</i>` because the italic regex's lookahead missed the third asterisk; (b) `a*b*c` arithmetic shorthand (no spaces around `*`) rendered as `a<i>b</i>c` because the lookbehind only checked one prior character. Both fixed by widening the italic-star regex's lookbehind+lookahead to `(?<![*A-Za-z0-9_])` / `(?![*A-Za-z0-9_])` so `*italic*` jammed against alphanum no longer matches at all.
- Telegram `/version`, `/ver`, and `/ip` commands' docstrings explicitly said "works pre-link" but the dispatcher's `open_commands` set didn't include them — an unmapped user typing `/version` got the 🔒 "Link your account first" reply contradicting documented behaviour. Added all three to `open_commands` so the dispatcher gate matches the documented intent.
- `_resolve_target` priority-1 (exact per-provider target match) and priority-4 (asset-inventory match via `custom_number`) silently returned only the FIRST match in iteration order — same anti-pattern the label-path (priority-3) already disambiguates correctly via a single-hit / multi-hit chooser contract. Two hosts sharing an IP (typo OR overlapping `snmp_name` / `beszel_name` aliases) OR a duplicate `custom_number` (legitimate when the inventory key isn't enforced unique) silently picked the first row and the operator never knew. Both paths now gather ALL matches with id-dedup, then apply the same disambiguation contract.
- `_resolve_target` priority-4 raised `AttributeError` when an asset row had `"Type": null` or `"Type": "string-value"` instead of the expected dict — `asset.get("Type", {}).get("ShortName")` assumed Type-when-present was always a dict. Now narrows via an intermediate `isinstance(_type_raw, dict)` check.
- `port_scan_refresh` scheduler ran `SELECT host_id FROM host_failure_state WHERE paused_at IS NOT NULL AND resolved_at IS NULL AND host_id NOT LIKE '%:%'` on every tick — the LIKE-leading-wildcard schema smell on a table that already has a dedicated `provider` column (composite PK migration). Rewrote to `AND provider = ''` (empty-string sentinel for whole-host pauses) so the `idx_host_failure_state_provider` index gets used.
- Telegram `/update all` (and `/update <name>`) reported "Nothing to update — every stack and container is on its latest remote digest" while stacks were actually pending updates. Root cause was a wrong-field reference: the Telegram `_cmd_update` filter read `i.get("update_available")` but `gather.py:enrich()` sets the canonical "needs update" signal as `status == "update"` (from the remote-digest comparison) — there is no `update_available` field on items in the canonical pipeline. Every call site in `logic/telegram_listener.py` that filtered for "needs update" now reads `(i.get("status") or "") == "update"` — the main preview filter in `_cmd_update`, the partial-name-match fallback, AND `_build_telegram_ai_context` (which still emits a synthetic `update_available` field for AI prompt-copy compatibility but derives it from `status == "update"`). Same drift fixed on the SPA side: `_buildAiPaletteContext` in `static/js/app.js` now derives `update_available` from `status === 'update'` for the AI context payload, and the blast-radius modal's update-chip render also tests `(it.status || '') === 'update'` instead of the absent field. After the fix, `/update` lists every item whose remote digest is newer, `/update all` spawns Operations for each, and the AI's "any stacks need updating" answer cites the correct count from `items_summary.updatable_total`.
- `_resolve_target` in `logic/telegram_listener.py` priority-1 (exact IP / per-provider target match) returned the FIRST match in iteration order silently — same anti-pattern the label-path (priority-3) already disambiguates via the "single hit returns the row, multiple hits return None + the full list" chooser contract. Two hosts sharing an IP (typo OR overlapping `snmp_name` / `beszel_name` aliases) silently picked the first row. Fix: gather ALL priority-1 matches with id-dedup first, then apply the same single-hit / multi-hit disambiguation contract as priority-3.
- Telegram `/version` + `/ver` + `/ip` commands' docstrings explicitly say "works pre-link" / "non-sensitive command — works pre-link" but the three were absent from the dispatcher's `open_commands` set, so an unmapped user typing `/version` got the 🔒 "Link your account first" reply contradicting documented behaviour. Extended `open_commands` to include `/version`, `/ver`, `/ip` (the latter is independently gated by the `tuning_public_ip_enabled` tunable, so opening the dispatcher gate doesn't bypass the privacy guard).
- TOTP-setup-confirm route (`api_local_login_totp_setup_confirm`) and one-shot bootstrap-admin route (`api_local_bootstrap`) both fired login notifications via `notify()` but neither wrote a `history` audit row — post-mortem "who logged in at 2am" required cross-referencing Apprise output (ephemeral + configurable) rather than the durable history table. Both routes now call `write_admin_audit(c, "user_login", ...)` inside the same `with db_conn() as c:` block as the session creation. Audit row carries `method=local_totp_setup` / `bootstrap`, `auth_source=local`, and the resolved IP.
