...
- Worker-node service stats regression — CPU / memory / disk no longer show as `—` for services whose tasks live on a Swarm worker. Modern Portainer agent-mode endpoints return only the manager's containers from the unrouted `/containers/json` listing, so worker-node containers never entered `svc_by_cid` / `size_root_by_cid` / `running_cids` and the per-item walk found nothing — `has_stats` / `has_size` stayed False even though the existing `_one_container_stats` agent-target dance would have worked if the CID had ever made it into the pipeline. `gather_stats` now mirrors `logic/gather.py`'s own per-node sweep: when 2+ Swarm hostnames are known it fans out per host with `X-PortainerAgent-Target`, dedups by container Id, and derives cid → host directly from which sweep returned the entry. Single-node deploys keep the unrouted fast-path. Cost: one extra HTTP call per Swarm node per stats refresh, all in parallel.
