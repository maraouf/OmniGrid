...
- **The task ledger in `notes/note_todo.txt` is retired.** It had grown until every entry was a paragraph nobody read, costing more to maintain than it returned. The file stays as history and its ids remain valid to quote, but nothing is added to it and no entry moves. This CHANGELOG is now the durable record of what shipped; anything code-complete but unverified is said when the work is handed over rather than filed. The working agreement, `CLAUDE.md`, the agent definitions and agent memory were all updated together, and the rules that existed only to govern the ledger's formatting were deleted rather than left to contradict the new state. A replacement way of tracking releases is planned.
...
- **One file now holds the working agreement.** `notes/standing-reminders.md` collects how work gets done here — each rule with what it cost — and the 37 scattered copies that could drift from it were reduced to a single pointer. Type-checking was also tightened where it can be trusted, which immediately surfaced two latent crashes, one of them already deployed.
