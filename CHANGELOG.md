...
- **A read-only API token can now read the logs and the AI call log.** Those were admin-only, so a read-only token -- which is the right shape for an automated or assistive client, since it cannot change anything -- could see every host's telemetry but not the logs explaining why a host was failing. That is backwards: the diagnostic is the part a read-only client is for. Everything that WRITES, including clearing the log buffer, stays admin-only.
