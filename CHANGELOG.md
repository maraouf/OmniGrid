...
- Per-row UID generation in Admin → Hosts now uses `crypto.randomUUID()` (cryptographically strong) via a new `_mintRowUid()` helper. The previous `Math.random()` composite was UI-only and not a security secret, but the new source closes the CodeQL `js/insecure-randomness` flag and removes any collision risk in fleets with thousands of rows. Fallback path covers ancient browsers + carries an inline suppression annotation.
