...
- Host status taxonomy false-positive on curated rows with stale per-host SNMP overrides. `_shape_host_api_row`'s `snmp_mapped` predicate (`main.py:10708`) counted any non-empty `snmp` sub-dict as "mapped", so a row that had previously typed a community / version / port override and was then toggled OFF (operator unchecked the SNMP enable box, every provider field empty, only the shared `address` field set) still classified as `status: "unknown"` (red — "real outage") instead of `unconfigured` (grey — "nothing to probe"). Mismatched the rest of the codebase: `_merge_one_host` (line 12214) and the debug-panel `host_active` (line 13845) both gate on `snmp.enabled === True` AND a resolvable target (`snmp_name` OR shared `address`). The shape helper now matches: a row reads as SNMP-mapped iff (a) `snmp.enabled === True` AND (b) `snmp_name` OR `address` is non-empty. Address-only inventory rows (e.g. a router carrying `hdhomerun.home.lan` in `address` with no provider opt-ins) now correctly render `unconfigured` — excluded from the Problem-hosts filter chip, excluded from the Stats dashboard Problem tile, no "no data" banner in the drawer.
