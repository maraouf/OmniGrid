...
| [`guidelines/telegram.md`](guidelines/telegram.md)           | Telegram integration — outbound notifications, inbound slash commands (`/help`, `/hosts`, `/host`, `/restart` (alias `/reboot`), `/cleanup`, `/update`, `/link`, `/whoami`, `/weather`, `/moon`, `/time`, `/version`, `/ip`), AI free-form chat surface, account linking, tunables, troubleshooting.                                                           |
