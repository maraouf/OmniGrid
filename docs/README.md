...
| [`guidelines/scheduler.md`](guidelines/scheduler.md) | Scheduler kinds (`gather_refresh` / `prune_node` / `prune_all_nodes` / `backup` / `config_backup` / `asset_inventory_refresh` / `prune_logs` / `prune_notifications` / `swarm_agent_health` / `port_scan_refresh`), endpoints, safety properties. |
...
| [`guidelines/telegram.md`](guidelines/telegram.md) | Telegram integration — outbound notifications, inbound slash commands (`/help`, `/hosts`, `/host`, `/restart`, `/cleanup`, `/link`, `/whoami`, `/weather`, `/time`, `/version`, `/ip`), AI free-form chat surface, account linking, tunables, troubleshooting. |
