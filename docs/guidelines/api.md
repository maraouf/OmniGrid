...
{
  "ok": true,
  "detail": "OK — Portainer 2.27.4, endpoint primary reachable"
}
```
...
{
  "ok": false,
  "detail": "endpoint 99 not found on this Portainer",
  "status": 404
}
```
...
| `cache:invalidated`          | Items cache has been marked stale (post-op refresh, settings save). SPA reacts with a debounced **forced** `/api/items?force=true` (kicks a fresh gather). | `{reason}`                                                                                                                                                                                                                                                                                       | |
| `cache:refreshed`            | A background `_gather()` FINISHED rebuilding the items cache. SPA reacts with a debounced **non-force** `/api/items` refetch so the view catches up the moment fresh data lands (instead of a poll cycle later) without triggering another gather. | `{}` | |
...
| `DELETE` | `/api/apps/views/{id}`                                   | **Owner only.** Delete a dashboard. 403 for non-owners; 404 hides others' private views. Writes an `app_view_delete` history row.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `GET`    | `/api/apps/arr-calendar?start=&end=`                     | Merged upcoming-release calendar across every configured Radarr / Sonarr / Lidarr / Readarr instance (a pinned chip with an api_key set) for the `[start, end]` (`YYYY-MM-DD` inclusive) window. Returns normalised items + which services contributed; each item carries `host_id` + `service_idx` so the SPA routes its poster through the per-app image proxy. Powers the `arr_calendar` dashboard widget.                                                                                                                              |
| `POST`   | `/api/apps/tile-trace`                                   | Diagnostic sink — the Apps view POSTs one row per tile when its body finishes mounting (or throws), carrying `{group_id, slug, elapsed_ms, phase?, error?}`. Mirrored to container stdout as a single `[tile-trace]` line for render-perf correlation. No persistence.                                                                                                                                                                                                                                                                  |
| `POST`   | `/api/admin/spa-diagnostic`                              | Generic counterpart to `tile-trace` — accepts a `{kind, ...}` body of SPA-side performance probes and mirrors them to container stdout as a single `[spa-diagnostic]` line. No persistence.                                                                                                                                                                                                                                                                                                                                            |
...
Current per-app roster (34 modules; the canonical list is
...
| Grafana                        | `grafana`                                       | Bearer service-account token  | status / dashboards / datasources / search                                                          |
| Rundeck                        | `rundeck`                                        | `X-Rundeck-Auth-Token` header | status / jobs / running / executions / run-job                                                      |
| GitSync                        | `gitsync`                                        | Bearer token (`gsc_…`)        | status / pairs / sync-all / pause-all / unpause-all / sync / pause / unpause                          |
| Nginx Proxy Manager            | `nginx-proxy-manager` / `npm`                   | Bearer token (per-fetch login)| status / proxy-hosts / expiring-certs / disable-host / enable-host / renew-cert / renew-expiring     |
| OPNsense                       | `opnsense`                                       | HTTP Basic (key:secret)       | status / gateways / services / restart-service                                                      |
| Proxmox VE                     | `proxmox` / `proxmox-ve` / `pve`                | `PVEAPIToken` header          | status / vms / containers / start-guest / stop-guest                                                |
| UniFi                          | `unifi-os-server` / `unifi` / `unifi-network` / `unifi-os` | `X-API-KEY` header  | status / devices / clients / wlans / restart-device / block-client / unblock-client                 |
| Fing                           | `fing` / `fing-agent`                           | api_key (`auth` query param)  | status / scan / devices / device                                                                    |
| FlareSolverr                   | `flaresolverr`                                  | none                          | status / sessions / destroy-session                                                                 |
| RustDesk Server                | `rustdesk` / `rustdesk-server` / `rustdesk-server-pro` | Bearer token (user + pass login) | status / devices / users                                                                     |
| netboot.xyz                    | `netboot-xyz` / `netbootxyz`                    | none (status / version only)  | status                                                                                              |
...
| `GET`  | `/api/widgets/favicon?url=<encoded>`           | Server-side favicon proxy + disk cache for bookmark / app tiles whose icon doesn't resolve to a brand / catalog icon (the resolver's final fallback before the letter-avatar). Fetches `/favicon.ico` + the page's `<link rel=icon>`, caches the bytes to `/app/data/favicons/`, and serves them from OmniGrid's own origin. SSRF-guarded (a private / internal target is refused unless its host is in `hosts_config`). |
