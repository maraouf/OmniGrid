...
Settings → Authentik OIDC. There are no `OIDC_*` env vars. Restart-free updates — the settings
...
- OmniGrid public URL — e.g. `https://omnigrid.www.home.lan/`.
- Authentik public URL — e.g. `https://authentik.www.home.lan/`.
...
| Redirect URIs / Origins (RegEx)    | `https://omnigrid.www.home.lan/api/oidc/callback` ← paste the **exact** URL OmniGrid shows in Settings → Authentik OIDC → Redirect URI (there's a Copy button right there). One per line; no trailing slash; case-sensitive.              |
...
| Launch URL        | `https://omnigrid.www.home.lan/`                                                                                                         |
...
Top-nav → **Settings** → **Authentik OIDC**.
...
| Issuer URL                 | `https://authentik.www.home.lan/application/o/omnigrid/` — from the Authentik provider's Metadata tab — copy the "OpenID Configuration Issuer" value. Trailing slash is fine either way. OmniGrid appends `/.well-known/openid-configuration` internally.                              |
...
1. Visit `https://omnigrid.www.home.lan/`.
...
```bash
curl -sS -b /tmp/c https://omnigrid.www.home.lan/api/me | jq
# {
#   "authenticated": true,
...
2. Visit `https://omnigrid.www.home.lan/`.
...
1. OmniGrid → Settings → Authentik OIDC → uncheck "Enable OIDC login" → Save.
