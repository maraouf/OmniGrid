...
| Redirect URIs / Origins (RegEx)    | `https://omnigrid.example.com/api/oidc/callback` ← paste the **exact** URL OmniGrid shows in Settings → Authentik OIDC → Redirect URI (there's a Copy button right there). One per line; no trailing slash; case-sensitive.              |
