...
# New-version watcher poll cadence in seconds. The SPA polls /api/version
# on this interval so a deploy that lands while the operator has the tab
# open surfaces the "New version — reload" banner — this is the detection
# lag. Backend multiplies × 1000 before delivering to the SPA's setInterval
# via /api/me's `client_config.version_poll_ms`. Range 10..3600.
VERSION_POLL_INTERVAL_SECONDS = 60
...
# Image-proxy disk cache (logic/image_cache.py). The TMDB /api/image-proxy
# and the per-app image proxy both fetch images server-side; this caches the
# bytes to <data_dir>/image_cache/ so the same picture (a poster shared across
# providers) downloads once. TTL = seconds an image is served before re-fetch;
# 0 disables the cache. Default 7 days. Range 0..2592000 (30 days).
IMAGE_PROXY_CACHE_TTL_SECONDS = 604800
# Max cached images before the oldest are pruned (cap on cache-dir growth).
# 0 = no cap (TTL only). Default 5000. Range 0..100000.
IMAGE_PROXY_CACHE_MAX_ENTRIES = 5000
...
# outbound calls to api.aladhan.com. The master enable toggle + method +
# Asr school + fallback location are all DB-backed (Admin → Prayer Times),
# NOT env (enable is the plain `prayer_times_enabled` setting, like
# `weather_enabled`). Only the cache TTL + fetch timeout are env/tunables.
# Cache TTL default 3600 s (daily times are static; refetch hourly picks
...
PROXMOX_SAMPLE_INTERVAL_SECONDS = 900
PROXMOX_HISTORY_DAYS = 30
# Direct-Docker node (Portainer-less, managed over SSH — Admin → Docker Nodes):
# per-call wall-clock budget (connect + tunnel to docker.sock + one API request).
DOCKER_DIRECT_TIMEOUT_SECONDS = 20
# How often the background probe checks Portainer reachability. On an outage the
# fleet keeps showing the last-good data + a banner, and Portainer actions pause.
PORTAINER_HEALTH_PROBE_INTERVAL_SECONDS = 30
...
# trend + "peak N concurrent streams today" (Tautulli keeps history but it's
# round-tripped per render). Same cadence / retention shape as the Plex sampler.
TAUTULLI_SAMPLE_INTERVAL_SECONDS = 300
TAUTULLI_HISTORY_DAYS = 30
# Tracearr fleet sampler — records each Tracearr chip's active-stream count +
# rolling 7-day violation count for the violation-rate trend + concurrency
# sparkline (Tracearr's API only reports point-in-time counts). Interval default
# 900 (15 min — violation/concurrency counts move slowly); 0 = inherit global.
TRACEARR_SAMPLE_INTERVAL_SECONDS = 900
TRACEARR_HISTORY_DAYS = 30
...
ADGUARDSYNC_HISTORY_DAYS = 90

# Per-app usage samplers — each records a current-state metric from its app's
# API (which exposes no long history) so the expanded card can render a trend
# sparkline. *_SAMPLE_INTERVAL_SECONDS = sample cadence (0 = inherit the global
# stats interval); *_HISTORY_DAYS = retention window for the samples.
#
#   AdGuard Home / Pi-hole — blocked-query % trend (fleet apps).
ADGUARD_SAMPLE_INTERVAL_SECONDS = 900
ADGUARD_HISTORY_DAYS = 90
PIHOLE_SAMPLE_INTERVAL_SECONDS = 900
PIHOLE_HISTORY_DAYS = 90
#   *arr family (Radarr / Sonarr / Lidarr / Readarr) — library / queue trend.
SERVARR_SAMPLE_INTERVAL_SECONDS = 900
SERVARR_HISTORY_DAYS = 365
#   Prowlarr — indexer health trend.
PROWLARR_SAMPLE_INTERVAL_SECONDS = 900
PROWLARR_HISTORY_DAYS = 90
#   Seerr (Overseerr / Jellyseerr) — request-backlog trend.
SEERR_SAMPLE_INTERVAL_SECONDS = 900
SEERR_HISTORY_DAYS = 90
#   qBittorrent — active-torrent / transfer-rate trend (finer cadence for bursty peaks).
QBITTORRENT_SAMPLE_INTERVAL_SECONDS = 300
QBITTORRENT_HISTORY_DAYS = 30
#   Kavita — library item-count trend.
KAVITA_SAMPLE_INTERVAL_SECONDS = 900
KAVITA_HISTORY_DAYS = 365
#   Tdarr — transcode-queue / health trend.
TDARR_SAMPLE_INTERVAL_SECONDS = 900
TDARR_HISTORY_DAYS = 365
#   Emby / Jellyfin — active-stream / transcode / bandwidth trend (both brands).
EMBY_SAMPLE_INTERVAL_SECONDS = 300
EMBY_HISTORY_DAYS = 30
#   Forgejo — open-PR / open-issue review-queue backlog trend.
FORGEJO_SAMPLE_INTERVAL_SECONDS = 900
FORGEJO_HISTORY_DAYS = 90
#   GitSync Connector — synced-refs / alert / mapping trend + stale-pair threshold.
GITSYNC_SAMPLE_INTERVAL_SECONDS = 900
GITSYNC_HISTORY_DAYS = 90
GITSYNC_STALE_PAIR_HOURS = 24
#   Grafana — firing-alert / dashboard meta-monitor trend.
GRAFANA_SAMPLE_INTERVAL_SECONDS = 900
GRAFANA_HISTORY_DAYS = 90
#   Nginx Proxy Manager — proxy-host / plain-HTTP config-drift trend.
NPM_SAMPLE_INTERVAL_SECONDS = 1800
NPM_HISTORY_DAYS = 180
#   OPNsense — interface download/upload throughput + period data-volume trend.
OPNSENSE_SAMPLE_INTERVAL_SECONDS = 300
OPNSENSE_HISTORY_DAYS = 90
#   Speedtest Tracker — result-history sparkline window.
SPEEDTEST_SAMPLE_INTERVAL_SECONDS = 900
SPEEDTEST_HISTORY_DAYS = 365
#   ddns-updater — record-update activity trend. *_STALE_RECORD_HOURS = the
#   age after which a record with no successful update counts as "stale" on the card.
DDNS_SAMPLE_INTERVAL_SECONDS = 600
DDNS_HISTORY_DAYS = 90
DDNS_STALE_RECORD_HOURS = 24
...
# APC UPS card battery/load/runtime sparkline display window (days). APC has no
# sampler of its own — it reads the already-persisted host_snmp_samples, so this
# is purely the sparkline's display window (data is bounded by the SNMP sample
# retention). Default 7. Range 1..90.
APC_HISTORY_DAYS = 7
...
# Host OS-update (osupdate_host / /osupdate) SSH command wall-clock. An
# apt/yum dist-upgrade (+ optional firmware) can run many minutes; the update
# runs as a background job so this only bounds one host's run. 60..14400.
SSH_UPDATE_TIMEOUT_SECONDS = 1800
...
| `VERSION_POLL_INTERVAL_SECONDS`                | Runtime    | `60`                    | SPA's /api/version poll cadence in seconds (new-version "reload" banner detection lag); multiplied × 1000 before delivery via `client_config.version_poll_ms`. Range 10..3600.                                                                                                                                                                                                                |
...
| `IMAGE_PROXY_CACHE_TTL_SECONDS`                | Runtime    | `604800`                | Image-proxy disk cache TTL (seconds) — how long a server-side-fetched poster / avatar is served from `<data_dir>/image_cache/` before re-fetch. 0 disables. Range 0..2592000 (30d).                                                                                                                                                                                                           |
| `IMAGE_PROXY_CACHE_MAX_ENTRIES`                | Runtime    | `5000`                  | Hard cap on cached images; the oldest are pruned past it. 0 = no cap (TTL only). Range 0..100000.                                                                                                                                                                                                                                                                                             |
...
| `AI_MAX_TOKENS`                                | Runtime    | `1024`                  | Max output tokens per AI request (the budget we ASK FOR; provider caps actual). Reasoning models spend this on internal thinking BEFORE any visible text, so too low a value returns an empty answer. Not used by the per-provider Test button. Range 16..16384.                                                                                                                                                                                                                                                                                              |
...
| `APC_HISTORY_DAYS`                             | Runtime    | `7`                     | APC UPS card battery / load / runtime sparkline display window (days). Reads the already-persisted `host_snmp_samples` (no APC sampler); data is bounded by the SNMP sample retention. Range 1..90.                                                                                                                                                                                           |
...
| `PROXMOX_SAMPLE_INTERVAL_SECONDS`              | Runtime    | `900`                   | Proxmox cluster-resource sampler cadence — records each Proxmox chip's cluster CPU% / memory% / storage% for the card's resource-trend sparkline (Proxmox keeps no such rollup of its own). 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                              |
| `PROXMOX_HISTORY_DAYS`                         | Runtime    | `30`                    | Retention window (days) for the Proxmox cluster-resource samples driving the trend. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                             |
| `DOCKER_DIRECT_TIMEOUT_SECONDS`                | Runtime    | `20`                    | Per-call wall-clock budget for a Portainer-less Docker node managed over SSH (Admin → Docker Nodes) — connect + tunnel to the daemon socket + one Docker Engine API request. Bounds a slow / unreachable node. Range 3..300.                                                                                                                                                                    |
| `PORTAINER_HEALTH_PROBE_INTERVAL_SECONDS`      | Runtime    | `30`                    | How often the background probe checks Portainer reachability (`GET /api/status`). On an outage the fleet keeps showing the last-good services / containers / stacks + a banner, and Portainer write-ops fast-fail; the banner clears on recovery. Range 5..600.                                                                                                                                 |
...
| `TAUTULLI_SAMPLE_INTERVAL_SECONDS`             | Runtime    | `300`                   | Tautulli concurrent-stream sampler cadence — records each Tautulli chip's active-stream count (+ transcodes + bandwidth) for the card's streams-over-time sparkline + "peak N concurrent streams today" stat (an OmniGrid-owned trend, no per-render history round-trip). 0 = inherit the global stats interval. Range 0..86400.                                                                |
| `TAUTULLI_HISTORY_DAYS`                        | Runtime    | `30`                    | Retention window (days) for the Tautulli concurrent-stream samples driving the sparkline. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                       |
| `TRACEARR_SAMPLE_INTERVAL_SECONDS`             | Runtime    | `900`                   | Tracearr fleet sampler cadence — records each Tracearr chip's active-stream count + rolling 7-day violation count for the violation-rate trend + concurrency sparkline. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                 |
| `TRACEARR_HISTORY_DAYS`                        | Runtime    | `30`                    | Retention window (days) for the Tracearr fleet samples driving the violation-rate + concurrency trends. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                         |
...
| `ADGUARD_SAMPLE_INTERVAL_SECONDS`              | Runtime    | `900`                   | AdGuard Home blocked-query-% sampler cadence (fleet app). 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                              |
| `ADGUARD_HISTORY_DAYS`                         | Runtime    | `90`                    | Retention window (days) for the AdGuard Home blocked-% samples driving the card trend. Pruned hourly. Range 1..730.                                                                                                                                                                                                                                                                           |
| `PIHOLE_SAMPLE_INTERVAL_SECONDS`               | Runtime    | `900`                   | Pi-hole blocked-query-% sampler cadence (fleet app). 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                                   |
| `PIHOLE_HISTORY_DAYS`                          | Runtime    | `90`                    | Retention window (days) for the Pi-hole blocked-% samples driving the card trend. Pruned hourly. Range 1..730.                                                                                                                                                                                                                                                                                |
| `SERVARR_SAMPLE_INTERVAL_SECONDS`              | Runtime    | `900`                   | *arr family (Radarr / Sonarr / Lidarr / Readarr) library / queue sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                      |
| `SERVARR_HISTORY_DAYS`                         | Runtime    | `365`                   | Retention window (days) for the *arr library / queue samples driving the card trend. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                            |
| `PROWLARR_SAMPLE_INTERVAL_SECONDS`             | Runtime    | `900`                   | Prowlarr indexer-health sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                                               |
| `PROWLARR_HISTORY_DAYS`                        | Runtime    | `90`                    | Retention window (days) for the Prowlarr indexer-health samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                                                |
| `SEERR_SAMPLE_INTERVAL_SECONDS`                | Runtime    | `900`                   | Seerr (Overseerr / Jellyseerr) request-backlog sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                        |
| `SEERR_HISTORY_DAYS`                           | Runtime    | `90`                    | Retention window (days) for the Seerr request-backlog samples. Pruned hourly. Range 1..730.                                                                                                                                                                                                                                                                                                   |
| `QBITTORRENT_SAMPLE_INTERVAL_SECONDS`          | Runtime    | `300`                   | qBittorrent active-torrent / transfer-rate sampler cadence (finer to catch bursty peaks). 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                              |
| `QBITTORRENT_HISTORY_DAYS`                     | Runtime    | `30`                    | Retention window (days) for the qBittorrent transfer samples driving the sparkline. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                             |
| `KAVITA_SAMPLE_INTERVAL_SECONDS`               | Runtime    | `900`                   | Kavita library item-count sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                                             |
| `KAVITA_HISTORY_DAYS`                          | Runtime    | `365`                   | Retention window (days) for the Kavita library samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                                                         |
| `TDARR_SAMPLE_INTERVAL_SECONDS`                | Runtime    | `900`                   | Tdarr transcode-queue / health sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                                        |
| `TDARR_HISTORY_DAYS`                           | Runtime    | `365`                   | Retention window (days) for the Tdarr queue / health samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                                                   |
| `EMBY_SAMPLE_INTERVAL_SECONDS`                 | Runtime    | `300`                   | Emby / Jellyfin streaming sampler cadence (shared by both brands). 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                     |
| `EMBY_HISTORY_DAYS`                            | Runtime    | `30`                    | Retention window (days) for the Emby / Jellyfin active-stream samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                                          |
| `FORGEJO_SAMPLE_INTERVAL_SECONDS`              | Runtime    | `900`                   | Forgejo review-queue backlog sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                                          |
| `FORGEJO_HISTORY_DAYS`                         | Runtime    | `90`                    | Retention window (days) for the Forgejo open-PR / open-issue backlog samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                                   |
| `GITSYNC_SAMPLE_INTERVAL_SECONDS`              | Runtime    | `900`                   | GitSync Connector synced-refs / alert / mapping sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                       |
| `GITSYNC_HISTORY_DAYS`                         | Runtime    | `90`                    | Retention window (days) for the GitSync mapping / alert samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                                                |
| `GITSYNC_STALE_PAIR_HOURS`                     | Runtime    | `24`                    | A GitSync sync pair is "stale" when its last sync is older than this many hours (or it never synced). Drives the card's stale-pair stat. Range 1..720.                                                                                                                                                                                                                                        |
| `GRAFANA_SAMPLE_INTERVAL_SECONDS`              | Runtime    | `900`                   | Grafana firing-alert / dashboard meta-monitor sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                         |
| `GRAFANA_HISTORY_DAYS`                         | Runtime    | `90`                    | Retention window (days) for the Grafana firing-alert / dashboard samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                                       |
| `NPM_SAMPLE_INTERVAL_SECONDS`                  | Runtime    | `1800`                  | Nginx Proxy Manager config-drift sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                                      |
| `NPM_HISTORY_DAYS`                             | Runtime    | `180`                   | Retention window (days) for the NPM proxy-host / plain-HTTP / certs-expiring samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                           |
| `OPNSENSE_SAMPLE_INTERVAL_SECONDS`             | Runtime    | `300`                   | OPNsense interface-throughput sampler cadence. 0 = inherit the global stats interval. Keep ≤ 900s so each tick has a usable rate. Range 0..86400.                                                                                                                                                                                                                                              |
| `OPNSENSE_HISTORY_DAYS`                        | Runtime    | `90`                    | Retention window (days) for the OPNsense interface download/upload throughput samples. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                          |
| `SPEEDTEST_SAMPLE_INTERVAL_SECONDS`            | Runtime    | `900`                   | Speedtest Tracker result-sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                                              |
| `SPEEDTEST_HISTORY_DAYS`                       | Runtime    | `365`                   | Retention window (days) for the Speedtest Tracker result samples driving the sparkline. Pruned hourly. Range 1..1095.                                                                                                                                                                                                                                                                         |
| `DDNS_SAMPLE_INTERVAL_SECONDS`                 | Runtime    | `600`                   | ddns-updater record-update activity sampler cadence. 0 = inherit the global stats interval. Range 0..86400.                                                                                                                                                                                                                                                                                   |
| `DDNS_HISTORY_DAYS`                            | Runtime    | `90`                    | Retention window (days) for the ddns-updater activity samples. Pruned hourly. Range 1..730.                                                                                                                                                                                                                                                                                                   |
| `DDNS_STALE_RECORD_HOURS`                      | Runtime    | `24`                    | Age (hours) after which a ddns-updater record with no successful update counts as "stale" on the card. Range 1..8760.                                                                                                                                                                                                                                                                         |
...
| `SSH_UPDATE_TIMEOUT_SECONDS`                   | Runtime    | `1800`                  | Host OS-update (`osupdate_host` / `/osupdate`) SSH command wall-clock. An apt/yum dist-upgrade (+ optional rpi-update firmware) can run many minutes; the update runs as a background Operation so this only bounds one host's run. Range 60..14400.                                                                                                                                              |
