...
`SCHEDULE_KINDS` dict. Add a UI option if the operator should see it in the create-schedule
dropdown.
...
Operator-confirmed (session 2026-04-25): a `Nightly backup` row that had been drifting with
`cadence_mode=''` — firing via interval inference rather than at the 01:00 anchor — was re-saved
from the UI and now fires daily at the configured clock time. If an operator reports "my daily
schedule isn't firing at the right time," check `cadence_mode` in the DB row first; a blank
