...
async def _tool_upcoming_releases(args: dict, _ctx: dict) -> dict:
    ...
    runtime_min. Delegates to the shared ``arr_calendar.upcoming_items`` (also
    used by the Telegram ``/upcoming`` command). Never raises — a fetch failure
    comes back in the result."""
    import asyncio as _asyncio  # noqa: PLC0415
    from logic.apps.arr_calendar import upcoming_items  # noqa: PLC0415
    try:
    ...
    if isinstance(ctx, dict):
        ...
        if problem_hosts:
            parts.append(_format_records_block(
                ...
                "problem hosts', 'name the unknowns'. CRITICAL — these are "
                "a MIXED bag of statuses; GROUP the bullets BY each host's "
                "`status` field into SEPARATE sections (Down / Unknown / "
                "Paused) and label each host with its REAL status. An "
                "`unknown` or `paused` host is NOT 'down' — do NOT lump them "
                "under a single 'Down' heading. The number of DOWN hosts is "
                "EXACTLY `hosts_down` above (status == 'down'), which is "
                "usually FAR smaller than the total problem count; never "
                "report every problem host as 'down'. Emit `id (status)` per "
                "host. DO NOT say 'there are N unknown hosts' without naming "
