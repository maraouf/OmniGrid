...
# 1 GiB in bytes — Lidarr reports disk space in bytes; the card shows GiB
# (matching Lidarr's own UI).
_GIB = 1024 ** 3
...
async def _fetch_version(cli: httpx.AsyncClient, base: str, key: str) -> str:
    """Best-effort Lidarr version via ``GET /api/v1/system/status``. ``''`` on
...
# module's test_credential (radarr / sonarr / …) — the deliberate per-app
...
async def test_credential(host_row: dict, chip: dict, candidate_key: str, **_kw) -> dict:
    """Probe Lidarr's auth-required ``/api/v1/system/status`` with the supplied
    ...
    if r.status_code == 200:
        ver = _version_from(r)
        return {"ok": True, "detail": f"OK (Lidarr {ver})" if ver else "OK",
                "status": 200}
...
def _fmt_size_gib(gib: Any) -> str:
    ...
    (matches Lidarr's GiB / TiB display). ``Any`` arg — callers pass raw
...
def _resolve_skill_target(host_row: dict, chip: dict) -> "tuple[str, str, Optional[dict]]":
    api_key = (chip.get("api_key") or "").strip()
    if not api_key:
        return "", "", {"ok": False, "status": 0, "detail": "Lidarr api_key not set"}
...
async def _command_skill(host_row: dict, chip: dict, *, command: str,
                         started_msg: str,
                         host_id: Optional[str] = None) -> dict:
    """Action skill: POST a non-destructive background command to Lidarr's
    ``/api/v1/command`` endpoint. Never raises."""
    api_key, base, err = _resolve_skill_target(host_row, chip)
    if err:
        return err
    print(f"[lidarr] INFO command host={host_id} name={command!r}")
    try:
        async with httpx.AsyncClient(verify=False, timeout=20.0,
                                     follow_redirects=True) as cli:
        ...
        print(f"[lidarr] warning: command {command!r} failed — {type(e).__name__}: {e}")
    ...
    if r.status_code in (401, 403):
        return {"ok": False, "status": r.status_code, "detail": "auth failed (check Lidarr api_key)"}
    _body = ""
    try:
        _body = (r.text or "")[:160]
    except (ValueError, TypeError):
    ...
    return {"ok": False, "status": r.status_code,
            "detail": f"Lidarr returned HTTP {r.status_code} for {command}"
