...
async def _lifespan(_app: FastAPI):
    ...
    # it. Spawned as a FIRE-AND-FORGET background task (NOT awaited)
    # because each unresolvable host can hang `getaddrinfo` for
    # several seconds — with a 172-host fleet where 58 are
    # unresolvable, awaiting this would push lifespan past the
    # Docker healthcheck start_period (120s) and crash-loop the
    # container at boot. The diagnostic still lands in logs
    # whenever it finishes (typically <30s for a clean fleet, up
    # to a few minutes for a fleet with many unresolvable short
    # names — but the app is alive the entire time). Each
    # `getaddrinfo` call is also wrapped in a short per-call
    # timeout so a single deadlocked DNS server can't stall the
    # whole sweep.
    try:
        ...
        def _dns_probe_all() -> dict:
            ...
            fail = 0
            failed: list[str] = []
            # Per-call default timeout — libc's getaddrinfo respects
            # `socket.setdefaulttimeout()`. 2s per probe means even
            # a fleet of 200 unresolvable hosts caps at ~7 minutes
            # max in the worker thread (and clean entries take ~ms).
        ...
        async def _run_dns_check_bg():
            """Run the DNS check in a worker thread; log the result
            when it finishes. NOT awaited from lifespan."""
            try:
                _dns_result = await asyncio.to_thread(_dns_probe_all)
                _ok = _dns_result["ok_count"]
                _fail = _dns_result["fail_count"]
                if _fail == 0:
                    print(f"[boot] DNS startup check: all {_ok} curated host "
                          f"targets resolved cleanly")
                else:
                    ...
                    print(f"[boot] DNS startup check warning: {_fail} of "
                          ...
                          f"(first {len(_dns_result['samples_failed'])}: "
                          f"{_samples}) — check Docker DNS / use FQDNs / "
                          f"add `extra_hosts:` in docker-compose.yml")
            except Exception as _bg_e:  # noqa: BLE001
                print(f"[boot] DNS startup check skipped (non-fatal): {_bg_e}")

        # Fire-and-forget via the canonical `spawn_background_task`
        # helper — keeps a strong ref so asyncio doesn't GC the task
        # mid-execution + logs creation/completion in the `[bg]`
        # tag so a vanished task is visible in Admin → Logs.
        # Lifespan continues immediately; the DNS check log appears
        # whenever the sweep finishes.
        spawn_background_task(_run_dns_check_bg(), label="boot-dns-check")
    except Exception as _dns_e:  # noqa: BLE001
        print(f"[boot] DNS startup check spawn failed (non-fatal): {_dns_e}")
    ...
    # Proxmox cluster-resource sampler — records each Proxmox chip's cluster
    # CPU% / memory% / storage% per tick (tuning_proxmox_sample_interval_seconds,
    # default 900s; 0 = inherit the global stats interval) into proxmox_samples,
    # driving the card's cluster-resource trend (Proxmox keeps no such rollup of
    # its own). Dormant-cheap when no Proxmox chip is pinned.
    from logic.apps import proxmox_sampler as _proxmox_sampler
    proxmox_sampler = asyncio.create_task(
        _proxmox_sampler.proxmox_sampler_loop(),
        name="proxmox-sampler",
    )
    ...
    # AdGuard Home Sync reliability sampler — records each configured AGS chip's
    # replica in-sync count every tuning_adguardsync_sample_interval_seconds
    # (default 900s; 0 = inherit the global stats interval) into
    # adguardhome_sync_samples, driving the card's sync-reliability trend (the
    # sync tool keeps no history of its own). Dormant-cheap when no AGS chip is
    # pinned.
    from logic.apps import adguardhome_sync_sampler as _adguardsync_sampler
    adguardsync_sampler = asyncio.create_task(
        _adguardsync_sampler.adguardhome_sync_sampler_loop(),
        name="adguardsync-sampler",
    )
    ...
    # Bazarr subtitle-backlog retention sampler — snapshots each chip's
    # missing-subtitle counts (episodes + movies) every
    # tuning_bazarr_sample_interval_seconds (default 900s; 0 = inherit the global
    # stats interval) into bazarr_samples, driving the card's backlog-over-time
    # sparkline + "backlog down N this week" stat (Bazarr keeps no wanted-count
    # history of its own). Dormant-cheap when no Bazarr chip is pinned.
    from logic.apps import bazarr_sampler as _bazarr_sampler
    bazarr_sampler = asyncio.create_task(
        _bazarr_sampler.bazarr_sampler_loop(),
        name="bazarr-sampler",
    )
    # Plex concurrent-stream retention sampler — snapshots each chip's active
    # session count (+ transcodes + bandwidth) every
    # tuning_plex_sample_interval_seconds (default 300s; 0 = inherit the global
    # stats interval) into plex_samples, driving the card's streams-over-time
    # sparkline + "peak N concurrent streams today" stat (Plex's PMS keeps no
    # easy long history). Dormant-cheap when no Plex chip is pinned.
    from logic.apps import plex_sampler as _plex_sampler
    plex_sampler = asyncio.create_task(
    ...
    tracearr_sampler = asyncio.create_task(
        _tracearr_sampler.tracearr_sampler_loop(),
        name="tracearr-sampler",
    )
    ...
    from logic.apps import tdarr_sampler as _tdarr_sampler
    tdarr_sampler = asyncio.create_task(
        _tdarr_sampler.tdarr_sampler_loop(),
        name="tdarr-sampler",
    )
    # Emby / Jellyfin streaming retention sampler (SHARED by both brands) —
    # snapshots active-stream / transcode / bandwidth gauges so the card can
    # draw a 'peak N streams today' stat + a daily peak-streams sparkline.
    # Dormant-cheap when no Emby / Jellyfin chip is pinned.
    from logic.apps import emby_sampler as _emby_sampler
    emby_sampler = asyncio.create_task(
    ...
    # GitSync Connector mirror retention sampler — snapshots synced-refs / alert
    # / mapping totals so the card can draw a mappings-growth + alert trend.
    # Dormant-cheap when no GitSync chip is pinned.
    from logic.apps import gitsync_sampler as _gitsync_sampler
    gitsync_sampler = asyncio.create_task(
    ...
    # OPNsense interface-throughput retention sampler — drives the period
    # data-volume + peak / average throughput trend. Dormant-cheap when no
    # OPNsense chip is pinned.
    from logic.apps import opnsense_sampler as _opnsense_sampler
    opnsense_sampler = asyncio.create_task(
        _opnsense_sampler.opnsense_sampler_loop(),
        name="opnsense-sampler",
    )
    ...
    kavita_sampler = asyncio.create_task(
...
# The init_db() boot orchestrator (core-table creation + the additive
# ALTER list + module schema hooks + numbered migrations + built-in
# seeds) was extracted to logic/schema.py; it's imported below and
# called from main.py's boot wrapper.
# ============================================================================
