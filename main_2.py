...
`from main_2 import *` at the bottom of `main.py`, which pulls
...
Loading order:
  ...
  2. main.py end: `from main_2 import *` triggers main_2 load.
  3. main_2.py top: `from main import *` pulls EVERY symbol
     main has defined so far (`app`, helpers, Pydantic models,
     etc.) into main_2's namespace so the route decorators
     below can reference them.
  4. main_2.py body runs; routes register against the shared
     `app` instance.
  5. main_2.py finishes; control returns to main.py's star-
     import which now has every main_2 symbol available.
"""
"""
