"""Admin → Stats dashboard endpoints — `/api/admin/stats/overview`,
`/database`, `/network`, plus the AI fallback-chain resolver
helpers consumed by both stats and AI admin pages.
...
Chain: main → main_pkg.settings_routes → main_pkg.admin_ai_routes → main_pkg.apps_routes
→ main_pkg.hosts_routes → ... → main_pkg.auth_routes.
settings_routes loads at main's tail BEFORE core so its symbols are in
...
# "Unresolved reference" without this hint block. Listing the names
# inside a TYPE_CHECKING block tells the analyzer where they come
# from WITHOUT running the imports at runtime (TYPE_CHECKING is False
# then), so a name defined in a LATER sibling chunk can't fail at
# child-load time.
...
# ----------------------------------------------------------------------------
# Row-shape helpers — SQLite cursors return either Row objects (with
# `keys()` for named access) OR plain tuples depending on whether the
# caller installed `row_factory = sqlite3.Row`. Wrapping the
# `r["name"] if hasattr(r, "keys") else r[idx]` ternary as small
# helpers de-dupes the same shape across the AI-stats + network-stats
# query blocks. Returning the raw value keeps the consumer free to
# pick `int()` / `float()` / pass-through as needed.
# ----------------------------------------------------------------------------
def _row_get(r, name: str, idx: int):
    """Return ``r[name]`` when the row supports keyed access, else
    ``r[idx]``. Backs every per-field accessor below."""
    return r[name] if hasattr(r, "keys") else r[idx]


def _unpack_net_row(r) -> tuple[str, int, int]:
    """Unpack a `(host_id, bytes_rx, bytes_tx)` row from the
    `host_net_samples` queries used by `api_admin_stats_network`.
    `bytes_*` are int-coerced + `None`-tolerant (the SUM(...) +
    WHERE-no-rows path can yield NULL)."""
    hid = _row_get(r, "host_id", 0)
    bx = int(_row_get(r, "bytes_rx", 1) or 0)
    bt = int(_row_get(r, "bytes_tx", 2) or 0)
    return hid, bx, bt


def _unpack_jobs_summary(r) -> dict:
    """Unpack a `(jobs, tokens, cost)` summary row from the
    `ai_jobs` rollup queries used by the MTD / last-month panels of
    `api_admin_stats_ai_cost`. Returns the dict shape the SPA expects
    so the two consumer sites just `out[...] = _unpack_jobs_summary(r)`."""
    return {
        "jobs": int(_row_get(r, "jobs", 0)),
        "tokens": int(_row_get(r, "tokens", 1)),
        "cost_usd": float(_row_get(r, "cost", 2)),
    }
...
def _ai_supported_providers() -> tuple[str, ...]:
    """The tuple of AI provider names OmniGrid supports (from logic.ai)."""
...
def _compute_admin_stats_summary() -> dict:
    """Compute the Stats dashboard summary cards (samplers / total samples / incidents / network / AI cost + jobs / DB size)."""
    import os as _os
    import time as _time
...
def _compute_admin_stats_database() -> dict:
    """Compute the Stats database page (DB size + per-table row counts + size history)."""
    import os as _os
    out: dict = {
...
def _compute_admin_stats_samples(range_str: str = "90d") -> dict:
    """Compute the Stats samples page — per-(sample table) row counts + per-host breakdown over the requested window."""
