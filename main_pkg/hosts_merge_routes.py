"""main_pkg.hosts_merge_routes — the Hosts merge / shape / provider-cache pipeline.

Extracted from main_pkg.apps_routes (which crossed the line-count threshold).
Holds GET /api/hosts + the whole per-host provider probe + merge + shape
machinery: _get_host_provider_state / _do_host_provider_probe / _merge_one_host
/ _shape_host_api_row / _shape_host_apps, the per-(provider, host) caches +
locks, the background gather/stats task handles, detected-ports, and the asset
/ ping / swarm-node resolvers. Loaded via apps_routes's tail star-import so its
routes register before auth_routes mounts the static catch-all; its cross-
module underscore helpers are wired by main._wire_cross_module_underscore_globals.
"""
