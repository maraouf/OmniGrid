"""Per-host SSH endpoints + host-debug surfaces —
`/api/debug/subject`, `/api/hosts/debug`,
`/api/hosts/{id}/ssh/{status,test,run}` + the SSH terminal
WebSocket upgrade at `/api/hosts/{id}/ssh/terminal`.
...
# Explicit re-imports for PyCharm static analysis — the star-import
# above propagates these at runtime but PyCharm cannot trace `from X
# import *` to resolve symbol references. Each name listed here is
# actually consumed below; truly-unused stdlib imports are pruned.
# noinspection PyUnresolvedReferences
from main import (  # noqa: E402,F401,F811
    app,
    AdminUser,
    Request,
    db_conn,
...
# at runtime AND gives PyCharm a real symbol to bind against.
from main_pkg.hosts_routes import _load_hosts_config  # noqa: E402
import asyncio  # noqa: F401,F811  (used at runtime + re-export safety; star-import shadows)
import json  # noqa: F401,F811
import os  # noqa: F401,F811
import time  # noqa: F401,F811
import uuid  # noqa: F401,F811
from typing import Any, Optional  # noqa: F401,F811
    ...
    if not sid and hid:
        ...
        if not series:
            try:
                ...
                curated = []
            row = next((r for r in curated if isinstance(r, dict) and r.get("id") == hid), None)
            if isinstance(row, dict) and (row.get("pulse_name") or "").strip():
...
from main_pkg.hosts_provider_routes import *  # noqa: E402,F401,F403
