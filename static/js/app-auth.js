...
//
// Auth tab in Admin → Authentication uses `_totpPolicyBaseline` /
// `_totpPolicySnapshot` / `totpPolicyDirty` / `saveTotpPolicy` for the
// admin-side per-policy save.
//
// Profile → Security uses `startTotpEnrol` / `confirmTotpEnrol` /
// `cancelTotpEnrol` / `regenerateTotpCodes` / `disableTotpSelf` /
// `loadTotpStatus` for the user-self flow, plus `addPasskey` /
// `revokePasskey` / `loadPasskeys` / `passkeyBrowserSupported` for the
// WebAuthn surface.
//
// Self-service password change lives in `changePassword` + `passwordForm`
// + `passwordBusy` + `passwordStrength`.
//
// Phase 2, Batch 8 of the static/js/app.js modularisation.
