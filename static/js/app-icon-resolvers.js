...
//
// These functions resolve a service / host / stack name to a local
// `static/img/icons/<slug>.svg` URL, with a theme-aware variant swap
// (`<slug>-dark.svg` on dark theme for any slug in the `KNOWN_DARK_ICONS`
// set). The keyword-scan tables embedded in `iconUrlFor` and `hostIconUrl`
// are intentionally long — every entry maps one brand keyword (or alias)
// to its canonical slug. Ordering matters: longer / more-specific phrases
// MUST come before shorter ones (e.g. `nginx proxy manager` BEFORE
// `nginx`, `windows server` BEFORE `windows`). First match wins.
//
// Pure-ish helpers — `_themeIcon` reads `this.themePref` to pick variant;
// `hostIconUrl` reads `h.icon` / `h.label` / `h.id` / `h.host` from the
// passed-in host. Otherwise no `this`-state I/O.
//
// Phase 2, Batch 3 of the static/js/app.js modularisation.
