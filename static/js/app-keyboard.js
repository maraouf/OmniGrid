...
//
// `handleHotkey` is bound to `window.keydown` at component init and
// dispatches every modifier-aware shortcut (Cmd-K palette open, '?' for
// the hotkey cheat-sheet, ESC ordering for drawer / modal stacking, the
// drawer next/prev arrow-key nav). `hotkeyGroups` produces the
// translated cheat-sheet content rendered by the modal.
//
// `_buttonGroupArrowKey` is the reusable handler bound to button-group
// wrappers (range pickers, segmented controls) for left/right arrow
// keyboard navigation with RTL awareness.
//
// `showHotkeys` is the reactive boolean that drives the cheat-sheet
// modal's visibility.
//
// Phase 2, Batch 4 of the static/js/app.js modularisation.
