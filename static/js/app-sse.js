...
//
// `_initSSE` opens the `/api/events` EventSource and registers handlers
// for every event type published by the in-process bus (see
// `docs/guidelines/api.md` "Real-time events" for the wire contract).
// `_isSelfEvent` cross-references the event's `client_id` against the
// per-tab `window.__ogClientId` so a tab doesn't react to its own
// echo. `_handleOpEvent` routes `op:*` events into the activeOps list +
// history reconcile. `sseStatusKey` is the i18n key for the topbar
// SSE connection-state pill.
//
// Phase 2, Batch 5 of the static/js/app.js modularisation.
