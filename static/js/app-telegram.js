...
//
// Two related surfaces here:
//   1. Settings → Profile → Telegram: generate / show / expire / unlink the
//      per-user `/link <code>` bot pairing (one-time codes minted by the
//      bot listener, ~10 min validity).
//   2. Admin → Notifications → Telegram: Test-before-Save gate on the bot
//      token + chat_id pair, plus the admin unlink-row helper.
//
// SSE `telegram:linked` / `telegram:unlinked` handlers live in `app-sse.js`;
// those handlers call `loadTelegramLinks()` from here.
//
// Phase 2, Batch 7 of the static/js/app.js modularisation.
