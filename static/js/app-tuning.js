...
// surfaced via `GET /api/admin/tuning`. The form renders one row per
// knob with bounds chips + effective-value pill (DB > env > default).
//
// State + helpers for the editor; `relocatedTuningKeys` is the
// special-case array of keys that were migrated from plain settings
// and now live as `tuning_*`. `_perProviderTuneKeys` is the lookup map
// that section-owned save handlers use to decide which tunables they
// should ship in their own POST.
//
// Phase 2, Batch 29 of the static/js/app.js modularisation.
